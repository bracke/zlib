...
procedure Streaming_Roundtrip_Gzip is
...
begin
   ...
   declare
      Decoded : constant Zlib.Byte_Array :=
        Zlib.Inflate_With_Header (To_Byte_Array (Compressed), Zlib.GZip, Status);
   begin
   ...
   Ada.Text_IO.Put_Line ("streaming gzip roundtrip ok");
exception
   when Zlib.Zlib_Error =>
      Ada.Text_IO.Put_Line ("streaming gzip roundtrip zlib error");
      Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
   when Zlib.Status_Error =>
      Ada.Text_IO.Put_Line ("streaming gzip roundtrip lifecycle error");
      Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
end Streaming_Roundtrip_Gzip;
