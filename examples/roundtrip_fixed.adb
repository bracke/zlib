...
begin
   ...
   begin
      if Status /= Zlib.Ok then
         Ada.Text_IO.Put_Line ("fixed compression failed: " & Zlib.Status_Image (Status));
      ...
      begin
         if Status = Zlib.Ok and then Output = Input then
            Ada.Text_IO.Put_Line ("fixed roundtrip ok");
         else
            Ada.Text_IO.Put_Line ("fixed inflate failed: " & Zlib.Status_Image (Status));
            Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
         end if;
      end;
   end;
