...
procedure Streaming_Deflate_Zlib is
...
exception
   when Zlib.Zlib_Error =>
      Ada.Text_IO.Put_Line ("streaming zlib compression failed");
      Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
      if Zlib.Is_Open (Filter) then
         Zlib.Compress_Close (Filter, Ignore_Error => True);
      end if;
   when Zlib.Status_Error =>
      Ada.Text_IO.Put_Line ("streaming zlib compression lifecycle misuse");
      Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
end Streaming_Deflate_Zlib;
