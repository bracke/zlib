...
package body Zlib_Streaming_Compress_Raw_Auto_Tests is
   ...
   begin
      return AUnit.Format ("Zlib streaming raw Auto Deflate compression");
   end Name;
   ...
   begin
      ...
      while Input_Data'Length > 0 and then Next_Input <= Input_Data'Last loop
         ...
         begin
            ...
            Calls := Calls + 1;
            Assert (Calls < 1_000_000, "raw Auto input loop must make progress");
         end;
      end loop;
      ...
      while not Zlib.Compress_Stream_End (Filter) loop
         ...
         begin
            ...
            Append (Out_Buffer, Out_Last, Output, Out_Count);
            Calls := Calls + 1;
            Assert (Calls < 1_000_000, "raw Auto finish loop must make progress");
         end;
      end loop;
   ...
   is
      pragma Unreferenced (T);
      Output : constant Zlib.Byte_Array := Compress_Raw_Auto (Empty, Output_Buffer_Size => 1);
   begin
      Assert_Raw_Inflates (Output, Empty, "streaming raw Auto empty");
   end Test_Empty;
   ...
   begin
      Assert_Raw_Inflates
        (Compress_Raw_Auto (Input, Input_Chunk_Size => 1, Output_Buffer_Size => 1),
         Input,
         "streaming raw Auto hello one-byte buffers");
   ...
   begin
      Assert_Raw_Inflates
        (Compress_Raw_Auto (Input, Input_Chunk_Size => 1, Output_Buffer_Size => 1),
         Input,
         "streaming raw Auto Git-shaped payload");
   end Test_Git_Shaped;
   ...
   begin
      ...
      Assert (not Zlib.Compress_Stream_End (Filter), "raw Auto stream end false before drain");
      while not Zlib.Compress_Stream_End (Filter) loop
         Zlib.Compress_Flush (Filter, Output, Out_Last, Zlib.Finish);
      end loop;
      Assert (Zlib.Compress_Stream_End (Filter), "raw Auto stream end true after drain");
      Zlib.Compress_Close (Filter);
      ...
      Assert (Raised, "raw Auto close before Finish raises");
   ...
   is
      pragma Unreferenced (T);
      Input : Zlib.Byte_Array (1 .. 2048);
   begin
      for I in Input'Range loop
         Input (I) := Zlib.Byte ((I * 43) mod 256);
      end loop;

      Assert_Same
        (Compress_Raw_Auto (Input, Input_Chunk_Size => 17, Output_Buffer_Size => 31),
         Compress_Raw_Auto (Input, Input_Chunk_Size => 17, Output_Buffer_Size => 31),
         "streaming raw Auto deterministic output");
   ...
   begin
      Registration.Register_Routine (T, Test_Empty'Access, "streaming raw Auto empty input");
      Registration.Register_Routine
        (T, Test_Hello_One_Byte_Buffers'Access,
         "streaming raw Auto hello one-byte buffers");
      Registration.Register_Routine (T, Test_Binary'Access, "streaming raw Auto binary payload");
      Registration.Register_Routine (T, Test_Repeated'Access, "streaming raw Auto repeated payload");
      Registration.Register_Routine (T, Test_Git_Shaped'Access, "streaming raw Auto Git-shaped payload");
      Registration.Register_Routine
        (T, Test_Large_Multiple_Blocks'Access,
         "streaming raw Auto large multiple blocks");
      Registration.Register_Routine (T, Test_Stream_End_And_Close'Access, "streaming raw Auto stream end and close");
      Registration.Register_Routine (T, Test_Deterministic'Access, "streaming raw Auto deterministic output");
   end Register_Tests;
end Zlib_Streaming_Compress_Raw_Auto_Tests;
