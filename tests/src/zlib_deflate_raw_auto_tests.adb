...
package body Zlib_Deflate_Raw_Auto_Tests is
   use type Zlib.Byte;
   ...
   begin
      return AUnit.Format ("Zlib one-shot raw Auto Deflate compression");
   end Name;
   ...
   is
      ...
      Compressed : constant Zlib.Byte_Array :=
        Zlib.Deflate_Raw (Input, Zlib.Auto, Status);
   ...
   begin
      Assert (Status = Zlib.Ok, Message & ": Deflate_Raw Auto status");
   ...
   is
      pragma Unreferenced (T);
   begin
      Assert_Roundtrip (Empty, "empty raw Auto");
   end Test_Empty;
   ...
   is
      pragma Unreferenced (T);
   begin
      Assert_Roundtrip (Hello, "hello raw Auto");
   end Test_Hello;
   ...
   begin
      ...
      Assert_Roundtrip (Input, "binary raw Auto");
   end Test_Binary;
   ...
   begin
      ...
      Assert_Roundtrip (Input, "repeated raw Auto");
   end Test_Repeated;
   ...
   is
      pragma Unreferenced (T);
      Input_Path  : constant String := "raw_Auto_input.bin";
      Output_Path : constant String := "raw_Auto_output.deflate";
      Status      : Zlib.Status_Code := Zlib.Ok;
   ...
   begin
      ...
      Assert (Status = Zlib.Ok, "Deflate_Raw_File Auto must succeed");
      Assert_Roundtrip (Input, "raw Auto file source payload");
      Assert_Same
        (Zlib.Inflate_With_Header (Read_File (Output_Path), Zlib.Raw_Deflate, Status),
         Input,
         "raw Auto file output roundtrip");
      Assert (Status = Zlib.Ok, "raw Auto file inflate status");
   ...
   is
      ...
      Raw_Output    : constant Zlib.Byte_Array :=
        Zlib.Deflate_Raw (Hello, Zlib.Auto, Raw_Deflate_Status);
      Raw_Status    : Zlib.Status_Code := Zlib.Ok;
      Zlib_Status   : Zlib.Status_Code := Zlib.Ok;
      GZip_Status   : Zlib.Status_Code := Zlib.Ok;
      Zlib_Output   : constant Zlib.Byte_Array :=
        Zlib.Deflate (Hello, Zlib.Auto, Zlib_Wrapped_Status);
      ...
      GZip_Output   : constant Zlib.Byte_Array :=
        Zlib.GZip (Hello, Zlib.Auto, GZip_Wrapped_Status);
   ...
   begin
      ...
      Assert (Raw_Status = Zlib.Ok, "raw Auto output accepted by Raw_Deflate inflate");
      Assert_Same (Raw_Inflated, Hello, "raw Auto strictness roundtrip");
      Assert (Zlib_Status /= Zlib.Ok, "zlib Auto output rejected by Raw_Deflate inflate");
      Assert (GZip_Status /= Zlib.Ok, "gzip Auto output rejected by Raw_Deflate inflate");
   ...
   begin
      for I in Input'Range loop
         Input (I) := Zlib.Byte ((I * 17) mod 256);
      end loop;

      declare
         A : constant Zlib.Byte_Array := Zlib.Deflate_Raw (Input, Zlib.Auto, Status_A);
         B : constant Zlib.Byte_Array := Zlib.Deflate_Raw (Input, Zlib.Auto, Status_B);
      begin
         Assert (Status_A = Zlib.Ok and then Status_B = Zlib.Ok, "Deflate_Raw Auto deterministic statuses");
         Assert_Same (A, B, "Deflate_Raw Auto deterministic output");
      end;
   ...
   begin
      Registration.Register_Routine (T, Test_Empty'Access, "Deflate_Raw Auto empty roundtrips");
      Registration.Register_Routine (T, Test_Hello'Access, "Deflate_Raw Auto hello roundtrips");
      Registration.Register_Routine (T, Test_Binary'Access, "Deflate_Raw Auto binary roundtrips");
      Registration.Register_Routine (T, Test_Repeated'Access, "Deflate_Raw Auto repeated payload roundtrips");
      Registration.Register_Routine (T, Test_File'Access, "Deflate_Raw_File Auto roundtrips");
      Registration.Register_Routine (T, Test_Deterministic'Access, "Deflate_Raw Auto deterministic output");
      Registration.Register_Routine
        (T, Test_Wrapper_Strictness'Access,
         "raw Auto wrapper strictness");
   end Register_Tests;
end Zlib_Deflate_Raw_Auto_Tests;
