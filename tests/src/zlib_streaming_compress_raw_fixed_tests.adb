...
package body Zlib_Streaming_Compress_Raw_Fixed_Tests is
   ...
   begin
      return AUnit.Format ("Zlib streaming raw fixed Deflate compression");
   end Name;
   ...
   begin
      ...
      while Input_Data'Length > 0 and then Next_Input <= Input_Data'Last loop
         ...
         begin
            ...
            Calls := Calls + 1;
            Assert (Calls < 1_000_000, "raw fixed input loop must make progress");
         end;
      end loop;
      ...
      while not Zlib.Compress_Stream_End (Filter) loop
         ...
         begin
            ...
            Append (Out_Buffer, Out_Last, Output, Out_Count);
            Calls := Calls + 1;
            Assert (Calls < 1_000_000, "raw fixed finish loop must make progress");
         end;
      end loop;
   ...
   is
      pragma Unreferenced (T);
      Output : constant Zlib.Byte_Array := Compress_Raw_Fixed (Empty, Output_Buffer_Size => 1);
   begin
      Assert_Raw_Inflates (Output, Empty, "streaming raw fixed empty");
   end Test_Empty;
   ...
   begin
      Assert_Raw_Inflates
        (Compress_Raw_Fixed (Input, Input_Chunk_Size => 1, Output_Buffer_Size => 1),
         Input,
         "streaming raw fixed hello one-byte buffers");
   ...
   begin
      ...
      Assert_Raw_Inflates
        (Compress_Raw_Fixed (Input, Input_Chunk_Size => 4096, Output_Buffer_Size => 1),
         Input,
         "streaming raw fixed large");
   ...
   begin
      ...
      Assert (not Zlib.Compress_Stream_End (Filter), "raw fixed stream end false before drain");
      while not Zlib.Compress_Stream_End (Filter) loop
         Zlib.Compress_Flush (Filter, Output, Out_Last, Zlib.Finish);
      end loop;
      Assert (Zlib.Compress_Stream_End (Filter), "raw fixed stream end true after drain");
      Zlib.Compress_Close (Filter);
      ...
      Assert (Raised, "raw fixed close before Finish raises");
   ...
   begin
      Registration.Register_Routine (T, Test_Empty'Access, "streaming raw fixed empty input");
      Registration.Register_Routine
        (T, Test_Hello_One_Byte_Buffers'Access,
         "streaming raw fixed hello one-byte buffers");
      Registration.Register_Routine (T, Test_Binary'Access, "streaming raw fixed binary payload");
      Registration.Register_Routine (T, Test_Large_Multiple_Blocks'Access, "streaming raw fixed large multiple blocks");
      Registration.Register_Routine (T, Test_Stream_End_And_Close'Access, "streaming raw fixed stream end and close");
   end Register_Tests;
end Zlib_Streaming_Compress_Raw_Fixed_Tests;
