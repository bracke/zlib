...
Streaming compression is safe for parallel callers when each Ada task uses its
own `Compression_Filter_Type` object and caller-owned input/output buffers. A
compression filter is mutable session state and is not internally synchronized;
protect it externally if it is shared between tasks.
...
are emitted. Level-based APIs may map to Auto while preserving the same
block-local behavior and configured level policy.
