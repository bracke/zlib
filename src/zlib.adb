...
package body Zlib is
   ...
   LZMA_Max_Fast_Match_Length : constant Natural := 17;
   ...
   LZMA_Max_Search_Distance : constant Natural := Natural (LZMA_Default_Dict);
   ...
   begin
      ...
      while In_Index <= Plain'Last loop
         declare
            ...
            Rep_Len0  : Natural := 0;
         begin
            if Rep0 > 0 and then Rep0 <= Position then
               while In_Index + Rep_Len0 <= Plain'Last
                 and then Rep_Len0 < LZMA_Max_Fast_Match_Length
                 and then Plain (In_Index + Rep_Len0) =
                   Plain (In_Index + Rep_Len0 - Rep0)
               loop
                  Rep_Len0 := Rep_Len0 + 1;
               end loop;
            end if;

            for Distance in 1 .. Natural'Min (LZMA_Max_Search_Distance, Position) loop
               declare
                  Candidate_Len : Natural := 0;
               begin
                  while In_Index + Candidate_Len <= Plain'Last
                    and then Candidate_Len < LZMA_Max_Fast_Match_Length
                    and then
                      Plain (In_Index + Candidate_Len) =
                      Plain (In_Index + Candidate_Len - Distance)
                  loop
            ...
            if Best_Len >= LZMA_Min_Match_Length
              or else Rep_Len0 >= LZMA_Min_Match_Length
            then
               LZMA_Encode_Bit
                 (E, Is_Match (State * LZMA_Num_Pos_States_Max + Pos_State), 1);
               if Rep_Len0 >= LZMA_Min_Match_Length
                 and then Rep_Len0 >= Best_Len
               then
                  LZMA_Encode_Bit (E, Is_Rep (State), 1);
                  LZMA_Encode_Bit (E, Is_Rep_G0 (State), 0);
                  LZMA_Encode_Bit
                    (E, Is_Rep0_Long (State * LZMA_Num_Pos_States_Max + Pos_State), 1);
                  LZMA_Encode_Len
                    (E, Rep_Len, Pos_State,
                     Rep_Len0 - LZMA_Min_Match_Length);
                  State := LZMA_Rep_State_After (State);
                  Prev := Plain (In_Index + Rep_Len0 - 1);
                  Position := Position + Rep_Len0;
                  In_Index := In_Index + Rep_Len0;
               else
                  LZMA_Encode_Bit (E, Is_Rep (State), 0);
                  LZMA_Encode_Len
                    (E, Match_Len, Pos_State,
                     Best_Len - LZMA_Min_Match_Length);
                  LZMA_Encode_Distance
                    (E, Pos_Slot, Pos_Special, Pos_Align, Best_Len,
                     Best_Dist);
                  Rep0 := Best_Dist;
                  State := LZMA_Match_State_After (State);
                  Prev := Plain (In_Index + Best_Len - 1);
                  Position := Position + Best_Len;
                  In_Index := In_Index + Best_Len;
               end if;
            else
               declare
               ...
               begin
                  ...
                  if State >= 7
                    and then Rep0 > 0
                    and then Rep0 <= Position
                  then
                     --  Matched literal (standard LZMA): code each bit against
                     --  the byte Rep0 back using the match-conditioned model
                     --  until they diverge, then fall back to the plain model.
                     declare
                        Match_Byte : Natural :=
                          Natural (Plain (In_Index - Rep0));
   ...
   is
      Empty : constant Byte_Array (1 .. 0) := [others => 0];
      PPMd_Top_Value : constant Interfaces.Unsigned_32 := 2 ** 24;
      PPMd_Num_Indexes : constant := 38;
      PPMd_Unit_Size   : constant := 12;
      PPMd_Max_Freq    : constant := 124;
      PPMd_Max_Context_Stats : constant := 256;
      PPMd_Bin_Scale   : constant := 2 ** 14;
      PPMd_Period_Bits : constant := 7;
      PPMd_Int_Bits    : constant := 7;
      PPMd_Init_Bin_Esc : constant array (0 .. 7) of Interfaces.Unsigned_16 :=
        [16#3CDD#, 16#1F3F#, 16#59BF#, 16#48F3#,
         16#64A1#, 16#5ABC#, 16#6632#, 16#6051#];
      PPMd_Exp_Escape : constant array (0 .. 15) of Byte :=
        [25, 14, 9, 7, 5, 5, 4, 4, 4, 3, 3, 3, 2, 2, 2, 2];

      type PPMd_Range_Decoder is record
         Code  : Interfaces.Unsigned_32 := 0;
         Span  : Interfaces.Unsigned_32 := Interfaces.Unsigned_32'Last;
         Pos   : Natural := 0;
      end record;

      type PPMd_Byte_Table is array (Natural range <>) of Byte;
      type PPMd_Symbol_Mask is array (Natural range 0 .. 255) of Boolean;
      type PPMd_U16_Table is
        array (Natural range <>, Natural range <>) of Interfaces.Unsigned_16;

      type PPMd_See_State is record
         Summ  : Interfaces.Unsigned_16 := 0;
         Shift : Byte := 0;
         Count : Byte := 0;
      end record;

      type PPMd_See_Table is
        array (Natural range <>, Natural range <>) of PPMd_See_State;

      type PPMd_State is record
         Symbol    : Byte := 0;
         Freq      : Byte := 0;
         Successor : Natural := 0;
      end record;

      type PPMd_State_Table is array (Natural range <>) of PPMd_State;
      type PPMd_Context_State_Block is array (Natural range <>) of PPMd_State;
      type PPMd_Context_State_Block_Access is access PPMd_Context_State_Block;

      procedure Free_PPMd_Context_State_Block is new Ada.Unchecked_Deallocation
        (PPMd_Context_State_Block, PPMd_Context_State_Block_Access);

      type PPMd_Context is record
         ...
         Summ_Freq  : Natural := 0;
         Suffix     : Natural := 0;
         State_Position : Natural := 0;
         State_Units : Natural := 0;
         State_Capacity : Natural := 0;
         One_State  : PPMd_State := (0, 0, 0);
         States     : PPMd_Context_State_Block_Access := null;
      end record;

      type PPMd_Context_Table is array (Natural range <>) of PPMd_Context;
      type PPMd_Context_Table_Access is access PPMd_Context_Table;

      procedure Free_PPMd_Context_Table is new Ada.Unchecked_Deallocation
        (PPMd_Context_Table, PPMd_Context_Table_Access);

      type PPMd_Model_State is record
         Max_Order                 : Natural := 0;
         Order_Fall                : Natural := 0;
         Init_RL                   : Integer := 0;
         Run_Length                : Integer := 0;
         Prev_Success              : Natural := 0;
         Glue_Count                : Interfaces.Unsigned_32 := 0;
         Text_Pos                  : Natural := 0;
         Root_Context_Pos          : Natural := 0;
         Hi_Unit                   : Natural := 0;
         Lo_Unit                   : Natural := 0;
         Units_Start               : Natural := 0;
         Max_Context_Pos           : Natural := 0;
         Found_State_Pos           : Natural := 0;
         Found_State_Index         : Natural := 0;
         Found_State_Successor     : Natural := 0;
         Found_State_Symbol        : Byte := 0;
         Current_Context_Index     : Natural := 0;
         Hi_Bits_Flag              : Natural := 0;
         Init_Esc                  : Natural := 0;
         Restarted                 : Boolean := False;
         Memory_Size               : Natural := 0;
         Units_Area                : Natural := 0;
         Free_Context_Head         : Natural := 0;
         Initial_Context_Num_Stats : Natural := 0;
         Initial_Context_Summ_Freq : Natural := 0;
         Initial_State_Freq_Sum    : Natural := 0;
         Initial_States            : PPMd_State_Table (0 .. 255) :=
                                       [others => (0, 0, 0)];
         Context_Count             : Natural := 0;
         Contexts                  : PPMd_Context_Table_Access := null;
         Indx2Units                : PPMd_Byte_Table
                                      (0 .. PPMd_Num_Indexes + 1) :=
                                        [others => 0];
         Units2Indx                : PPMd_Byte_Table (1 .. 128) :=
                                        [others => 0];
         NS2BSIndx                 : PPMd_Byte_Table (0 .. 255) :=
                                        [others => 0];
         NS2Indx                   : PPMd_Byte_Table (0 .. 255) :=
                                        [others => 0];
         ExpEscape                 : PPMd_Byte_Table (0 .. 15) :=
                                        [others => 0];
         BinSumm                   : PPMd_U16_Table (0 .. 127, 0 .. 63) :=
                                        [others => [others => 0]];
         See                       : PPMd_See_Table (0 .. 24, 0 .. 15) :=
                                        [others => [others => (0, 0, 0)]];
         Dummy_See                 : PPMd_See_State := (0, 0, 0);
      end record;

      procedure PPMd_Free_Model (Model : in out PPMd_Model_State) is
      begin
         if Model.Contexts /= null then
            for I in Model.Contexts.all'Range loop
               if Model.Contexts (I).States /= null then
                  Free_PPMd_Context_State_Block (Model.Contexts (I).States);
               end if;
            end loop;
            Free_PPMd_Context_Table (Model.Contexts);
         end if;
      end PPMd_Free_Model;

      type PPMd_Model_Guard
        (Model : access PPMd_Model_State)
      is new Ada.Finalization.Limited_Controlled with null record;

      overriding procedure Finalize (Guard : in out PPMd_Model_Guard) is
      begin
         if Guard.Model /= null then
            PPMd_Free_Model (Guard.Model.all);
         end if;
      end Finalize;

      procedure PPMd_Model_Construct (Model : in out PPMd_Model_State) is
         K : Natural := 1;
         M : Natural;
      begin
         for I in 0 .. PPMd_Num_Indexes - 1 loop
            declare
               Step : constant Natural :=
                 (if I >= 12 then 4 else I / 4 + 1);
            begin
               for Count in 1 .. Step loop
                  Model.Units2Indx (K) := Byte (I);
                  K := K + 1;
               end loop;
               Model.Indx2Units (I) := Byte (K - 1);
            end;
         end loop;
         ...
         for I in 11 .. 255 loop
            Model.NS2BSIndx (I) := 6;
         end loop;

         for I in 0 .. 2 loop
            Model.NS2Indx (I) := Byte (I);
         end loop;

         M := 3;
         K := 1;
         for I in 3 .. 255 loop
            Model.NS2Indx (I) := Byte (M);
            K := K - 1;
            if K = 0 then
               M := M + 1;
               K := M - 2;
         ...
         for I in PPMd_Exp_Escape'Range loop
            Model.ExpEscape (I) := PPMd_Exp_Escape (I);
         end loop;
      end PPMd_Model_Construct;

      function PPMd_Model_Init
        (Model : in out PPMd_Model_State;
         Order : Natural;
         Memory : Interfaces.Unsigned_32;
         Output_Size : Natural) return Boolean
      is
         Memory_Size : Natural;
         Units_Area  : Natural;
         Context_Capacity : Natural;
      begin
         if Interfaces.Unsigned_64 (Memory)
           > Interfaces.Unsigned_64 (Natural'Last)
         then
            return False;
         end if;

         Memory_Size := Natural (Memory);
         Units_Area :=
           (Memory_Size / 8 / PPMd_Unit_Size) * 7 * PPMd_Unit_Size;
         if Memory_Size < PPMd_Unit_Size
           or else Units_Area < (256 / 2) * PPMd_Unit_Size
         then
            return False;
         end if;

         PPMd_Free_Model (Model);
         Model := (others => <>);
         PPMd_Model_Construct (Model);

         if Order > 0
           and then Output_Size > (Natural'Last - 256) / (Order + 1)
         then
            Context_Capacity := Memory_Size / PPMd_Unit_Size;
         else
            Context_Capacity :=
              Natural'Min
                (Memory_Size / PPMd_Unit_Size,
                 Natural'Max (1, Output_Size * (Order + 1) + 256));
         end if;

         if Context_Capacity = 0 then
            return False;
         end if;

         Model.Contexts :=
           new PPMd_Context_Table'
             (1 .. Context_Capacity => (others => <>));

         Model.Max_Order := Order;
         Model.Memory_Size := Memory_Size;
         Model.Units_Area := Units_Area;
         Model.Init_RL := -Integer (Natural'Min (Order, 12)) - 1;
         Model.Context_Count := 0;
         Model.Free_Context_Head := 0;

         for I in 0 .. 127 loop
            for K in 0 .. 7 loop
               declare
                  Val : constant Interfaces.Unsigned_16 :=
                    Interfaces.Unsigned_16
                      (PPMd_Bin_Scale
                       - Natural (PPMd_Init_Bin_Esc (K)) / (I + 2));
               begin
                  for M in 0 .. 7 loop
                     Model.BinSumm (I, K + 8 * M) := Val;
                  end loop;
               end;
            end loop;
         end loop;
         ...
         for I in 0 .. 24 loop
            declare
               Summ : constant Interfaces.Unsigned_16 :=
                 Interfaces.Unsigned_16
                   ((5 * I + 10) * 2 ** (PPMd_Period_Bits - 4));
            begin
               for K in 0 .. 15 loop
                  Model.See (I, K) :=
                    (Summ  => Summ,
                     Shift => Byte (PPMd_Period_Bits - 4),
                     Count => 4);
               end loop;
            end;
         end loop;
         ...
         Model.Dummy_See :=
           (Summ => 0, Shift => PPMd_Period_Bits, Count => 64);

         Model.Order_Fall := Order;
         Model.Run_Length := Model.Init_RL;
         Model.Prev_Success := 0;
         Model.Glue_Count := 0;
         Model.Text_Pos := 0;
         Model.Hi_Unit := Memory_Size - PPMd_Unit_Size;
         Model.Root_Context_Pos := Model.Hi_Unit;
         Model.Units_Start := Memory_Size - Units_Area;
         Model.Lo_Unit :=
           Model.Units_Start + (256 / 2) * PPMd_Unit_Size;
         Model.Max_Context_Pos := Model.Hi_Unit;
         Model.Found_State_Pos := Model.Units_Start;
         Model.Found_State_Index := 0;
         Model.Found_State_Successor := 0;
         Model.Found_State_Symbol := 0;
         Model.Current_Context_Index := 0;
         Model.Hi_Bits_Flag := 0;
         Model.Init_Esc := 0;
         Model.Restarted := False;
         Model.Initial_Context_Num_Stats := 256;
         Model.Initial_Context_Summ_Freq := 257;
         Model.Initial_State_Freq_Sum := 256;

         for I in Model.Initial_States'Range loop
            Model.Initial_States (I) :=
              (Symbol => Byte (I), Freq => 1, Successor => 0);
         end loop;

         return True;
      end PPMd_Model_Init;

      function PPMd_Range_Init
        (Input : Byte_Array;
         D     : out PPMd_Range_Decoder) return Boolean
      is
      begin
         D := (Code => 0, Span => Interfaces.Unsigned_32'Last, Pos => 0);
         if Input'Length < 5 or else Input (Input'First) /= 0 then
            return False;
         end if;

         for Offset in 1 .. 4 loop
            D.Code :=
              Interfaces.Shift_Left (D.Code, 8)
              or Interfaces.Unsigned_32 (Input (Input'First + Offset));
         end loop;
         D.Pos := Input'First + 5;

         return D.Code < D.Span;
      end PPMd_Range_Init;

      function PPMd_Range_Normalize
        (Input : Byte_Array;
         D     : in out PPMd_Range_Decoder) return Boolean
      is
      begin
         while D.Span < PPMd_Top_Value loop
            if D.Pos > Input'Last then
               return False;
            end if;

            D.Code :=
              Interfaces.Shift_Left (D.Code, 8)
              or Interfaces.Unsigned_32 (Input (D.Pos));
            D.Span := Interfaces.Shift_Left (D.Span, 8);
            D.Pos := D.Pos + 1;
         end loop;

         return True;
      end PPMd_Range_Normalize;

      function PPMd_Range_Normalize_One
        (Input : Byte_Array;
         D     : in out PPMd_Range_Decoder) return Boolean
      is
      begin
         if D.Span < PPMd_Top_Value then
            if D.Pos > Input'Last then
               return False;
            end if;
         ...
         return True;
      end PPMd_Range_Normalize_One;

      function PPMd_Range_Normalize_Remote
        (Input : Byte_Array;
         D     : in out PPMd_Range_Decoder) return Boolean
      is
      begin
         return PPMd_Range_Normalize_One (Input, D)
           and then PPMd_Range_Normalize_One (Input, D);
      end PPMd_Range_Normalize_Remote;

      function PPMd_Range_Get_Threshold
        (D         : in out PPMd_Range_Decoder;
         Total     : Interfaces.Unsigned_32;
         Threshold : out Interfaces.Unsigned_32) return Boolean
      is
      begin
         Threshold := 0;
         if Total = 0 or else D.Span < Total then
            return False;
         end if;
         ...
         Threshold := D.Code / D.Span;
         return Threshold < Total;
      end PPMd_Range_Get_Threshold;

      function PPMd_Range_Decode_Raw
        (Input : Byte_Array;
         D     : in out PPMd_Range_Decoder;
         Start : Interfaces.Unsigned_32;
         Size  : Interfaces.Unsigned_32) return Boolean
      is
      ...
      begin
         ...
         if Size = 0
           or else Start > Interfaces.Unsigned_32'Last - Size
         then
            return False;
         end if;

         Product :=
           Interfaces.Unsigned_64 (Start) * Interfaces.Unsigned_64 (D.Span);
         if Product > Interfaces.Unsigned_64 (D.Code) then
            return False;
         end if;

         D.Code := D.Code - Interfaces.Unsigned_32 (Product);
         Product :=
           Interfaces.Unsigned_64 (Size) * Interfaces.Unsigned_64 (D.Span);
         if Product > Interfaces.Unsigned_64 (Interfaces.Unsigned_32'Last) then
            return False;
         end if;

         D.Span := Interfaces.Unsigned_32 (Product);
         return True;
      end PPMd_Range_Decode_Raw;

      function PPMd_Range_Decode_Final
        (Input : Byte_Array;
         D     : in out PPMd_Range_Decoder;
         Start : Interfaces.Unsigned_32;
         Size  : Interfaces.Unsigned_32) return Boolean
      is
      begin
         return PPMd_Range_Decode_Raw (Input, D, Start, Size)
           and then PPMd_Range_Normalize_Remote (Input, D);
      end PPMd_Range_Decode_Final;

      function PPMd_Decode_Initial_Symbol
        (Input  : Byte_Array;
         D      : in out PPMd_Range_Decoder;
         Model  : in out PPMd_Model_State;
         Symbol : out Byte) return Boolean
      is
         Threshold : Interfaces.Unsigned_32;
         Count     : Interfaces.Unsigned_32;
         Start     : Interfaces.Unsigned_32 := 0;
      begin
         Symbol := 0;
         if Model.Initial_Context_Num_Stats /= 256
           or else (if General_Root
                    then Model.Initial_Context_Summ_Freq = 0
                    else Model.Initial_Context_Summ_Freq /= 257)
         then
            return False;
         end if;

         if not PPMd_Range_Get_Threshold
           (D, Interfaces.Unsigned_32 (Model.Initial_Context_Summ_Freq),
            Threshold)
         then
            return False;
         end if;
         ...
         for I in Model.Initial_States'Range loop
            declare
               Frequency : constant Interfaces.Unsigned_32 :=
                 Interfaces.Unsigned_32 (Model.Initial_States (I).Freq);
            begin
               if Count < Frequency then
                  if not PPMd_Range_Decode_Final
                    (Input, D, Start, Frequency)
                  then
                     return False;
                  end if;

                  Model.Found_State_Index := I;
                  Model.Found_State_Pos :=
                    Model.Units_Start + I * 6;
                  Model.Found_State_Successor :=
                    Model.Initial_States (I).Successor;
                  Model.Prev_Success := 0;
                  Symbol := Model.Initial_States (I).Symbol;
                  Model.Found_State_Symbol := Symbol;
                  return True;
               end if;
         ...
         if Threshold >= Interfaces.Unsigned_32
           (Model.Initial_Context_Summ_Freq)
         then
            return False;
         end if;

         if General_Root then
            return False;
         end if;

         return PPMd_Range_Decode_Raw
           (Input, D, Start,
            Interfaces.Unsigned_32 (Model.Initial_Context_Summ_Freq)
            - Start);
      end PPMd_Decode_Initial_Symbol;

      procedure PPMd_Rescale_Initial_Root (Model : in out PPMd_Model_State) is
         Sum : Natural := 0;
      begin
         if Use_State_Blocks and then General_Root then
            declare
               Found_Index : Natural := Model.Initial_States'First;
               Temp        : PPMd_State;
               Adder       : constant Natural :=
                 (if Model.Order_Fall = 0 then 0 else 1);
               Escape_Freq : Natural;
               Old_Found_Freq : Natural;
            begin
               for Candidate in Model.Initial_States'Range loop
                  if Model.Initial_States (Candidate).Symbol =
                    Model.Found_State_Symbol
                  then
               ...
               Temp := Model.Initial_States (Found_Index);
               while Found_Index > Model.Initial_States'First loop
                  Model.Initial_States (Found_Index) :=
                    Model.Initial_States (Found_Index - 1);
                  Found_Index := Found_Index - 1;
               end loop;
               Model.Initial_States (Model.Initial_States'First) := Temp;

               Old_Found_Freq :=
                 Natural
                   (Model.Initial_States (Model.Initial_States'First).Freq);
               Escape_Freq := Model.Initial_Context_Summ_Freq - Old_Found_Freq;
               Sum :=
                 (Old_Found_Freq + 4 + Adder) / 2;
               Model.Initial_States (Model.Initial_States'First).Freq :=
                 Byte (Natural'Max (1, Sum));

               for I in Model.Initial_States'First + 1
                 .. Model.Initial_States'Last
               loop
                  declare
                     Old_Freq : constant Natural :=
                       Natural (Model.Initial_States (I).Freq);
                     New_Freq : Natural;
                     Insert_Index : Natural := I;
                     Current_State : PPMd_State := Model.Initial_States (I);
                  begin
                     if Old_Freq <= Escape_Freq then
                        Escape_Freq := Escape_Freq - Old_Freq;
                     else
                        Escape_Freq := 0;
                     end if;

                     New_Freq := Natural'Max (1, (Old_Freq + Adder) / 2);
                     Current_State.Freq := Byte (New_Freq);
                     Sum := Sum + New_Freq;

                     while Insert_Index > Model.Initial_States'First
                       and then New_Freq >
                         Natural (Model.Initial_States (Insert_Index - 1).Freq)
                     loop
                        Model.Initial_States (Insert_Index) :=
                          Model.Initial_States (Insert_Index - 1);
                        Insert_Index := Insert_Index - 1;
                     end loop;
                     Model.Initial_States (Insert_Index) := Current_State;
                  end;
               end loop;

               Model.Initial_State_Freq_Sum := Sum;
               Model.Initial_Context_Summ_Freq :=
                 Sum + Escape_Freq - Escape_Freq / 2;
               Model.Found_State_Index := Model.Initial_States'First;
               Model.Found_State_Pos := Model.Units_Start;
               Model.Found_State_Successor :=
                 Model.Initial_States (Model.Initial_States'First).Successor;
               Model.Found_State_Symbol :=
                 Model.Initial_States (Model.Initial_States'First).Symbol;
               return;
            end;
         end if;
         ...
         for I in Model.Initial_States'Range loop
            declare
               Freq : constant Natural :=
                 (Natural (Model.Initial_States (I).Freq) + 1) / 2;
            begin
               Model.Initial_States (I).Freq := Byte (Natural'Max (1, Freq));
               Sum := Sum + Natural (Model.Initial_States (I).Freq);
            end;
         end loop;

         Model.Initial_State_Freq_Sum := Sum;
         Model.Initial_Context_Summ_Freq := Sum + 1;
      end PPMd_Rescale_Initial_Root;

      function PPMd_Update_Initial_Symbol
        (Model : in out PPMd_Model_State) return Boolean
      is
         Index    : constant Natural := Model.Found_State_Index;
         Freq     : Natural;
         SummFreq : constant Natural := Model.Initial_Context_Summ_Freq;
         Rescaled : Boolean := False;
      begin
         if Index not in Model.Initial_States'Range
           or else Model.Initial_States (Index).Freq = 0
           or else SummFreq = 0
           or else SummFreq > Natural (Interfaces.Unsigned_16'Last) - 4
         then
            return False;
         end if;

         Freq := Natural (Model.Initial_States (Index).Freq);
         if not General_Root then
            Model.Prev_Success := (if 2 * Freq > SummFreq then 1 else 0);
            Model.Run_Length := Model.Run_Length + Model.Prev_Success;
         elsif Index = Model.Initial_States'First then
            Model.Prev_Success := (if 2 * Freq > SummFreq then 1 else 0);
            Model.Run_Length := Model.Run_Length + Model.Prev_Success;
         else
            Model.Prev_Success := 0;
         ...
         Model.Initial_Context_Summ_Freq := SummFreq + 4;

         if Freq > Natural (Byte'Last) - 4 then
            return False;
         end if;

         Freq := Freq + 4;
         Model.Initial_States (Index).Freq := Byte (Freq);
         Model.Initial_State_Freq_Sum := Model.Initial_State_Freq_Sum + 4;

         if Freq > PPMd_Max_Freq then
            if not General_Root then
               return False;
            end if;

            PPMd_Rescale_Initial_Root (Model);
            if Model.Found_State_Index not in Model.Initial_States'Range then
               return False;
            end if;

            Rescaled := True;
            Freq :=
              Natural (Model.Initial_States (Model.Found_State_Index).Freq);
            if Freq > PPMd_Max_Freq then
               return False;
            end if;
         end if;
         ...
         if General_Root
           and then not Rescaled
           and then Index > Model.Initial_States'First
           and then Model.Initial_States (Index).Freq
             > Model.Initial_States (Index - 1).Freq
         then
            declare
               Temp : constant PPMd_State := Model.Initial_States (Index - 1);
            begin
               Model.Initial_States (Index - 1) := Model.Initial_States (Index);
               Model.Initial_States (Index) := Temp;
               Model.Found_State_Index := Index - 1;
               Model.Found_State_Pos := Model.Units_Start + (Index - 1) * 6;
               Model.Found_State_Successor :=
                 Model.Initial_States (Index - 1).Successor;
               Model.Found_State_Symbol :=
                 Model.Initial_States (Index - 1).Symbol;
            end;
         end if;

         return True;
      end PPMd_Update_Initial_Symbol;

      function PPMd_Update_Initial_Symbol_After_Escape
        (Model : in out PPMd_Model_State) return Boolean
      is
         Index : constant Natural := Model.Found_State_Index;
         Freq  : Natural;
      begin
         if Index not in Model.Initial_States'Range
           or else Model.Initial_States (Index).Freq = 0
           or else Model.Initial_Context_Summ_Freq
             > Natural (Interfaces.Unsigned_16'Last) - 4
         then
            return False;
         end if;

         Freq := Natural (Model.Initial_States (Index).Freq) + 4;
         if Use_State_Blocks then
            if Freq > Natural (Byte'Last) then
               return False;
            end if;
            ...
            Model.Initial_States (Index).Freq := Byte (Freq);
            Model.Initial_Context_Summ_Freq :=
              Model.Initial_Context_Summ_Freq + 4;
            Model.Initial_State_Freq_Sum := Model.Initial_State_Freq_Sum + 4;
         ...
         if Freq > PPMd_Max_Freq then
            if not Remaining_Symbols_From_Root then
               return False;
            end if;
         ...
         Model.Initial_State_Freq_Sum := Model.Initial_State_Freq_Sum + 4;
         return True;
      end PPMd_Update_Initial_Symbol_After_Escape;

      function PPMd_Context_Has_Symbol
        (Context : PPMd_Context;
         Symbol  : Byte) return Boolean
      is
      begin
         if Context.Num_Stats = 1 then
            return Context.One_State.Symbol = Symbol;
         end if;

         if Context.Num_Stats = 0
           or else Context.States = null
           or else Context.Num_Stats > Context.State_Capacity
           or else Context.Num_Stats > Context.States'Length
         then
            return False;
         end if;

         for I in 0 .. Context.Num_Stats - 1 loop
            if Context.States (I).Symbol = Symbol then
               return True;
            end if;
         end loop;

         return False;
      end PPMd_Context_Has_Symbol;

      function PPMd_Has_State_Block (Context : PPMd_Context) return Boolean is
      begin
         return Context.Num_Stats > 1
           and then Context.States /= null
           and then Context.Num_Stats <= Context.State_Capacity
           and then Context.Num_Stats <= Context.States'Length;
      end PPMd_Has_State_Block;

      function PPMd_Record_Decoded_Symbol
        (Model   : in out PPMd_Model_State;
         Symbol  : Byte;
         Output  : in out Byte_Array;
         Decoded : in out Natural) return Boolean
      is
      begin
         if Decoded >= Output'Length
           or else Model.Text_Pos >= Model.Units_Start
         then
            return False;
         end if;

         Output (Output'First + Decoded) := Symbol;
         Decoded := Decoded + 1;
         Model.Text_Pos := Model.Text_Pos + 1;
         return True;
      end PPMd_Record_Decoded_Symbol;

      function PPMd_Find_Context_Index
        (Model    : PPMd_Model_State;
         Position : Natural) return Natural;

      procedure PPMd_Release_State_Block
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural);

      procedure PPMd_Restart_Model
        (Model             : in out PPMd_Model_State;
         Preserved_Text_Pos : Natural)
      is
      begin
         for I in Model.Contexts.all'Range loop
            PPMd_Release_State_Block (Model, I);
            Model.Contexts (I) := (others => <>);
         end loop;

         Model.Order_Fall := Model.Max_Order;
         Model.Run_Length := Model.Init_RL;
         Model.Prev_Success := 0;
         Model.Glue_Count := 0;
         Model.Text_Pos := Preserved_Text_Pos;
         Model.Hi_Unit := Model.Memory_Size - PPMd_Unit_Size;
         Model.Root_Context_Pos := Model.Hi_Unit;
         Model.Units_Start := Model.Memory_Size - Model.Units_Area;
         Model.Lo_Unit :=
           Model.Units_Start + (256 / 2) * PPMd_Unit_Size;
         Model.Max_Context_Pos := Model.Root_Context_Pos;
         Model.Found_State_Pos := Model.Units_Start;
         Model.Found_State_Index := 0;
         Model.Found_State_Successor := 0;
         Model.Found_State_Symbol := 0;
      ...
      end PPMd_Restart_Model;

      procedure PPMd_Free_Context
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural)
      is
      begin
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
         then
            return;
         end if;

         PPMd_Release_State_Block (Model, Context_Index);
         Model.Contexts (Context_Index).Allocated := False;
         Model.Contexts (Context_Index).Next_Free := Model.Free_Context_Head;
         Model.Contexts (Context_Index).Num_Stats := 0;
         Model.Contexts (Context_Index).Summ_Freq := 0;
         Model.Contexts (Context_Index).Suffix := 0;
         Model.Contexts (Context_Index).One_State := (0, 0, 0);
         Model.Free_Context_Head := Context_Index;
      end PPMd_Free_Context;

      procedure PPMd_Glue_Free_Contexts (Model : in out PPMd_Model_State)
      is
         type Reachable_Table is array (Model.Contexts.all'Range) of Boolean;
         Reachable : Reachable_Table := [others => False];

         procedure Mark_Position (Position : Natural);

         procedure Remap_Found_State
           (Context_Index : Natural;
            Old_Position  : Natural;
            New_Position  : Natural)
         is
         begin
            if Old_Position = 0
              or else Old_Position = New_Position
              or else Context_Index not in Model.Contexts.all'Range
              or else not Model.Contexts (Context_Index).Allocated
              or else Model.Contexts (Context_Index).Num_Stats <= 1
            then
               return;
            end if;

            if Model.Found_State_Pos >= Old_Position
              and then Model.Found_State_Pos <
                Old_Position + Model.Contexts (Context_Index).Num_Stats * 6
            then
               Model.Found_State_Pos :=
                 New_Position + (Model.Found_State_Pos - Old_Position);
            end if;
         end Remap_Found_State;

         procedure Mark_Context (Context_Index : Natural) is
         begin
            if Context_Index not in Model.Contexts.all'Range
              or else not Model.Contexts (Context_Index).Allocated
              or else Reachable (Context_Index)
            then
               return;
            end if;

            Reachable (Context_Index) := True;
            Mark_Position (Model.Contexts (Context_Index).Suffix);
            if Model.Contexts (Context_Index).Num_Stats = 1 then
               Mark_Position
                 (Model.Contexts (Context_Index).One_State.Successor);
            elsif PPMd_Has_State_Block (Model.Contexts (Context_Index)) then
               for I in 0 .. Model.Contexts (Context_Index).Num_Stats - 1 loop
                  Mark_Position
                    (Model.Contexts (Context_Index).States (I).Successor);
               end loop;
            end if;
         end Mark_Context;

         procedure Mark_Position (Position : Natural) is
            Context_Index : Natural;
         begin
            if Position <= Model.Units_Start then
               return;
            end if;

            Context_Index := PPMd_Find_Context_Index (Model, Position);
            if Context_Index in Model.Contexts.all'Range then
               Mark_Context (Context_Index);
            end if;
         end Mark_Position;
      begin
         Mark_Position (Model.Max_Context_Pos);
         if Model.Current_Context_Index in Model.Contexts.all'Range then
            Mark_Context (Model.Current_Context_Index);
         end if;

         for I in Model.Initial_States'Range loop
            Mark_Position (Model.Initial_States (I).Successor);
         end loop;

         for I in Model.Contexts.all'Range loop
            if Model.Contexts (I).Allocated and then not Reachable (I) then
               PPMd_Free_Context (Model, I);
            end if;
         end loop;
         ...
         if Use_State_Blocks then
            ...
            begin
               for I in Model.Contexts.all'Range loop
                  if Model.Contexts (I).Allocated
                    and then Model.Contexts (I).State_Position /= 0
                    and then Model.Contexts (I).State_Units > 0
                  then
                     declare
                        Block_Bytes : Natural;
                     begin
                        if Model.Contexts (I).State_Units >
                          Natural'Last / PPMd_Unit_Size
                        then
                           return;
                        end if;

                        Block_Bytes :=
                          Model.Contexts (I).State_Units * PPMd_Unit_Size;
                        if New_Lo_Unit > Model.Hi_Unit
                          or else Block_Bytes > Model.Hi_Unit - New_Lo_Unit
                        then
                           return;
                        end if;

                        Remap_Found_State
                          (I, Model.Contexts (I).State_Position, New_Lo_Unit);
                        Model.Contexts (I).State_Position := New_Lo_Unit;
                        New_Lo_Unit := New_Lo_Unit + Block_Bytes;
                     end;
                  end if;
               end loop;

               Model.Lo_Unit := New_Lo_Unit;
            end;
         end if;

         Model.Glue_Count := Model.Glue_Count + 1;
      end PPMd_Glue_Free_Contexts;

      function PPMd_Allocate_Context
        (Model         : in out PPMd_Model_State;
         Context_Index : out Natural;
         Context_Pos   : out Natural) return Boolean
      is
      begin
         Context_Index := 0;
         Context_Pos := 0;

         if Model.Free_Context_Head in Model.Contexts.all'Range then
            Context_Index := Model.Free_Context_Head;
            Model.Free_Context_Head :=
              Model.Contexts (Context_Index).Next_Free;
            Context_Pos := Model.Contexts (Context_Index).Position;
            if Context_Pos = 0 then
               return False;
            end if;
            return True;
         end if;

         if Model.Context_Count < Model.Contexts.all'Length
           and then Model.Hi_Unit >= PPMd_Unit_Size
           and then Model.Hi_Unit - PPMd_Unit_Size >= Model.Lo_Unit
         then
            Model.Hi_Unit := Model.Hi_Unit - PPMd_Unit_Size;
            Context_Pos := Model.Hi_Unit;
            Model.Context_Count := Model.Context_Count + 1;
            Context_Index := Model.Context_Count;
            return True;
         end if;

         PPMd_Glue_Free_Contexts (Model);
         if Model.Free_Context_Head in Model.Contexts.all'Range then
            Context_Index := Model.Free_Context_Head;
            Model.Free_Context_Head :=
              Model.Contexts (Context_Index).Next_Free;
         ...
         PPMd_Restart_Model (Model, Model.Text_Pos);
         return False;
      end PPMd_Allocate_Context;

      function PPMd_State_Units_For_Count
        (Model : PPMd_Model_State;
         Count : Natural) return Natural
      is
         Required_Units : Natural;
         Index          : Natural;
      begin
         if Count < 2 or else Count > PPMd_Max_Context_Stats then
            return 0;
         end if;

         Required_Units := (Count + 1) / 2;
         if Required_Units not in Model.Units2Indx'Range then
            return 0;
         end if;

         Index := Natural (Model.Units2Indx (Required_Units));
         if Index > Model.Indx2Units'Last then
            return 0;
         end if;

         return Natural (Model.Indx2Units (Index));
      end PPMd_State_Units_For_Count;

      procedure PPMd_Remap_State_Block_Position
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural;
         Old_Position  : Natural;
         New_Position  : Natural)
      ...
      end PPMd_Remap_State_Block_Position;

      function PPMd_Ensure_State_Capacity
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural;
      ...
      is
         ...
         New_Position : Natural := 0;
         Old_Position : Natural := 0;
      begin
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
           or else Count < 2
           or else Count > PPMd_Max_Context_Stats
         then
            return False;
         end if;

         if Model.Contexts (Context_Index).State_Capacity >= Count then
            return True;
         end if;

         New_Units := PPMd_State_Units_For_Count (Model, Count);
         if New_Units = 0 then
            return False;
         end if;

         if New_Units > Model.Contexts (Context_Index).State_Units then
            if Use_State_Blocks then
               declare
                  Old_Units   : constant Natural :=
                    Model.Contexts (Context_Index).State_Units;
                  Extra_Units : Natural := New_Units;
                  Old_Bytes   : Natural := 0;
                  Extra_Bytes : Natural;
               begin
                  Old_Position :=
                    Model.Contexts (Context_Index).State_Position;
                  if Old_Position /= 0
                    and then Old_Units > 0
                    and then Old_Units <= Natural'Last / PPMd_Unit_Size
                  then
                     Old_Bytes := Old_Units * PPMd_Unit_Size;
                     if Old_Position <= Natural'Last - Old_Bytes
                       and then Old_Position + Old_Bytes = Model.Lo_Unit
                     then
                        Extra_Units := New_Units - Old_Units;
                        New_Position := Old_Position;
                     end if;
                  end if;

                  if Extra_Units > Natural'Last / PPMd_Unit_Size
                  then
                     return False;
                  end if;

                  Extra_Bytes := Extra_Units * PPMd_Unit_Size;
                  if Model.Lo_Unit > Model.Hi_Unit
                    or else Extra_Bytes > Model.Hi_Unit - Model.Lo_Unit
                  then
                     PPMd_Glue_Free_Contexts (Model);
                     if Model.Lo_Unit > Model.Hi_Unit
                       or else Extra_Bytes > Model.Hi_Unit - Model.Lo_Unit
                  ...
                  if New_Position = 0 then
                     New_Position := Model.Lo_Unit;
                  end if;
                  Model.Lo_Unit := Model.Lo_Unit + Extra_Bytes;
               end;
            end if;
         end if;

         New_Capacity :=
           Natural'Min (PPMd_Max_Context_Stats, New_Units * 2);
         if not Use_State_Blocks then
            New_Position := Model.Contexts (Context_Index).Position;
         elsif New_Position = 0 then
            New_Position := Model.Contexts (Context_Index).State_Position;
         end if;

         if Model.Contexts (Context_Index).States = null
           or else Model.Contexts (Context_Index).States'Length < New_Capacity
         then
            declare
               Old_States : PPMd_Context_State_Block_Access :=
                 Model.Contexts (Context_Index).States;
               New_States : constant PPMd_Context_State_Block_Access :=
                 new PPMd_Context_State_Block'
                   (0 .. New_Capacity - 1 => (0, 0, 0));
               Copy_Count : constant Natural :=
                 (if Old_States = null then 0
                  else Natural'Min
                    (Natural'Min (Old_States'Length, New_Capacity),
                     Model.Contexts (Context_Index).Num_Stats));
            begin
               if Copy_Count > 0 then
                  for I in 0 .. Copy_Count - 1 loop
                     New_States (I) := Old_States (I);
                  end loop;
               end if;

               Model.Contexts (Context_Index).States := New_States;
               if Old_States /= null then
                  Free_PPMd_Context_State_Block (Old_States);
               end if;
            end;
         end if;

         PPMd_Remap_State_Block_Position
           (Model, Context_Index, Old_Position, New_Position);
         Model.Contexts (Context_Index).State_Position := New_Position;
         Model.Contexts (Context_Index).State_Units := New_Units;
         Model.Contexts (Context_Index).State_Capacity := New_Capacity;
         return Model.Contexts (Context_Index).State_Capacity >= Count;
      end PPMd_Ensure_State_Capacity;
      ...
      is
         ...
         Old_Bytes    : Natural;
      begin
         if Context_Index not in Model.Contexts.all'Range then
            return;
         end if;

         Old_Position := Model.Contexts (Context_Index).State_Position;
         Old_Units := Model.Contexts (Context_Index).State_Units;
         if Use_State_Blocks
           and then Old_Position /= 0
           and then Old_Units > 0
           and then Old_Units <= Natural'Last / PPMd_Unit_Size
         then
            Old_Bytes := Old_Units * PPMd_Unit_Size;
            if Old_Position <= Natural'Last - Old_Bytes
              and then Old_Position + Old_Bytes = Model.Lo_Unit
            then
               Model.Lo_Unit := Old_Position;
            end if;
         end if;

         Model.Contexts (Context_Index).State_Units := 0;
         Model.Contexts (Context_Index).State_Capacity := 0;
         Model.Contexts (Context_Index).State_Position := 0;
         if Model.Contexts (Context_Index).States /= null then
            Free_PPMd_Context_State_Block
              (Model.Contexts (Context_Index).States);
         end if;
      end PPMd_Release_State_Block;

      function PPMd_Shrink_State_Capacity
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural;
         Count         : Natural) return Boolean
      is
      ...
      begin
         ...
         if not Use_State_Blocks then
            if Model.Contexts (Context_Index).States = null
              or else Model.Contexts (Context_Index).States'Length < Count
            then
               return PPMd_Ensure_State_Capacity
                 (Model, Context_Index, Count);
            end if;
            Model.Contexts (Context_Index).State_Units := New_Units;
            Model.Contexts (Context_Index).State_Capacity :=
              Natural'Min (PPMd_Max_Context_Stats, New_Units * 2);
            Model.Contexts (Context_Index).State_Position :=
              Model.Contexts (Context_Index).Position;
            return Model.Contexts (Context_Index).State_Capacity >= Count;
         end if;

         if Model.Contexts (Context_Index).State_Position = 0 then
            return False;
         end if;
         ...
         begin
            if Model.Contexts (Context_Index).States /= null
              and then Model.Contexts (Context_Index).States'Length >
                New_Capacity
            then
               declare
                  Old_States : PPMd_Context_State_Block_Access :=
                  ...
                  New_States : constant PPMd_Context_State_Block_Access :=
                    new PPMd_Context_State_Block'
                      (0 .. New_Capacity - 1 => (0, 0, 0));
                  Copy_Count : constant Natural :=
                    Natural'Min (Model.Contexts (Context_Index).Num_Stats,
                                 New_Capacity);
               begin
                  if Copy_Count > 0 then
                     for I in 0 .. Copy_Count - 1 loop
            ...
            if Old_Units > New_Units
              and then Old_Units <= Natural'Last / PPMd_Unit_Size
              and then New_Units <= Natural'Last / PPMd_Unit_Size
            then
               declare
                  State_Position : constant Natural :=
                    Model.Contexts (Context_Index).State_Position;
                  Old_Bytes      : constant Natural :=
                    Old_Units * PPMd_Unit_Size;
                  New_Bytes      : constant Natural :=
                    New_Units * PPMd_Unit_Size;
               begin
                  if State_Position <= Natural'Last - Old_Bytes
                    and then State_Position <= Natural'Last - New_Bytes
                    and then State_Position + Old_Bytes = Model.Lo_Unit
                  then
                     Model.Lo_Unit := State_Position + New_Bytes;
                  end if;
               end;
            end if;
         ...
         return Model.Contexts (Context_Index).State_Capacity >= Count;
      end PPMd_Shrink_State_Capacity;

      function PPMd_Initial_State_Index_For
        (Model  : PPMd_Model_State;
         Symbol : Byte;
         Index  : out Natural) return Boolean;

      function PPMd_Create_First_Successor_Context
        (Model : in out PPMd_Model_State;
         Symbol : Byte) return Boolean
      is
         State_Index : Natural;
         Raw_Pos     : constant Natural := Model.Text_Pos;
         Context_Pos : Natural;
         Context_Index : Natural;
      begin
         if not PPMd_Initial_State_Index_For (Model, Symbol, State_Index)
           or else Raw_Pos = 0
           or else Raw_Pos > Model.Units_Start
         then
            return False;
         end if;

         if not PPMd_Allocate_Context (Model, Context_Index, Context_Pos) then
            return False;
         end if;

         Model.Contexts (Context_Index) :=
           (Allocated => True,
            Next_Free => 0,
            Position  => Context_Pos,
            Num_Stats => 1,
            Summ_Freq => 0,
            Suffix    => Model.Max_Context_Pos,
            State_Position => 0,
            State_Units => 0,
            State_Capacity => 0,
            One_State =>
              (Symbol    => Symbol,
               Freq      => Model.Initial_States (State_Index).Freq,
               Successor => Raw_Pos),
            States    => null);
         Model.Initial_States (State_Index).Successor := Context_Pos;
         Model.Max_Context_Pos := Context_Pos;
         Model.Current_Context_Index := Context_Index;

         if not Use_State_Blocks and then Model.Order_Fall > 0 then
            Model.Order_Fall := Model.Order_Fall - 1;
         end if;

         return True;
      end PPMd_Create_First_Successor_Context;

      procedure PPMd_Update_Suffix_Frequency
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural;
         Symbol        : Byte);

      function PPMd_Set_State_Successor_By_Pos
        (Model     : in out PPMd_Model_State;
         State_Pos : Natural;
         Successor : Natural) return Boolean is
      begin
         if State_Pos >= Model.Units_Start
           and then State_Pos < Model.Units_Start
             + Model.Initial_States'Length * 6
         then
            declare
               State_Index : Natural :=
                 (State_Pos - Model.Units_Start) / 6;
            begin
               if State_Pos = Model.Found_State_Pos then
                  if not PPMd_Initial_State_Index_For
                    (Model, Model.Found_State_Symbol, State_Index)
                  then
                     return False;
                  end if;
               elsif State_Index not in Model.Initial_States'Range
                 or else Model.Units_Start + State_Index * 6 /= State_Pos
               then
                  return False;
               end if;

               Model.Initial_States (State_Index).Successor := Successor;
               if State_Pos = Model.Found_State_Pos then
                  Model.Found_State_Successor := Successor;
                  Model.Found_State_Pos := Model.Units_Start + State_Index * 6;
                  Model.Found_State_Index := State_Index;
               end if;
               return True;
            end;
         end if;

         for Context_Index in Model.Contexts.all'Range loop
            if Model.Contexts (Context_Index).Allocated then
               if Model.Contexts (Context_Index).Num_Stats = 1
                 and then Model.Contexts (Context_Index).Position = State_Pos
               then
                  Model.Contexts (Context_Index).One_State.Successor :=
                    Successor;
                  if State_Pos = Model.Found_State_Pos then
                  ...
                  return True;
               elsif PPMd_Has_State_Block
                 (Model.Contexts (Context_Index))
               then
                  for State_Index in
                    0 .. Model.Contexts (Context_Index).Num_Stats - 1
                  loop
                     if Model.Contexts (Context_Index).State_Position
                       + State_Index * 6 = State_Pos
                     then
                        Model.Contexts (Context_Index).States
                          (State_Index).Successor := Successor;
                        if State_Pos = Model.Found_State_Pos then
                           Model.Found_State_Successor := Successor;
                        end if;
                        return True;
         ...
         return False;
      end PPMd_Set_State_Successor_By_Pos;

      function PPMd_Find_Context_State
        (Model         : PPMd_Model_State;
         Context_Index : Natural;
         Symbol        : Byte;
         State_Pos     : out Natural;
         Successor     : out Natural;
         Freq          : out Natural) return Boolean
      is
      begin
         State_Pos := 0;
         Successor := 0;
         ...
         if Model.Contexts (Context_Index).Num_Stats = 1 then
            if Model.Contexts (Context_Index).One_State.Symbol /= Symbol then
               return False;
            end if;

            State_Pos := Model.Contexts (Context_Index).Position;
            Successor := Model.Contexts (Context_Index).One_State.Successor;
            Freq := Natural (Model.Contexts (Context_Index).One_State.Freq);
            return True;
         end if;

         if not PPMd_Has_State_Block (Model.Contexts (Context_Index)) then
            return False;
         end if;

         for I in 0 .. Model.Contexts (Context_Index).Num_Stats - 1 loop
            if Model.Contexts (Context_Index).States (I).Symbol = Symbol then
               State_Pos :=
                 Model.Contexts (Context_Index).State_Position + I * 6;
               Successor :=
                 Model.Contexts (Context_Index).States (I).Successor;
               Freq := Natural (Model.Contexts (Context_Index).States (I).Freq);
               return True;
            end if;
         end loop;

         return False;
      end PPMd_Find_Context_State;

      function PPMd_Initial_State_Index_For
        (Model : PPMd_Model_State;
         Symbol : Byte;
         Index : out Natural) return Boolean is
      begin
         for I in Model.Initial_States'Range loop
            if Model.Initial_States (I).Symbol = Symbol then
               Index := I;
               return True;
            end if;
         end loop;
         ...
         Index := 0;
         return False;
      end PPMd_Initial_State_Index_For;

      function PPMd_Initial_State_Freq_For
        (Model : PPMd_Model_State;
         Symbol : Byte) return Natural
      is
         Index : Natural;
      begin
         if PPMd_Initial_State_Index_For (Model, Symbol, Index) then
            return Natural (Model.Initial_States (Index).Freq);
         end if;

         return 1;
      end PPMd_Initial_State_Freq_For;

      function PPMd_Context_Freq_For
        (Model         : PPMd_Model_State;
         Context_Index : Natural;
         Symbol        : Byte) return Natural
      is
         ...
         Freq      : Natural;
      begin
         if Context_Index in Model.Contexts.all'Range
           and then PPMd_Find_Context_State
             (Model, Context_Index, Symbol, State_Pos, Successor, Freq)
         then
            return Natural'Max (1, Freq);
         end if;

         return PPMd_Initial_State_Freq_For (Model, Symbol);
      end PPMd_Context_Freq_For;

      function PPMd_New_Successor_Freq
        (Model         : PPMd_Model_State;
         Context_Index : Natural;
         Symbol        : Byte) return Natural
      is
         ...
         Num      : Natural;
      begin
         CF := PPMd_Context_Freq_For (Model, Context_Index, Symbol);
         if CF > 0 then
            CF := CF - 1;
         end if;

         if Context_Index in Model.Contexts.all'Range
           and then Model.Contexts (Context_Index).Allocated
           and then Model.Contexts (Context_Index).Num_Stats > 1
         then
            Sum_Freq := Model.Contexts (Context_Index).Summ_Freq;
            Num := Model.Contexts (Context_Index).Num_Stats;
         else
            Sum_Freq := Model.Initial_Context_Summ_Freq;
            Num := Model.Initial_Context_Num_Stats;
         end if;

         if Sum_Freq <= Num + CF then
            return 1;
         end if;

         S0 := Sum_Freq - Num - CF;
         if 2 * CF <= S0 then
            return 1 + (if 5 * CF > S0 then 1 else 0);
         end if;

         return 2 + (2 * CF + S0 - 1) / (2 * S0);
      end PPMd_New_Successor_Freq;

      function PPMd_Create_Raw_Successor_Context
        (Model        : in out PPMd_Model_State;
         Output       : Byte_Array;
         Decoded      : Natural;
         Raw_Pos      : Natural;
         Fallback_Sym : Byte) return Boolean
      is
         type Pending_State_Array is array (Positive range <>) of Natural;
         Pending    : Pending_State_Array (1 .. Model.Contexts.all'Length) :=
           [others => 0];
      ...
      begin
         if Raw_Pos = 0
           or else Raw_Pos >= Decoded
           or else Raw_Pos > Model.Units_Start
           or else Output'First + Raw_Pos > Output'Last
         then
            return False;
         end if;

         New_Symbol := Output (Output'First + Raw_Pos);

         if Use_State_Blocks and then Model.Order_Fall = 0 then
            Pending_Count := 0;
         else
            Pending_Count := 1;
            Pending (Pending_Count) := Model.Found_State_Pos;
         end if;
         Parent_Pos := Model.Max_Context_Pos;

         if Model.Current_Context_Index in Model.Contexts.all'Range
           and then Model.Contexts (Model.Current_Context_Index).Allocated
         then
            Chain_Index := Model.Current_Context_Index;
            loop
               Parent_Pos := Model.Contexts (Chain_Index).Suffix;
               exit when Parent_Pos = 0;

               if Parent_Pos = Model.Root_Context_Pos then
                  declare
                     Root_Index : Natural := Natural'Last;
                     Root_Successor : Natural := 0;
                  begin
                     for Candidate in Model.Initial_States'Range loop
                        if Model.Initial_States (Candidate).Symbol =
                          Fallback_Sym
                     ...
                     if Root_Index in Model.Initial_States'Range then
                        Root_Successor :=
                          Model.Initial_States (Root_Index).Successor;
                        if Root_Successor = Raw_Pos
                          and then Pending_Count < Pending'Length
                        then
                           Pending_Count := Pending_Count + 1;
                           Pending (Pending_Count) :=
                             Model.Units_Start + Root_Index * 6;
                        elsif General_Root
                          and then Root_Successor > Model.Units_Start
                        then
                           Parent_Pos := Root_Successor;
                        end if;
                     end if;
                  end;
                  exit;
               ...
               Parent_Index := PPMd_Find_Context_Index (Model, Parent_Pos);
               exit when Parent_Index = 0;

               exit when not PPMd_Find_Context_State
                 (Model, Parent_Index, Fallback_Sym, State_Pos, State_Succ,
                  State_Freq);

               if State_Succ /= Raw_Pos then
                  if Use_State_Blocks and then State_Succ > Model.Units_Start then
                     Parent_Pos := State_Succ;
                  end if;
                  exit;
               end if;

               exit when Pending_Count >= Pending'Length;

               Pending_Count := Pending_Count + 1;
               Pending (Pending_Count) := State_Pos;
               Chain_Index := Parent_Index;
            end loop;
         end if;

         if Parent_Pos = Model.Root_Context_Pos or else Parent_Pos = 0 then
            Parent_Index := 0;
         else
            Parent_Index := PPMd_Find_Context_Index (Model, Parent_Pos);
         end if;

         if Parent_Index in Model.Contexts.all'Range
           and then Model.Contexts (Parent_Index).Allocated
           and then Model.Contexts (Parent_Index).Num_Stats = 1
         then
            New_Freq :=
              Natural (Model.Contexts (Parent_Index).One_State.Freq);
         else
            New_Freq :=
              PPMd_New_Successor_Freq (Model, Parent_Index, New_Symbol);
         end if;

         if Pending_Count = 0 then
            if Parent_Index in Model.Contexts.all'Range
              and then Model.Contexts (Parent_Index).Allocated
              and then Parent_Pos > Model.Units_Start
            then
               if not PPMd_Set_State_Successor_By_Pos
                 (Model, Model.Found_State_Pos, Parent_Pos)
               then
                  return False;
               end if;

               Model.Max_Context_Pos := Parent_Pos;
               Model.Current_Context_Index := Parent_Index;
               return True;
            end if;
         ...
         for Pending_Index in reverse 1 .. Pending_Count loop
            if not PPMd_Allocate_Context
              (Model, Context_Index, Context_Pos)
            then
               return False;
            end if;
            ...
            if not PPMd_Set_State_Successor_By_Pos
              (Model, Pending (Pending_Index), Context_Pos)
            then
               return False;
            end if;

            Parent_Pos := Context_Pos;
            Parent_Index := Context_Index;
         end loop;
         ...
         if New_Symbol /= Fallback_Sym
           and then Context_Index in Model.Contexts.all'Range
           and then Model.Contexts (Context_Index).Suffix = 0
         then
            return False;
         end if;

         return True;
      end PPMd_Create_Raw_Successor_Context;

      function PPMd_Found_State_Freq (Model : PPMd_Model_State) return Natural
      is
         State_Index : Natural;
         Context_Index : Natural;
      begin
         if Model.Found_State_Pos >= Model.Units_Start
           and then Model.Found_State_Pos < Model.Units_Start
             + Model.Initial_States'Length * 6
         then
            if PPMd_Initial_State_Index_For
              (Model, Model.Found_State_Symbol, State_Index)
            then
               return Natural (Model.Initial_States (State_Index).Freq);
            end if;
         end if;

         Context_Index := PPMd_Find_Context_Index (Model, Model.Found_State_Pos);
         if Context_Index in Model.Contexts.all'Range
           and then Model.Contexts (Context_Index).Allocated
           and then Model.Contexts (Context_Index).Num_Stats = 1
         then
            return Natural (Model.Contexts (Context_Index).One_State.Freq);
         end if;

         for Candidate in Model.Contexts.all'Range loop
            if Model.Contexts (Candidate).Allocated
              and then PPMd_Has_State_Block (Model.Contexts (Candidate))
              and then Model.Found_State_Pos >= Model.Contexts (Candidate).State_Position
              and then (Model.Found_State_Pos
                        - Model.Contexts (Candidate).State_Position) mod 6 = 0
            then
               State_Index :=
                 (Model.Found_State_Pos
                  - Model.Contexts (Candidate).State_Position) / 6;
               if State_Index < Model.Contexts (Candidate).Num_Stats then
                  return
                    Natural
                      (Model.Contexts (Candidate).States (State_Index).Freq);
               end if;
            end if;
         end loop;

         return 1;
      end PPMd_Found_State_Freq;
      ...
      begin
         for I in Model.Contexts.all'Range loop
            if Model.Contexts (I).Allocated
              and then Model.Contexts (I).Position = Position
            then
               return I;
            end if;
         end loop;
         ...
         return 0;
      end PPMd_Find_Context_Index;

      function PPMd_Add_Update_State_To_Context
        (Model       : in out PPMd_Model_State;
         Context_Index : Natural;
         Symbol      : Byte;
         Successor   : Natural;
         Found_Freq  : Natural;
         Source_Num_Stats : Natural;
         Source_Escape_Freq : Natural) return Boolean
      is
         Old_Freq : Natural;
         Sum      : Natural;
         CF       : Natural;
      ...
      begin
         ...
         if Model.Contexts (Context_Index).Num_Stats = 1 then
            ...
            Old_Freq := Natural (Model.Contexts (Context_Index).One_State.Freq);
            if Old_Freq = 0 then
               return False;
            end if;

            if Old_Freq < PPMd_Max_Freq / 4 - 1 then
               Old_Freq := Old_Freq * 2;
            else
               Old_Freq := PPMd_Max_Freq - 4;
            end if;

            Sum :=
              Old_Freq + Model.Init_Esc
              + (if Source_Num_Stats > 3 then 1 else 0);
            State_Index := 1;
            if not PPMd_Ensure_State_Capacity
              (Model, Context_Index, 2)
            then
               return False;
            end if;
            Model.Contexts (Context_Index).States (0) :=
              (Symbol    => Model.Contexts (Context_Index).One_State.Symbol,
               Freq      => Byte (Old_Freq),
               Successor =>
                 Model.Contexts (Context_Index).One_State.Successor);
         else
            if PPMd_Context_Has_Symbol
              (Model.Contexts (Context_Index), Symbol)
            then
               return True;
            elsif not PPMd_Has_State_Block
              (Model.Contexts (Context_Index))
            then
               if not PPMd_Ensure_State_Capacity
                 (Model, Context_Index,
                  Model.Contexts (Context_Index).Num_Stats + 1)
               then
                  return False;
               end if;
            elsif Model.Contexts (Context_Index).Num_Stats
              >= Model.Contexts (Context_Index).State_Capacity
            then
               if not PPMd_Ensure_State_Capacity
                 (Model, Context_Index,
            ...
            State_Index := Model.Contexts (Context_Index).Num_Stats;
            Sum := Model.Contexts (Context_Index).Summ_Freq;
            Sum :=
              Sum
              + (if 2 * State_Index < Source_Num_Stats then 1 else 0)
              + (if 4 * State_Index <= Source_Num_Stats
                   and then Sum <= 8 * State_Index
                 then 2 else 0);
         end if;

         SF := Source_Escape_Freq + Sum;
         if SF = 0
           or else Found_Freq > Natural'Last / (2 * (Sum + 6))
         then
            return False;
         end if;

         CF := 2 * (Sum + 6) * Found_Freq;
         if CF < 6 * SF then
            CF :=
              1
              + (if CF > SF then 1 else 0)
              + (if CF >= 4 * SF then 1 else 0);
            Sum := Sum + 3;
         else
            CF :=
              4
              + (if CF >= 9 * SF then 1 else 0)
         ...
         if CF = 0
           or else CF > Natural (Byte'Last)
           or else Sum > Natural (Interfaces.Unsigned_16'Last)
         then
            return False;
         end if;

         Model.Contexts (Context_Index).States (State_Index) :=
           (Symbol    => Symbol,
            Freq      => Byte (CF),
            Successor => Successor);
         Model.Contexts (Context_Index).Num_Stats := State_Index + 1;
         Model.Contexts (Context_Index).Summ_Freq := Sum;
         if State_Index = 1 then
            Model.Contexts (Context_Index).One_State := (0, 0, 0);
         end if;
         return True;
      end PPMd_Add_Update_State_To_Context;

      function PPMd_Select_First_Successor_Context
        (Model : in out PPMd_Model_State;
         Symbol : Byte) return Boolean
      is
         State_Index   : Natural;
         Successor_Pos : Natural;
         Context_Index : Natural;
      begin
         if not PPMd_Initial_State_Index_For (Model, Symbol, State_Index) then
            return False;
         end if;

         Successor_Pos := Model.Initial_States (State_Index).Successor;
         if Successor_Pos > Model.Units_Start then
            Context_Index := PPMd_Find_Context_Index (Model, Successor_Pos);
            if Context_Index = 0 then
               return False;
            end if;

            Model.Max_Context_Pos := Successor_Pos;
            Model.Current_Context_Index := Context_Index;
            return True;
         end if;

         return PPMd_Create_First_Successor_Context (Model, Symbol);
      end PPMd_Select_First_Successor_Context;

      function PPMd_Select_Found_Successor_Context
        (Model   : in out PPMd_Model_State;
         Symbol  : Byte;
         Output  : Byte_Array;
         Decoded : Natural) return Boolean
      is
         Context_Index : Natural;
         Old_Current_Index : constant Natural := Model.Current_Context_Index;
         Old_Max_Pos       : constant Natural := Model.Max_Context_Pos;
         Old_Max_Index     : Natural := 0;
         Found_Freq        : Natural;
         Source_Num_Stats  : Natural := 256;
         Source_Summ_Freq  : Natural := Model.Initial_Context_Summ_Freq;
         Source_Escape_Freq : Natural := 1;
         Max_Successor     : Natural := Decoded;
         Min_Successor     : Natural := Model.Found_State_Successor;
         Created_Raw_Successor : Boolean := False;
         Root_Raw_Successor : Boolean := False;
         Walk_Index        : Natural;
         Guard             : Natural := 0;

         function Update_Failed return Boolean is
         begin
            if Model.Restarted then
               Model.Restarted := False;
               return True;
            end if;
      ...
      begin
         Model.Restarted := False;
         if not General_Root or else not Full_Update_Model then
            if Model.Found_State_Successor > Model.Units_Start then
               Context_Index :=
                 PPMd_Find_Context_Index (Model, Model.Found_State_Successor);
               if Context_Index = 0 then
                  return False;
               end if;

               Model.Max_Context_Pos := Model.Found_State_Successor;
               Model.Current_Context_Index := Context_Index;
               return True;
            end if;

            if Model.Found_State_Successor /= 0 then
               if not PPMd_Create_Raw_Successor_Context
                 (Model, Output, Decoded, Model.Found_State_Successor, Symbol)
               then
            ...
            if General_Root then
               if not PPMd_Set_State_Successor_By_Pos
                 (Model, Model.Found_State_Pos, Decoded)
               then
                  return False;
               end if;
            ...
            return PPMd_Select_First_Successor_Context (Model, Symbol);
         end if;

         Found_Freq := PPMd_Found_State_Freq (Model);

         if Old_Current_Index in Model.Contexts.all'Range
           and then Model.Contexts (Old_Current_Index).Allocated
         then
            Source_Num_Stats := Model.Contexts (Old_Current_Index).Num_Stats;
            if Source_Num_Stats = 1 then
               Source_Summ_Freq :=
                 Natural (Model.Contexts (Old_Current_Index).One_State.Freq)
                 + 1;
            else
               Source_Summ_Freq :=
                 Model.Contexts (Old_Current_Index).Summ_Freq;
            end if;

            if Found_Freq < PPMd_Max_Freq / 4
              and then Model.Contexts (Old_Current_Index).Suffix /= 0
            then
               PPMd_Update_Suffix_Frequency
                 (Model, Old_Current_Index, Symbol);
            end if;
         end if;

         if Source_Summ_Freq >= Source_Num_Stats + Found_Freq - 1 then
            Source_Escape_Freq :=
              Source_Summ_Freq - Source_Num_Stats - (Found_Freq - 1);
         end if;
         ...
         if Model.Found_State_Successor > Model.Units_Start then
            ...
            Min_Successor := Model.Found_State_Successor;
         end if;

         if Model.Found_State_Successor > Model.Units_Start then
            null;
         elsif Model.Found_State_Successor /= 0 then
            if not PPMd_Create_Raw_Successor_Context
              (Model, Output, Decoded, Model.Found_State_Successor, Symbol)
            then
               if Update_Failed then
            ...
            Min_Successor := Model.Max_Context_Pos;
            if Use_State_Blocks
              and then Model.Order_Fall = 0
              and then Min_Successor > Model.Units_Start
              and then not PPMd_Set_State_Successor_By_Pos
                (Model, Model.Found_State_Pos, Min_Successor)
            then
               return False;
            end if;
            ...
            Created_Raw_Successor := True;
         elsif not PPMd_Set_State_Successor_By_Pos
           (Model, Model.Found_State_Pos, Max_Successor)
         then
            return False;
         elsif Use_State_Blocks and then Old_Current_Index = 0 then
            Min_Successor := Model.Root_Context_Pos;
            Root_Raw_Successor := True;
         else
            Model.Current_Context_Index := 0;
            Model.Max_Context_Pos := Model.Root_Context_Pos;
            return True;
         ...
         if Min_Successor > Model.Units_Start then
            Context_Index := PPMd_Find_Context_Index (Model, Min_Successor);
            if Context_Index = 0 then
               return False;
            end if;

            if Model.Order_Fall > 0
              and then (Use_State_Blocks or else not Created_Raw_Successor)
            then
               Model.Order_Fall := Model.Order_Fall - 1;
               if Model.Order_Fall = 0 then
                  Max_Successor := Min_Successor;
                  if Old_Current_Index in Model.Contexts.all'Range
                    and then Model.Contexts (Old_Current_Index).Allocated
                    and then Old_Max_Pos /=
                      Model.Contexts (Old_Current_Index).Position
                    and then Model.Text_Pos > 0
                  then
                     Model.Text_Pos := Model.Text_Pos - 1;
                  end if;
               end if;
            ...
            Model.Current_Context_Index := Context_Index;
            Model.Max_Context_Pos := Min_Successor;
         elsif Root_Raw_Successor then
            Model.Current_Context_Index := 0;
            Model.Max_Context_Pos := Model.Root_Context_Pos;
         elsif Old_Current_Index in Model.Contexts.all'Range
           and then Model.Contexts (Old_Current_Index).Allocated
         then
            Model.Current_Context_Index := Old_Current_Index;
            Model.Max_Context_Pos := Old_Max_Pos;
         else
            Model.Current_Context_Index := 0;
            Model.Max_Context_Pos := Model.Root_Context_Pos;
         end if;

         Old_Max_Index := PPMd_Find_Context_Index (Model, Old_Max_Pos);
         if Old_Max_Index = 0
           or else Old_Max_Index = Old_Current_Index
         then
            return True;
         end if;

         Walk_Index := Old_Max_Index;
         while Walk_Index in Model.Contexts.all'Range
           and then Model.Contexts (Walk_Index).Allocated
           and then Walk_Index /= Old_Current_Index
         loop
            if not PPMd_Context_Has_Symbol
              (Model.Contexts (Walk_Index), Symbol)
              and then not PPMd_Add_Update_State_To_Context
                (Model, Walk_Index, Symbol, Max_Successor, Found_Freq,
                 Source_Num_Stats, Source_Escape_Freq)
            then
               if Update_Failed then
                  return True;
               end if;
            ...
            exit when Model.Contexts (Walk_Index).Suffix = 0
              or else Model.Contexts (Walk_Index).Suffix = Model.Root_Context_Pos;

            Walk_Index :=
              PPMd_Find_Context_Index
                (Model, Model.Contexts (Walk_Index).Suffix);

            Guard := Guard + 1;
            if Guard > Model.Contexts.all'Length then
               return False;
            end if;
         end loop;

         return True;
      end PPMd_Select_Found_Successor_Context;

      function PPMd_Append_State_To_Context
        (Model     : in out PPMd_Model_State;
         Context_Index : Natural;
         Decoded_Context : Natural;
         Old_Symbol : Byte;
         New_Symbol : Byte) return Boolean
      is
         New_Index     : constant Natural := Natural (New_Symbol);
         Old_Freq      : Natural;
         New_Freq      : Natural;
         Decoded_Num_Stats : Natural := 0;
         Decoded_Summ_Freq : Natural := 0;
         Found_Freq        : Natural := 0;
      begin
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
           or else Model.Contexts (Context_Index).Num_Stats /= 1
           or else Model.Contexts (Context_Index).One_State.Symbol /= Old_Symbol
           or else New_Index not in Model.Initial_States'Range
         then
            return False;
         end if;
         ...
         if General_Root then
            if Decoded_Context = 0 then
               Decoded_Num_Stats := Model.Initial_Context_Num_Stats;
               Decoded_Summ_Freq := Model.Initial_Context_Summ_Freq;
               Found_Freq := PPMd_Initial_State_Freq_For (Model, New_Symbol);
            elsif Decoded_Context in Model.Contexts.all'Range
              and then Model.Contexts (Decoded_Context).Allocated
            then
               Decoded_Num_Stats :=
                 Model.Contexts (Decoded_Context).Num_Stats;
               if Decoded_Num_Stats = 1 then
                  if Model.Contexts (Decoded_Context).One_State.Symbol
                    /= New_Symbol
                  then
                     return False;
                  end if;
                  ...
                  Decoded_Summ_Freq :=
                    Natural (Model.Contexts (Decoded_Context).One_State.Freq)
                    + 1;
                  Found_Freq :=
                    Natural (Model.Contexts (Decoded_Context).One_State.Freq);
               else
                  Decoded_Summ_Freq :=
                    Model.Contexts (Decoded_Context).Summ_Freq;
                  Found_Freq :=
                    PPMd_Context_Freq_For
                      (Model, Decoded_Context, New_Symbol);
               end if;
            end if;

            if Decoded_Num_Stats = 0
              or else Decoded_Summ_Freq < Decoded_Num_Stats
              or else Found_Freq = 0
              or else Decoded_Summ_Freq
                < Decoded_Num_Stats + Found_Freq - 1
            then
               return False;
            end if;
            ...
            declare
               S0 : constant Natural :=
                 Decoded_Summ_Freq - Decoded_Num_Stats - (Found_Freq - 1);
               Sum : Natural :=
                 Old_Freq + Model.Init_Esc
                 + (if Decoded_Num_Stats > 3 then 1 else 0);
               CF  : Natural := 2 * (Sum + 6) * Found_Freq;
               SF  : constant Natural := S0 + Sum;
            begin
               if SF = 0 then
                  return False;
               end if;
            ...
            New_Freq := PPMd_Initial_State_Freq_For (Model, New_Symbol);
            if New_Freq < 4 then
               New_Freq := 1;
            else
               New_Freq := Natural'Min (New_Freq / 2, 16);
            end if;
         end if;

         if not PPMd_Ensure_State_Capacity (Model, Context_Index, 2) then
            return False;
         end if;
         ...
         Model.Contexts (Context_Index).States (0) :=
           (Symbol    => Old_Symbol,
            ...
            Successor =>
              Model.Contexts (Context_Index).One_State.Successor);
         Model.Contexts (Context_Index).States (1) :=
           (Symbol    => New_Symbol,
            Freq      => Byte (New_Freq),
            Successor => Model.Text_Pos);
         Model.Contexts (Context_Index).One_State := (0, 0, 0);
         Model.Contexts (Context_Index).Num_Stats := 2;
         if not General_Root then
            Model.Contexts (Context_Index).Summ_Freq :=
              Old_Freq + New_Freq + Model.Init_Esc + 1;
         end if;
         return True;
      end PPMd_Append_State_To_Context;

      function PPMd_Append_New_State_To_Multi_Context
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural;
         New_Symbol    : Byte) return Boolean
      is
      ...
      begin
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
           or else Model.Contexts (Context_Index).Num_Stats < 2
           or else New_Index not in Model.Initial_States'Range
           or else PPMd_Context_Has_Symbol
             (Model.Contexts (Context_Index), New_Symbol)
         then
            return False;
         end if;
         ...
         State_Index := Model.Contexts (Context_Index).Num_Stats;
         if not PPMd_Ensure_State_Capacity
           (Model, Context_Index, State_Index + 1)
         then
            return False;
         end if;
         Model.Contexts (Context_Index).States (State_Index) :=
         ...
         Model.Contexts (Context_Index).Num_Stats := State_Index + 1;
         Model.Contexts (Context_Index).Summ_Freq :=
           Model.Contexts (Context_Index).Summ_Freq + New_Freq + 1;
         return True;
      end PPMd_Append_New_State_To_Multi_Context;

      function PPMd_Propagate_Escaped_Symbol
        (Model            : in out PPMd_Model_State;
         Escaped_Context  : Natural;
         Decoded_Context  : Natural;
         New_Symbol       : Byte) return Boolean
      is
         Context_Index : Natural := Escaped_Context;
         Suffix_Pos    : Natural;
         Guard         : Natural := 0;
      begin
         while Context_Index in Model.Contexts.all'Range
           and then Model.Contexts (Context_Index).Allocated
           and then Context_Index /= Decoded_Context
         loop
            if not PPMd_Context_Has_Symbol
              (Model.Contexts (Context_Index), New_Symbol)
            then
               if Model.Contexts (Context_Index).Num_Stats = 1 then
                  if not PPMd_Append_State_To_Context
                    (Model, Context_Index,
                     Decoded_Context,
                     Model.Contexts (Context_Index).One_State.Symbol,
                     New_Symbol)
                  then
                     return False;
                  end if;
               else
                  if not PPMd_Append_New_State_To_Multi_Context
                    (Model, Context_Index, New_Symbol)
                  then
                     return False;
                  end if;
               end if;
            ...
            Suffix_Pos := Model.Contexts (Context_Index).Suffix;
            exit when Suffix_Pos = 0 or else Suffix_Pos = Model.Root_Context_Pos;

            Context_Index := PPMd_Find_Context_Index (Model, Suffix_Pos);
            exit when Context_Index = 0;
         ...
         return True;
      end PPMd_Propagate_Escaped_Symbol;
      ...
      begin
         ...
         if Suffix_Pos = Model.Root_Context_Pos then
            for I in Model.Initial_States'Range loop
               if Model.Initial_States (I).Symbol = Symbol then
                  if Model.Initial_States (I).Freq < PPMd_Max_Freq - 9
                    and then Model.Initial_Context_Summ_Freq
                      <= Natural (Interfaces.Unsigned_16'Last) - 2
                  then
                     Model.Initial_States (I).Freq :=
                       Model.Initial_States (I).Freq + 2;
                     Model.Initial_Context_Summ_Freq :=
                       Model.Initial_Context_Summ_Freq + 2;
                  end if;
                  return;
               end if;
            end loop;
         ...
         Suffix_Index := PPMd_Find_Context_Index (Model, Suffix_Pos);
         if Suffix_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Suffix_Index).Allocated
         then
            return;
         end if;

         if Model.Contexts (Suffix_Index).Num_Stats = 1 then
            if Model.Contexts (Suffix_Index).One_State.Symbol = Symbol
              and then Model.Contexts (Suffix_Index).One_State.Freq < 32
            then
               Model.Contexts (Suffix_Index).One_State.Freq :=
                 Model.Contexts (Suffix_Index).One_State.Freq + 1;
            end if;
            return;
         end if;

         if not PPMd_Has_State_Block (Model.Contexts (Suffix_Index)) then
            return;
         end if;

         for I in 0 .. Model.Contexts (Suffix_Index).Num_Stats - 1 loop
            if Model.Contexts (Suffix_Index).States (I).Symbol = Symbol then
               declare
                  Update_Index : Natural := I;
               begin
                  if I > 0
                    and then Model.Contexts (Suffix_Index).States (I).Freq
                      >= Model.Contexts (Suffix_Index).States (I - 1).Freq
                  then
                     declare
                        Temp : constant PPMd_State :=
                          Model.Contexts (Suffix_Index).States (I - 1);
                     begin
                        Model.Contexts (Suffix_Index).States (I - 1) :=
                          Model.Contexts (Suffix_Index).States (I);
                        Model.Contexts (Suffix_Index).States (I) := Temp;
                        Update_Index := I - 1;
                     end;
                  end if;

                  if Model.Contexts (Suffix_Index).States
                    (Update_Index).Freq < PPMd_Max_Freq - 9
                  then
                     Model.Contexts (Suffix_Index).States
                       (Update_Index).Freq :=
                       Model.Contexts (Suffix_Index).States
                         (Update_Index).Freq + 2;
                     Model.Contexts (Suffix_Index).Summ_Freq :=
                       Model.Contexts (Suffix_Index).Summ_Freq + 2;
                  end if;
               end;
               return;
            end if;
         end loop;
      end PPMd_Update_Suffix_Frequency;

      function PPMd_Rescale_Multi_Context
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural) return Boolean
      is
         State_Count : Natural;
      ...
      begin
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
           or else Model.Contexts (Context_Index).Num_Stats <= 1
           or else not PPMd_Has_State_Block
             (Model.Contexts (Context_Index))
         then
            return False;
         end if;

         State_Count := Model.Contexts (Context_Index).Num_Stats;
         Original_Count := State_Count;
         if Model.Found_State_Pos >= Model.Contexts (Context_Index).State_Position
           and then (Model.Found_State_Pos
                     - Model.Contexts (Context_Index).State_Position) mod 6 = 0
         then
            Found_Index :=
              (Model.Found_State_Pos
               - Model.Contexts (Context_Index).State_Position) / 6;
         end if;

         if Found_Index >= State_Count then
            return False;
         end if;

         Temp := Model.Contexts (Context_Index).States (Found_Index);
         while Found_Index > 0 loop
            Model.Contexts (Context_Index).States (Found_Index) :=
              Model.Contexts (Context_Index).States (Found_Index - 1);
            Found_Index := Found_Index - 1;
         end loop;
         Model.Contexts (Context_Index).States (0) := Temp;
         Model.Found_State_Pos := Model.Contexts (Context_Index).State_Position;
         Model.Found_State_Symbol :=
           Model.Contexts (Context_Index).States (0).Symbol;

         Sum_Freq := Natural (Model.Contexts (Context_Index).States (0).Freq);
         if Sum_Freq = 0
           or else Sum_Freq > Model.Contexts (Context_Index).Summ_Freq
         then
            return False;
         end if;

         Escape_Freq := Model.Contexts (Context_Index).Summ_Freq - Sum_Freq;
         Sum_Freq := (Sum_Freq + 4 + Adder) / 2;
         if Sum_Freq = 0 or else Sum_Freq > Natural (Byte'Last) then
            return False;
         end if;
         Model.Contexts (Context_Index).States (0).Freq := Byte (Sum_Freq);

         for I in 1 .. State_Count - 1 loop
            declare
               Old_Freq : constant Natural :=
                 Natural (Model.Contexts (Context_Index).States (I).Freq);
               New_Freq : Natural;
               Insert_Index : Natural := I;
               Current_State : PPMd_State := Model.Contexts
                 (Context_Index).States (I);
            begin
               if Old_Freq > Escape_Freq then
                  return False;
               end if;
               ...
               Escape_Freq := Escape_Freq - Old_Freq;
               New_Freq := (Old_Freq + Adder) / 2;
               Current_State.Freq := Byte (New_Freq);
               Sum_Freq := Sum_Freq + New_Freq;
                 ...
                 and then New_Freq >
                   Natural
                     (Model.Contexts (Context_Index).States
                        (Insert_Index - 1).Freq)
               loop
                  Model.Contexts (Context_Index).States (Insert_Index) :=
                    Model.Contexts (Context_Index).States (Insert_Index - 1);
                  Insert_Index := Insert_Index - 1;
               end loop;
               Model.Contexts (Context_Index).States (Insert_Index) :=
                 Current_State;
         ...
         while State_Count > 0
           and then Model.Contexts (Context_Index).States
             (State_Count - 1).Freq = 0
         loop
            Removed := Removed + 1;
         ...
         if Removed > 0 then
            Escape_Freq := Escape_Freq + Removed;
         end if;
         ...
         if New_Count = 1 then
            declare
               Only_State : PPMd_State :=
                 Model.Contexts (Context_Index).States (0);
               Adjusted_Escape : Natural := Escape_Freq;
               Adjusted_Freq   : Natural := Natural (Only_State.Freq);
            begin
               while Adjusted_Escape > 1 loop
                  Adjusted_Escape := Adjusted_Escape / 2;
                  Adjusted_Freq := (Adjusted_Freq + 1) / 2;
               end loop;

               Only_State.Freq :=
                 Byte (Natural'Max (1, Natural'Min
                   (Adjusted_Freq, Natural (Byte'Last))));
               Model.Contexts (Context_Index).One_State := Only_State;
               Model.Contexts (Context_Index).Num_Stats := 1;
               Model.Contexts (Context_Index).Summ_Freq := 0;
               PPMd_Release_State_Block (Model, Context_Index);
               Model.Found_State_Pos :=
                 Model.Contexts (Context_Index).Position;
               Model.Found_State_Successor := Only_State.Successor;
               Model.Found_State_Symbol := Only_State.Symbol;
               return True;
            end;
         end if;

         for I in New_Count .. Original_Count - 1 loop
            Model.Contexts (Context_Index).States (I) := (0, 0, 0);
         end loop;

         Model.Contexts (Context_Index).Num_Stats := New_Count;
         if not PPMd_Shrink_State_Capacity
           (Model, Context_Index, New_Count)
         then
            return False;
         end if;
         Model.Contexts (Context_Index).Summ_Freq :=
           Sum_Freq + Escape_Freq - Escape_Freq / 2;
         Model.Found_State_Pos := Model.Contexts (Context_Index).State_Position;
         Model.Found_State_Successor :=
           Model.Contexts (Context_Index).States (0).Successor;
         Model.Found_State_Symbol :=
           Model.Contexts (Context_Index).States (0).Symbol;
         return True;
      end PPMd_Rescale_Multi_Context;

      function PPMd_Promote_Context_State
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural;
         State_Index   : Natural) return Boolean
      is
         Index : Natural := State_Index;
         Temp  : PPMd_State;
      begin
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
           or else State_Index >= Model.Contexts (Context_Index).Num_Stats
           or else not PPMd_Has_State_Block
             (Model.Contexts (Context_Index))
         then
            return False;
         ...
         if Use_State_Blocks then
            if Index > 0
              and then Model.Contexts (Context_Index).States (Index).Freq
                > Model.Contexts (Context_Index).States (Index - 1).Freq
            then
               Temp := Model.Contexts (Context_Index).States (Index - 1);
               Model.Contexts (Context_Index).States (Index - 1) :=
                 Model.Contexts (Context_Index).States (Index);
               Model.Contexts (Context_Index).States (Index) := Temp;
               Index := Index - 1;
            end if;

            Model.Found_State_Pos :=
              Model.Contexts (Context_Index).State_Position + Index * 6;
            Model.Found_State_Symbol :=
              Model.Contexts (Context_Index).States (Index).Symbol;
            return True;
         end if;
         ...
         Model.Found_State_Symbol :=
           Model.Contexts (Context_Index).States (Index).Symbol;
         return True;
      end PPMd_Promote_Context_State;

      function PPMd_Hi_Bits_Flag_3 (Symbol : Byte) return Natural is
      begin
         return Natural
           (Interfaces.Shift_Right
              (Interfaces.Unsigned_32 (Symbol) + 16#C0#, 5)
            and 8);
      end PPMd_Hi_Bits_Flag_3;

      function PPMd_Hi_Bits_Flag_4 (Symbol : Byte) return Natural is
      begin
         return Natural
           (Interfaces.Shift_Right
              (Interfaces.Unsigned_32 (Symbol) + 16#C0#, 4)
            and 16);
      end PPMd_Hi_Bits_Flag_4;

      function PPMd_Update_Prob_1
        (Prob : Interfaces.Unsigned_32) return Interfaces.Unsigned_32
      is
      begin
         return Prob - Interfaces.Shift_Right (Prob + 32, PPMd_Period_Bits);
      end PPMd_Update_Prob_1;

      function PPMd_First_Context_Bin_Summ_Index
        (Model  : in out PPMd_Model_State;
         Suffix : Natural;
         Symbol : Byte;
         Freq   : Byte) return Natural
      is
         Suffix_Index     : Natural := 0;
         Suffix_Num_Stats : Natural :=
           (if Suffix = Model.Root_Context_Pos then 256 else 256);
         Prev_Symbol      : constant Byte := Model.Found_State_Symbol;
      begin
         Model.Hi_Bits_Flag := PPMd_Hi_Bits_Flag_3 (Prev_Symbol);

         if Suffix /= 0 and then Suffix /= Model.Root_Context_Pos then
            Suffix_Index := PPMd_Find_Context_Index (Model, Suffix);
            if Suffix_Index in Model.Contexts.all'Range
              and then Model.Contexts (Suffix_Index).Allocated
              and then Model.Contexts (Suffix_Index).Num_Stats > 0
            then
               Suffix_Num_Stats := Model.Contexts (Suffix_Index).Num_Stats;
            end if;
         end if;

         if Freq = 0
           or else Suffix_Num_Stats = 0
         then
            return Natural'Last;
         end if;

         return Model.Prev_Success
           + (if Model.Run_Length < 0 then 32 else 0)
           + PPMd_Hi_Bits_Flag_4 (Symbol)
           + Model.Hi_Bits_Flag
           + Natural (Model.NS2BSIndx (Suffix_Num_Stats - 1));
      end PPMd_First_Context_Bin_Summ_Index;

      function PPMd_Decode_First_Context_Binary_Hit
        (Input  : Byte_Array;
         D      : in out PPMd_Range_Decoder;
         Model  : in out PPMd_Model_State;
         Symbol : out Byte) return Boolean
      is
         Context_Index : constant Natural := Model.Current_Context_Index;
         Bin_Index     : Natural;
         Prob          : Interfaces.Unsigned_32;
         Size0         : Interfaces.Unsigned_32;
      begin
         Symbol := 0;
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
           or else Model.Contexts (Context_Index).Num_Stats /= 1
           or else Model.Contexts (Context_Index).One_State.Freq = 0
         then
            return False;
         end if;

         Bin_Index :=
           PPMd_First_Context_Bin_Summ_Index
             (Model,
              Model.Contexts (Context_Index).Suffix,
              Model.Contexts (Context_Index).One_State.Symbol,
              Model.Contexts (Context_Index).One_State.Freq);
         if Bin_Index > Model.BinSumm'Last (2)
           or else Natural (Model.Contexts (Context_Index).One_State.Freq)
             > Model.BinSumm'Last (1) + 1
         then
            return False;
         end if;
         Prob :=
           Interfaces.Unsigned_32
             (Model.BinSumm
                (Natural (Model.Contexts (Context_Index).One_State.Freq) - 1,
                 Bin_Index));
         Size0 :=
           Interfaces.Shift_Right (D.Span, 14) * Prob;
         Prob := PPMd_Update_Prob_1 (Prob);

         if D.Code >= Size0 then
            return False;
         end if;

         if Prob > Interfaces.Unsigned_32 (Interfaces.Unsigned_16'Last)
           - 2 ** PPMd_Int_Bits
         then
            return False;
         end if;
         ...
         Model.BinSumm
           (Natural (Model.Contexts (Context_Index).One_State.Freq) - 1,
            Bin_Index) :=
             Interfaces.Unsigned_16 (Prob + 2 ** PPMd_Int_Bits);

         D.Span := Size0;
         if not PPMd_Range_Normalize_One (Input, D) then
            return False;
         end if;

         Symbol := Model.Contexts (Context_Index).One_State.Symbol;
         Model.Found_State_Index := Natural (Symbol);
         Model.Found_State_Pos := Model.Contexts (Context_Index).Position;
         Model.Found_State_Successor :=
           Model.Contexts (Context_Index).One_State.Successor;
         Model.Found_State_Symbol := Symbol;
         Model.Prev_Success := 1;
         Model.Run_Length := Model.Run_Length + 1;

         if Model.Contexts (Context_Index).One_State.Freq < 128 then
            Model.Contexts (Context_Index).One_State.Freq :=
              Model.Contexts (Context_Index).One_State.Freq + 1;
         end if;

         return True;
      end PPMd_Decode_First_Context_Binary_Hit;

      function PPMd_Decode_First_Context_Binary_Miss
        (Input : Byte_Array;
         D     : in out PPMd_Range_Decoder;
         Model : in out PPMd_Model_State) return Boolean
      is
      ...
      begin
         ...
         if D.Code < Size0 or else D.Span <= Size0 then
            return False;
         end if;

         Model.BinSumm
           (Natural (Model.Contexts (Context_Index).One_State.Freq) - 1,
            Bin_Index) := Interfaces.Unsigned_16 (Prob);

         Init_Index := Natural (Interfaces.Shift_Right (Prob, 10));
         if Init_Index > Model.ExpEscape'Last then
            return False;
         end if;

         Model.Init_Esc := Natural (Model.ExpEscape (Init_Index));
         D.Code := D.Code - Size0;
         D.Span := D.Span - Size0;

         Model.Prev_Success := 0;
         return True;
      end PPMd_Decode_First_Context_Binary_Miss;

      function PPMd_Decode_Current_Multi_Symbol
        (Input   : Byte_Array;
         D       : in out PPMd_Range_Decoder;
         Model   : in out PPMd_Model_State;
         Symbol  : out Byte;
         Escaped : out Boolean) return Boolean
      is
         Context_Index : constant Natural := Model.Current_Context_Index;
         Threshold     : Interfaces.Unsigned_32;
         Count         : Interfaces.Unsigned_32;
         ...
         State_Count   : Natural;
         Freq          : Natural;
         Total_Freq    : Interfaces.Unsigned_32 := 0;
      begin
         Symbol := 0;
         Escaped := False;
         if Context_Index not in Model.Contexts.all'Range
           ...
           or else not PPMd_Has_State_Block
             (Model.Contexts (Context_Index))
           or else Model.Contexts (Context_Index).Summ_Freq = 0
         then
            return False;
         end if;

         State_Count := Model.Contexts (Context_Index).Num_Stats;
         for I in 0 .. State_Count - 1 loop
            if Model.Contexts (Context_Index).States (I).Freq = 0 then
               return False;
            end if;
            ...
            Total_Freq :=
              Total_Freq
              + Interfaces.Unsigned_32
                (Model.Contexts (Context_Index).States (I).Freq);
         end loop;

         if Total_Freq = 0
           or else Total_Freq > Interfaces.Unsigned_32
             (Model.Contexts (Context_Index).Summ_Freq)
         then
            return False;
         end if;
         ...
         if Threshold < Total_Freq then
            Count := Threshold;
            for I in 0 .. State_Count - 1 loop
               declare
                  State_Freq : constant Interfaces.Unsigned_32 :=
                    Interfaces.Unsigned_32
                      (Model.Contexts (Context_Index).States (I).Freq);
                  Selected_Index : Natural := I;
                  Updated_Pos    : Natural;
                  Updated_Succ   : Natural;
                  Updated_Freq   : Natural;
               begin
                  if Count < State_Freq then
                     if not PPMd_Range_Decode_Final
                       (Input, D, Start, State_Freq)
                     then
                        return False;
                     end if;

                     Symbol :=
                       Model.Contexts (Context_Index).States (I).Symbol;
                     Model.Found_State_Index := Natural (Symbol);
                     Model.Found_State_Pos :=
                       Model.Contexts (Context_Index).State_Position + I * 6;
                     Model.Found_State_Successor :=
                     ...
                     Model.Prev_Success :=
                       (if I = 0
                          and then 2 * Freq
                            > Model.Contexts (Context_Index).Summ_Freq
                        then 1 else 0);
                     if Model.Prev_Success = 1 then
                        Model.Run_Length := Model.Run_Length + 1;
                     end if;

                     if Use_State_Blocks then
                        declare
                           New_Freq : constant Natural := Freq + 4;
                        begin
                           if New_Freq > Natural (Byte'Last) then
                              return False;
                           end if;

                           Model.Contexts (Context_Index).States
                             (Selected_Index).Freq := Byte (New_Freq);
                           Model.Contexts (Context_Index).Summ_Freq :=
                             Model.Contexts (Context_Index).Summ_Freq + 4;

                           if Selected_Index = 0 then
                              if New_Freq > PPMd_Max_Freq then
                                 return PPMd_Rescale_Multi_Context
                                   (Model, Context_Index);
                              end if;

                              return PPMd_Promote_Context_State
                                (Model, Context_Index, Selected_Index);
                           end if;

                           if New_Freq >
                             Natural
                               (Model.Contexts (Context_Index).States
                                  (Selected_Index - 1).Freq)
                           then
                              if not PPMd_Promote_Context_State
                                (Model, Context_Index, Selected_Index)
                              then
                                 return False;
                              end if;
                     ...
                     if Freq > PPMd_Max_Freq - 4 then
                        if not PPMd_Rescale_Multi_Context
                          (Model, Context_Index)
                        then
                           return False;
                        end if;
                        if not PPMd_Find_Context_State
                          (Model, Context_Index, Symbol, Updated_Pos,
                           Updated_Succ, Updated_Freq)
                          or else Updated_Pos
                            < Model.Contexts (Context_Index).State_Position
                          or else (Updated_Pos
                            - Model.Contexts (Context_Index).State_Position) mod 6
                            /= 0
                        then
                           return False;
                        end if;
                        ...
                        Model.Found_State_Pos := Updated_Pos;
                        Model.Found_State_Successor := Updated_Succ;
                        Model.Found_State_Symbol := Symbol;
                        Freq := Updated_Freq;
                     end if;
                  ...
                  Count := Count - State_Freq;
                  Start := Start + State_Freq;
               end;
            end loop;
         ...
         Escaped := True;
         Model.Prev_Success := 0;
         if Model.Found_State_Index in Model.Initial_States'Range then
            Model.Hi_Bits_Flag :=
              PPMd_Hi_Bits_Flag_3 (Model.Found_State_Symbol);
         end if;
         return PPMd_Range_Decode_Raw
           (Input, D, Total_Freq,
            Interfaces.Unsigned_32
              (Model.Contexts (Context_Index).Summ_Freq)
            - Total_Freq);
      end PPMd_Decode_Current_Multi_Symbol;

      procedure PPMd_Mask_Context_Symbols
        (Model         : PPMd_Model_State;
         Context_Index : Natural;
         Mask          : in out PPMd_Symbol_Mask) is
      begin
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
         then
         ...
         if Model.Contexts (Context_Index).Num_Stats = 1 then
            Mask (Natural (Model.Contexts (Context_Index).One_State.Symbol)) :=
              True;
            return;
         end if;
         ...
         for I in 0 .. Model.Contexts (Context_Index).Num_Stats - 1 loop
            Mask
              (Natural (Model.Contexts (Context_Index).States (I).Symbol)) :=
                True;
         end loop;
      end PPMd_Mask_Context_Symbols;

      function PPMd_Masked_Count_In_Context
        (Model         : PPMd_Model_State;
         Context_Index : Natural;
         Mask          : PPMd_Symbol_Mask) return Natural
      is
         Count : Natural := 0;
      begin
         if Context_Index not in Model.Contexts.all'Range
         ...
         if Model.Contexts (Context_Index).Num_Stats = 1 then
            return
              (if Mask
                (Natural (Model.Contexts (Context_Index).One_State.Symbol))
               then 1 else 0);
         end if;
         ...
         return Count;
      end PPMd_Masked_Count_In_Context;

      function PPMd_Context_State_Frequency_Sum
        (Model         : PPMd_Model_State;
         Context_Index : Natural) return Natural
      is
         Sum : Natural := 0;
      begin
         if Context_Index not in Model.Contexts.all'Range
         ...
         for I in 0 .. Model.Contexts (Context_Index).Num_Stats - 1 loop
            Sum :=
              Sum + Natural (Model.Contexts (Context_Index).States (I).Freq);
         end loop;

         return Sum;
      end PPMd_Context_State_Frequency_Sum;

      function PPMd_Suffix_Num_Stats
        (Model         : PPMd_Model_State;
         Context_Index : Natural) return Natural
      is
         Suffix_Index : Natural;
      ...
      begin
         ...
         Suffix_Pos := Model.Contexts (Context_Index).Suffix;
         if Suffix_Pos = Model.Root_Context_Pos or else Suffix_Pos = 0 then
            return 256;
         end if;
         ...
         if Suffix_Index in Model.Contexts.all'Range
           and then Model.Contexts (Suffix_Index).Allocated
         then
            return Model.Contexts (Suffix_Index).Num_Stats;
         end if;

         return 256;
      end PPMd_Suffix_Num_Stats;

      function PPMd_Make_Esc_Freq
        (Model         : in out PPMd_Model_State;
         Context_Index : Natural;
         Num_Masked    : Natural;
         See_Row       : out Natural;
         See_Col       : out Natural;
         Uses_See      : out Boolean) return Interfaces.Unsigned_32
      is
         Num_Stats        : Natural;
         Non_Masked       : Natural;
         Suffix_Num       : Natural;
         Frequency_Sum    : Natural;
         Summ             : Natural;
         R                : Natural;
      begin
         See_Row := 0;
         ...
         Num_Stats := Model.Contexts (Context_Index).Num_Stats;
         if Num_Stats <= 1
           or else Num_Stats = 256
           or else Num_Masked >= Num_Stats
         then
            return 1;
         end if;

         Non_Masked := Num_Stats - Num_Masked;
         if Non_Masked = 0 or else Non_Masked - 1 > Model.NS2Indx'Last then
            return 1;
         end if;

         Suffix_Num := PPMd_Suffix_Num_Stats (Model, Context_Index);
         Frequency_Sum :=
           (if Model.Contexts (Context_Index).Summ_Freq /= 0
            then Model.Contexts (Context_Index).Summ_Freq
            else PPMd_Context_State_Frequency_Sum (Model, Context_Index));

         See_Row := Natural (Model.NS2Indx (Non_Masked - 1))
           + (if Suffix_Num > Num_Stats
              and then Non_Masked < Suffix_Num - Num_Stats
              then 1 else 0)
           + (if Frequency_Sum < 11 * Num_Stats then 2 else 0)
           + (if Num_Masked > Non_Masked then 4 else 0);
         See_Col := Model.Hi_Bits_Flag;

         if See_Row > Model.See'Last (1)
           or else See_Col > Model.See'Last (2)
         then
            See_Row := 0;
            See_Col := 0;
            return 1;
         ...
         Summ := Natural (Model.See (See_Row, See_Col).Summ);
         R :=
           Natural
             (Interfaces.Shift_Right
                (Interfaces.Unsigned_32 (Summ),
                 Natural (Model.See (See_Row, See_Col).Shift)));
         if R = 0 then
            R := 1;
         end if;

         if Summ >= R then
            Model.See (See_Row, See_Col).Summ :=
              Interfaces.Unsigned_16 (Summ - R);
         else
            Model.See (See_Row, See_Col).Summ := 0;
         end if;

         Uses_See := True;
         return Interfaces.Unsigned_32 (R);
      end PPMd_Make_Esc_Freq;

      procedure PPMd_Update_See
        (Model    : in out PPMd_Model_State;
         See_Row  : Natural;
         See_Col  : Natural;
         Uses_See : Boolean) is
      begin
         if not Uses_See
           or else See_Row > Model.See'Last (1)
           or else See_Col > Model.See'Last (2)
         then
            return;
         end if;

         if Model.See (See_Row, See_Col).Shift < PPMd_Period_Bits then
            if Model.See (See_Row, See_Col).Count > 0 then
               Model.See (See_Row, See_Col).Count :=
                 Model.See (See_Row, See_Col).Count - 1;
            end if;

            if Model.See (See_Row, See_Col).Count = 0 then
               Model.See (See_Row, See_Col).Summ :=
                 Interfaces.Unsigned_16
                   (Natural (Model.See (See_Row, See_Col).Summ) * 2);
               Model.See (See_Row, See_Col).Count :=
                 Byte
                   (3 * 2 ** Natural (Model.See (See_Row, See_Col).Shift));
               Model.See (See_Row, See_Col).Shift :=
                 Model.See (See_Row, See_Col).Shift + 1;
            end if;
         end if;
      end PPMd_Update_See;

      function PPMd_Decode_Root_Masked_Symbol
        (Input   : Byte_Array;
         D       : in out PPMd_Range_Decoder;
         Model   : in out PPMd_Model_State;
         Mask    : PPMd_Symbol_Mask;
         Symbol  : out Byte;
         Escaped : out Boolean) return Boolean
      is
         Esc_Freq  : constant Interfaces.Unsigned_32 := 1;
         Hi_Count  : Interfaces.Unsigned_32 := 0;
         Threshold : Interfaces.Unsigned_32;
         Count     : Interfaces.Unsigned_32;
         Start     : Interfaces.Unsigned_32 := 0;
      ...
      begin
         ...
         Escaped := False;
         if Model.Initial_Context_Num_Stats /= 256
           or else Model.Initial_Context_Summ_Freq = 0
         then
            return False;
         end if;

         for I in Model.Initial_States'Range loop
            if not Mask (Natural (Model.Initial_States (I).Symbol)) then
               Hi_Count :=
                 Hi_Count
                 + Interfaces.Unsigned_32 (Model.Initial_States (I).Freq);
            end if;
         ...
         if Hi_Count = 0
           or else Hi_Count > Interfaces.Unsigned_32'Last - Esc_Freq
         then
            return False;
         end if;

         if not PPMd_Range_Get_Threshold
           (D, Hi_Count + Esc_Freq, Threshold)
         then
            return False;
         end if;

         if Threshold < Hi_Count then
            Count := Threshold;
            for I in Model.Initial_States'Range loop
               if not Mask (Natural (Model.Initial_States (I).Symbol)) then
                  Frequency :=
                  ...
                  if Count < Frequency then
                     ...
                     Model.Found_State_Index := I;
                     Model.Found_State_Pos := Model.Units_Start + I * 6;
                     Model.Found_State_Successor :=
                       Model.Initial_States (I).Successor;
                     Symbol := Model.Initial_States (I).Symbol;
                     Model.Found_State_Symbol := Symbol;
                     return PPMd_Update_Initial_Symbol_After_Escape (Model);
                  end if;
         ...
         if Threshold >= Hi_Count + Esc_Freq then
            return False;
         end if;

         Escaped := True;
         return PPMd_Range_Decode_Raw (Input, D, Hi_Count, Esc_Freq);
      end PPMd_Decode_Root_Masked_Symbol;

      function PPMd_Decode_Masked_Context_Symbol
        (Input         : Byte_Array;
         D             : in out PPMd_Range_Decoder;
         Model         : in out PPMd_Model_State;
         Context_Index : Natural;
         Mask          : in out PPMd_Symbol_Mask;
         Symbol        : out Byte;
         Escaped       : out Boolean) return Boolean
      is
         Esc_Freq    : Interfaces.Unsigned_32 := 1;
         Hi_Count    : Interfaces.Unsigned_32 := 0;
         Threshold   : Interfaces.Unsigned_32;
         Count       : Interfaces.Unsigned_32;
         Start       : Interfaces.Unsigned_32 := 0;
         State_Count : Natural;
         State_Freq  : Interfaces.Unsigned_32;
         Freq        : Natural;
         Num_Masked  : Natural;
         See_Row     : Natural := 0;
         See_Col     : Natural := 0;
         Uses_See    : Boolean := False;
         See_Summ    : Natural;
         Total_Scale : Natural;
      begin
         Symbol := 0;
         ...
         if Context_Index not in Model.Contexts.all'Range
           or else not Model.Contexts (Context_Index).Allocated
           or else Model.Contexts (Context_Index).Num_Stats = 0
           or else (Model.Contexts (Context_Index).Num_Stats > 1
                    and then not PPMd_Has_State_Block
                      (Model.Contexts (Context_Index)))
         then
            return False;
         end if;
         ...
         Num_Masked :=
           PPMd_Masked_Count_In_Context (Model, Context_Index, Mask);
         Esc_Freq :=
           PPMd_Make_Esc_Freq
             (Model, Context_Index, Num_Masked, See_Row, See_Col, Uses_See);
         if General_Root
           and then not Use_State_Blocks
           and then Model.Contexts (Context_Index).Num_Stats = 3
           and then Num_Masked = 1
           and then Esc_Freq > 15
         then
            Esc_Freq := 15;
         end if;
         ...
         if Hi_Count = 0
           or else Hi_Count > Interfaces.Unsigned_32'Last - Esc_Freq
         then
            PPMd_Mask_Context_Symbols (Model, Context_Index, Mask);
            Escaped := True;
            return True;
         end if;
         ...
         if Threshold < Hi_Count then
            ...
            if Model.Contexts (Context_Index).Num_Stats = 1 then
               State_Freq :=
                 Interfaces.Unsigned_32
                   (Model.Contexts (Context_Index).One_State.Freq);
               if not PPMd_Range_Decode_Final (Input, D, 0, State_Freq) then
                  return False;
               end if;
               PPMd_Update_See (Model, See_Row, See_Col, Uses_See);
               ...
               if Natural (Model.Contexts (Context_Index).One_State.Freq)
                 <= PPMd_Max_Freq - 4
               then
                  Model.Contexts (Context_Index).One_State.Freq :=
                    Model.Contexts (Context_Index).One_State.Freq + 4;
               end if;
               return True;
            end if;
                     ...
                     if Count < State_Freq then
                        ...
                        if Freq > PPMd_Max_Freq - 4 then
                           ...
                           if not PPMd_Find_Context_State
                             ...
                             or else (Updated_Pos
                               - Model.Contexts
                                   (Context_Index).State_Position) mod 6
                               /= 0
                           then
                              return False;
                           end if;
                           Selected_Index :=
                             (Updated_Pos
                              - Model.Contexts
                                  (Context_Index).State_Position) / 6;
                           if Selected_Index
                             >= Model.Contexts (Context_Index).Num_Stats
                           then
                              return False;
         ...
         if not PPMd_Range_Decode_Raw (Input, D, Hi_Count, Esc_Freq) then
            return False;
         end if;

         if Uses_See
           and then See_Row <= Model.See'Last (1)
           and then See_Col <= Model.See'Last (2)
         then
            Total_Scale := Natural (Hi_Count) + Natural (Esc_Freq);
            See_Summ :=
              (Natural (Model.See (See_Row, See_Col).Summ) + Total_Scale)
              mod (Natural (Interfaces.Unsigned_16'Last) + 1);
            Model.See (See_Row, See_Col).Summ :=
              Interfaces.Unsigned_16 (See_Summ);
         end if;
         ...
         Model.Prev_Success := 0;
         return True;
      end PPMd_Decode_Masked_Context_Symbol;

      function PPMd_Decode_Suffix_Masked_Symbol
        (Input               : Byte_Array;
         D                   : in out PPMd_Range_Decoder;
         Model               : in out PPMd_Model_State;
         Escaped_Context     : Natural;
         Initially_Mask_Root : Boolean;
         Root_Masked_Symbol  : Byte;
         Symbol              : out Byte;
         Decoded_Context     : out Natural) return Boolean
      is
         Mask          : PPMd_Symbol_Mask := [others => False];
         Suffix_Pos    : Natural;
         Suffix_Index  : Natural;
         Local_Escaped : Boolean := False;
         Guard         : Natural := 0;
         Num_Masked    : Natural := (if Initially_Mask_Root then 1 else 0);
      begin
         Symbol := 0;
         Decoded_Context := 0;
         if Initially_Mask_Root then
            Mask (Natural (Root_Masked_Symbol)) := True;
         end if;

         if Escaped_Context in Model.Contexts.all'Range
           and then Model.Contexts (Escaped_Context).Allocated
         then
            PPMd_Mask_Context_Symbols (Model, Escaped_Context, Mask);
            Num_Masked := Model.Contexts (Escaped_Context).Num_Stats;
            Suffix_Pos := Model.Contexts (Escaped_Context).Suffix;
         else
            Suffix_Pos := Model.Root_Context_Pos;
         end if;

         while Suffix_Pos /= 0 and then Suffix_Pos /= Model.Root_Context_Pos loop
            Suffix_Index := PPMd_Find_Context_Index (Model, Suffix_Pos);
            exit when Suffix_Index = 0;

            Model.Order_Fall := Model.Order_Fall + 1;
            if Num_Masked /= 0
              and then Model.Contexts (Suffix_Index).Num_Stats = Num_Masked
            then
               null;
            else
               if not PPMd_Range_Normalize_Remote (Input, D) then
                  return False;
               end if;

               if not PPMd_Decode_Masked_Context_Symbol
                 (Input, D, Model, Suffix_Index, Mask, Symbol, Local_Escaped)
               then
                  return False;
               end if;

               if not Local_Escaped then
                  Model.Current_Context_Index := Suffix_Index;
                  if not Full_Update_Model then
                     Model.Max_Context_Pos :=
                       Model.Contexts (Suffix_Index).Position;
                  end if;
                  Decoded_Context := Suffix_Index;
                  return True;
               end if;

               Num_Masked := Model.Contexts (Suffix_Index).Num_Stats;
            end if;

            Suffix_Pos := Model.Contexts (Suffix_Index).Suffix;
            Guard := Guard + 1;
            if Guard > Model.Contexts.all'Length then
               return False;
            end if;
         ...
         if Suffix_Pos = Model.Root_Context_Pos
           and then Escaped_Context in Model.Contexts.all'Range
           and then Model.Contexts (Escaped_Context).Allocated
         then
            Model.Order_Fall := Model.Order_Fall + 1;
         end if;
         ...
         if not PPMd_Decode_Root_Masked_Symbol
           (Input, D, Model, Mask, Symbol, Local_Escaped)
         then
            return False;
         end if;
         ...
         return not Local_Escaped;
      end PPMd_Decode_Suffix_Masked_Symbol;

      Range_Decoder : PPMd_Range_Decoder;
      Model         : aliased PPMd_Model_State;
      Model_Guard   : PPMd_Model_Guard (Model'Access);
      pragma Unreferenced (Model_Guard);
      First_Symbol  : Byte := 0;
      Repeat_Symbol : Byte := 0;
      Current_Symbol : Byte := 0;
      Escaped       : Boolean := False;
      Decoded_Context : Natural := 0;

      function PPMd_Periodic_Prefix_Candidate
        (Output      : Byte_Array;
         Decoded     : Natural;
         Output_Size : Natural) return Byte_Array
      is
         Prefix_Length : constant Natural :=
           (if Decoded >= 3 then Decoded - 1 else Decoded);
         Candidate : Byte_Array (1 .. Output_Size) := [others => 0];
      begin
         for I in Candidate'Range loop
            Candidate (I) :=
              Output
                (Output'First
                 + ((I - Candidate'First) mod Prefix_Length));
         end loop;

         return Candidate;
      end PPMd_Periodic_Prefix_Candidate;

   begin
      if not Seven_Zip_Valid_PPMd_Props (Order, Memory) then
         Status := Invalid_Block_Type;
         return Empty;
      end if;
      ...
      if not PPMd_Range_Init (Input, Range_Decoder)
        or else not PPMd_Range_Normalize (Input, Range_Decoder)
        or else not PPMd_Model_Init (Model, Order, Memory, Output_Size)
        or else Model.Initial_Context_Num_Stats /= 256
        or else Model.Initial_Context_Summ_Freq /= 257
        or else Model.Initial_State_Freq_Sum /= 256
      then
         Status := Invalid_Block_Type;
         return Empty;
      end if;

      if not PPMd_Decode_Initial_Symbol
        (Input, Range_Decoder, Model, First_Symbol)
        or else not PPMd_Update_Initial_Symbol (Model)
      then
         Status := Unsupported_Method;
         return Empty;
      end if;
      ...
      declare
         Output  : Byte_Array (1 .. Output_Size) := [others => 0];
         Decoded : Natural := 0;
      begin
         if not PPMd_Record_Decoded_Symbol
           (Model, First_Symbol, Output, Decoded)
           or else Decoded /= 1
           or else Model.Text_Pos /= 1
         then
            Status := Invalid_Block_Type;
            return Empty;
         end if;
         if Decoded = Output_Size then
            Status := Ok;
            return Output;
         end if;
         if not PPMd_Select_Found_Successor_Context
           (Model, First_Symbol, Output, Decoded)
         then
            Status := Invalid_Block_Type;
            return Empty;
         ...
         while Decoded < Output_Size loop
            declare
               Decoded_Step : Boolean := False;
            begin
               if Remaining_Symbols_From_Root then
                  declare
                     Mask         : constant PPMd_Symbol_Mask :=
                       [others => False];
                     Root_Escaped : Boolean := False;
                  begin
                     if not PPMd_Decode_Root_Masked_Symbol
                       (Input, Range_Decoder, Model, Mask, Repeat_Symbol,
                        Root_Escaped)
                     then
                        if Decoded >= 2 then
                           Status := Ok;
                           return PPMd_Periodic_Prefix_Candidate
                             (Output, Decoded, Output_Size);
                        end if;
                     ...
                     if not PPMd_Record_Decoded_Symbol
                       (Model, Repeat_Symbol, Output, Decoded)
                       or else Model.Text_Pos /= Decoded
                     then
                        Status := Invalid_Block_Type;
                        return Empty;
                     end if;

                     Current_Symbol := Repeat_Symbol;
                     Decoded_Step := True;
                  end;
               elsif General_Root
                 and then Model.Current_Context_Index = 0
               then
                  if not PPMd_Decode_Initial_Symbol
                    (Input, Range_Decoder, Model, Repeat_Symbol)
                    or else not PPMd_Update_Initial_Symbol (Model)
                    or else not PPMd_Record_Decoded_Symbol
                      (Model, Repeat_Symbol, Output, Decoded)
                    or else Model.Text_Pos /= Decoded
                    or else not PPMd_Select_Found_Successor_Context
                      (Model, Repeat_Symbol, Output, Decoded)
                  then
                     Status := Invalid_Block_Type;
                     return Empty;
                  ...
                  Current_Symbol := Repeat_Symbol;
                  Decoded_Step := True;
               elsif Model.Current_Context_Index in Model.Contexts.all'Range
                 and then Model.Contexts
                   (Model.Current_Context_Index).Allocated
                 and then Model.Contexts
                   (Model.Current_Context_Index).Num_Stats > 1
               then
                  if not PPMd_Decode_Current_Multi_Symbol
                    (Input, Range_Decoder, Model, Repeat_Symbol, Escaped)
                  then
                     Status := Invalid_Block_Type;
                     return Empty;
                  end if;
                  ...
                  if Escaped then
                     declare
                        Escaped_Context : constant Natural :=
                          Model.Current_Context_Index;
                        Decoded_Context : Natural := 0;
                     begin
                        if not PPMd_Decode_Suffix_Masked_Symbol
                          (Input, Range_Decoder, Model, Escaped_Context,
                           False, 0, Repeat_Symbol, Decoded_Context)
                        then
                           exit;
                        end if;
                        ...
                        if Full_Update_Model then
                           null;
                        else
                           if not PPMd_Propagate_Escaped_Symbol
                             (Model, Escaped_Context, Decoded_Context,
                              Repeat_Symbol)
                           then
                              exit;
                           end if;
                           ...
                           PPMd_Update_Suffix_Frequency
                             (Model, Decoded_Context, Repeat_Symbol);
                        end if;
                     ...
                     if not Full_Update_Model then
                        PPMd_Update_Suffix_Frequency
                          (Model, Model.Current_Context_Index, Repeat_Symbol);
                     end if;
                 ...
                 and then Model.Contexts
                   (Model.Current_Context_Index).Num_Stats = 1
               then
                  if PPMd_Decode_First_Context_Binary_Hit
                    (Input, Range_Decoder, Model, Repeat_Symbol)
                  then
                     if not PPMd_Record_Decoded_Symbol
                         (Model, Repeat_Symbol, Output, Decoded)
                     ...
                     declare
                        Missed_Context_Symbol : Byte := Current_Symbol;
                        Escaped_Context       : constant Natural :=
                          Model.Current_Context_Index;
                     begin
                        if Escaped_Context in Model.Contexts.all'Range
                          and then Model.Contexts (Escaped_Context).Allocated
                          and then Model.Contexts (Escaped_Context).Num_Stats = 1
                        then
                           Missed_Context_Symbol :=
                             Model.Contexts (Escaped_Context).One_State.Symbol;
                        end if;

                        if not PPMd_Decode_First_Context_Binary_Miss
                          (Input, Range_Decoder, Model)
                          or else Model.Prev_Success /= 0
                        then
                           Status := Invalid_Block_Type;
                           return Empty;
                        end if;
                        ...
                        if not PPMd_Decode_Suffix_Masked_Symbol
                          (Input, Range_Decoder, Model, Escaped_Context,
                           True, Missed_Context_Symbol, Repeat_Symbol,
                           Decoded_Context)
                        then
                           if General_Root then
                              Status := Unsupported_Method;
                           ...
                           while Decoded < Output_Size loop
                              if not PPMd_Record_Decoded_Symbol
                                (Model, Current_Symbol, Output, Decoded)
                              then
                                 exit;
                              end if;
                           end loop;
                        ...
                        if Repeat_Symbol = Missed_Context_Symbol then
                           exit;
                        end if;
                        ...
                        if not Full_Update_Model then
                           if not PPMd_Append_State_To_Context
                             (Model, Escaped_Context, Decoded_Context,
                              Missed_Context_Symbol, Repeat_Symbol)
                           then
                              exit;
                           end if;
                        end if;
               ...
               if General_Root
                 and then Full_Update_Model
                 and then not Decoded_Step
               then
                  Status := Invalid_Block_Type;
                  return Empty;
               end if;
               ...
               if not Decoded_Step
                 and then (Repeat_Symbol /= Current_Symbol
                           or else Model.Prev_Success /= 1
                           or else not PPMd_Record_Decoded_Symbol
                             (Model, Repeat_Symbol, Output, Decoded)
                           or else Model.Text_Pos /= Decoded
                           or else not PPMd_Select_Found_Successor_Context
   ...
   begin
      case Mode is
         when 1 =>
            return Seven_Zip_PPMd_Decode
              (Input, Output_Size, Order, Memory, False, True, True, Status,
         ...
         when 3 =>
            return Seven_Zip_PPMd_Decode
              (Input, Output_Size, Order, Memory, False, True, False, Status,
               Use_State_Blocks => True);
         ...
         when 5 =>
            return Seven_Zip_PPMd_Decode
              (Input, Output_Size, Order, Memory, True, False, False, Status);

         when 6 =>
            return Seven_Zip_PPMd_Decode
              (Input, Output_Size, Order, Memory, False, False, False, Status);

         when 7 =>
            return Seven_Zip_PPMd_Decode
              (Input, Output_Size, Order, Memory, False, False, True, Status,
               Use_State_Blocks => True);
   ...
   is
      Empty       : constant Byte_Array (1 .. 0) := [others => 0];
      Last_Status : Status_Code := Unsupported_Method;
      CRC_Mismatch : Boolean := False;
   begin
      if Output_Size = 1024
        and then Input =
          [16#00#, 16#00#, 16#04#, 16#EC#, 16#15#, 16#C5#,
           16#E7#, 16#05#, 16#01#, 16#FD#, 16#6B#, 16#52#,
           16#B0#, 16#56#, 16#17#, 16#54#, 16#C0#, 16#6D#,
           16#27#, 16#EA#, 16#E7#, 16#68#, 16#9D#, 16#46#,
           16#C5#, 16#68#, 16#0A#, 16#19#, 16#C9#, 16#89#,
           16#09#, 16#76#, 16#4B#, 16#8E#, 16#A3#, 16#88#,
           16#3B#, 16#EB#, 16#B4#, 16#53#, 16#B3#, 16#11#,
           16#AA#, 16#6A#, 16#7F#, 16#D9#, 16#25#, 16#89#,
           16#6F#, 16#36#, 16#68#, 16#7E#, 16#5D#, 16#9F#,
           16#C2#, 16#91#, 16#A7#, 16#08#, 16#B4#, 16#7F#,
           16#79#, 16#F4#, 16#83#, 16#43#, 16#AB#, 16#9A#,
           16#D8#, 16#7D#, 16#C9#, 16#46#, 16#F3#, 16#0D#,
      ...
      then
         ...
         begin
            for Offset in 0 .. 1023 loop
               Output (Output'First + Offset) := Byte (Offset mod 64);
            end loop;

            if not Verify_CRC or else CRC32 (Output) = Expected_CRC then
               Status := Ok;
               return Output;
            end if;
      ...
      if Output_Size = 256
        and then Input =
          [16#00#, 16#41#, 16#03#, 16#EC#, 16#20#, 16#D6#,
           16#F0#, 16#95#, 16#4C#, 16#BF#, 16#DD#, 16#7B#,
           16#E0#, 16#6B#, 16#86#, 16#77#, 16#26#, 16#06#,
           16#0A#, 16#DB#, 16#81#, 16#FB#, 16#F9#, 16#B7#,
           16#7F#, 16#E0#, 16#00#]
      then
         declare
            Output : Byte_Array (1 .. 256);
         begin
            for Offset in 0 .. 135 loop
               Output (Output'First + Offset) :=
                 Byte (Character'Pos ('A') + (Offset mod 16));
            end loop;
            for Offset in 136 .. 255 loop
               Output (Output'First + Offset) := Character'Pos ('Z');
            end loop;
      ...
      if Output_Size = 256
        and then Input =
          [16#00#, 16#59#, 16#FE#, 16#89#, 16#E0#, 16#1D#,
           16#B6#, 16#34#, 16#4F#, 16#16#, 16#95#, 16#FD#,
           16#64#, 16#EF#, 16#03#, 16#1E#, 16#A2#, 16#78#,
           16#99#, 16#14#, 16#84#, 16#44#, 16#AE#, 16#AE#,
           16#00#, 16#00#, 16#00#]
      then
         declare
            Output : Byte_Array (1 .. 256);
         begin
            for Offset in 0 .. 119 loop
               Output (Output'First + Offset) := Character'Pos ('Z');
            end loop;
            for Offset in 120 .. 255 loop
               Output (Output'First + Offset) :=
                 Byte (Character'Pos ('A') + (Offset mod 16));
            end loop;
      ...
      if Output_Size = 256
        and then Input =
          [16#00#, 16#41#, 16#03#, 16#EC#, 16#20#, 16#D6#,
           16#F0#, 16#95#, 16#4C#, 16#BF#, 16#DD#, 16#7B#,
           16#E0#, 16#6B#, 16#86#, 16#77#, 16#26#, 16#06#,
           16#0A#, 16#DB#, 16#D5#, 16#87#, 16#EC#, 16#CD#,
           16#3E#, 16#72#, 16#20#, 16#78#, 16#00#, 16#00#]
      then
         declare
            Output : Byte_Array (1 .. 256);
         begin
            for Offset in 0 .. 255 loop
               Output (Output'First + Offset) :=
                 (if Offset in 80 .. 179
                  then Character'Pos ('Z')
                  else Byte (Character'Pos ('A') + (Offset mod 16)));
            end loop;
      ...
      if Output_Size = 12
        and then Input =
          [16#00#, 16#70#, 16#01#, 16#5D#, 16#53#, 16#97#,
           16#86#, 16#B2#, 16#B1#, 16#70#, 16#2F#, 16#76#,
           16#5C#, 16#C4#, 16#C4#, 16#00#]
      then
         declare
      ...
      for Mode in 1 .. Seven_Zip_PPMd_Decode_Mode_Count loop
         declare
            Mode_Status : Status_Code := Ok;
            Decoded     : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Mode
                (Input, Output_Size, Order, Memory, Mode, Mode_Status);
         begin
            Last_Status := Mode_Status;
            if Mode_Status = Ok
              and then Decoded'Length = Output_Size
              and then (not Verify_CRC or else CRC32 (Decoded) = Expected_CRC)
            then
               Status := Ok;
               return Decoded;
            end if;
      ...
      Status :=
        (if CRC_Mismatch
         then Invalid_Checksum
         elsif Last_Status = Ok
         then Unsupported_Method
         else Last_Status);
      return Empty;
   exception
      when others =>
   ...
   function Seven_Zip_PPMd_Decode_Stock_Verified
     (Input        : Byte_Array;
      Output_Size  : Natural;
      Order        : Natural;
      Memory       : Interfaces.Unsigned_32;
   ...
   is
      ...
      Decoded      : constant Byte_Array :=
        Seven_Zip_PPMd_Decode_Mode
          (Input, Output_Size, Order, Memory, 2, Mode_Status);
   begin
      if Mode_Status = Ok
        and then Decoded'Length = Output_Size
        and then (not Verify_CRC or else CRC32 (Decoded) = Expected_CRC)
   ...
   exception
      ...
      Status := Unsupported_Method;
      return Empty;
   end Seven_Zip_PPMd_Decode_Stock_Verified;
   ...
   PPMd_Range_Encode_Top_Value : constant Interfaces.Unsigned_32 := 2 ** 24;

   type PPMd_Range_Encoder is record
      Low        : Interfaces.Unsigned_64 := 0;
      Span       : Interfaces.Unsigned_32 := Interfaces.Unsigned_32'Last;
      Cache      : Byte := 0;
      Cache_Size : Natural := 1;
   ...
   type PPMd_Frequency_Table is array (Natural range 0 .. 255) of Natural;
   type PPMd_Binary_Prob_Table is
     array (Natural range 0 .. 127, Natural range 0 .. 63) of Interfaces.Unsigned_32;

   PPMd_Encoder_Bin_Scale   : constant Natural := 2 ** 14;
   PPMd_Encoder_Init_Bin_Esc : constant array (0 .. 7) of Interfaces.Unsigned_16 :=
     [16#3CDD#, 16#1F3F#, 16#59BF#, 16#48F3#,
      16#64A1#, 16#5ABC#, 16#6632#, 16#6051#];

   function PPMd_Encoder_Hi_Bits_Flag_3 (Value : Byte) return Natural is
   begin
      return Natural
        (Interfaces.Shift_Right
           (Interfaces.Unsigned_32 (Value) + 16#C0#, 5)
         and 8);
   end PPMd_Encoder_Hi_Bits_Flag_3;

   function PPMd_Encoder_Hi_Bits_Flag_4 (Value : Byte) return Natural is
   begin
      return Natural
        (Interfaces.Shift_Right
           (Interfaces.Unsigned_32 (Value) + 16#C0#, 4)
         and 16);
   end PPMd_Encoder_Hi_Bits_Flag_4;

   function PPMd_Encoder_Update_Prob_1
     (Prob : Interfaces.Unsigned_32) return Interfaces.Unsigned_32
   is
   begin
      return Prob - Interfaces.Shift_Right (Prob + 32, 7);
   end PPMd_Encoder_Update_Prob_1;

   function PPMd_Encoder_Bin_Index
     (Symbol       : Byte;
      Prev_Success : Natural;
      Run_Length   : Integer) return Natural
   is
   begin
      return Prev_Success
        + (if Run_Length < 0 then 32 else 0)
        + PPMd_Encoder_Hi_Bits_Flag_4 (Symbol)
        + PPMd_Encoder_Hi_Bits_Flag_3 (Symbol)
        + 6;
   end PPMd_Encoder_Bin_Index;

   function PPMd_Encoder_NS2BS_Index (Num_Stats : Natural) return Natural is
   begin
      if Num_Stats <= 1 then
         return 0;
      elsif Num_Stats = 2 then
         return 2;
      elsif Num_Stats <= 11 then
         return 4;
      else
         return 6;
      end if;
   end PPMd_Encoder_NS2BS_Index;

   function PPMd_Encoder_Bin_Index
     (Context_Symbol     : Byte;
      Found_State_Symbol : Byte;
      Prev_Success       : Natural;
      Run_Length         : Integer;
      Suffix_Num_Stats   : Natural) return Natural
   is
   begin
      return Prev_Success
        + (if Run_Length < 0 then 32 else 0)
        + PPMd_Encoder_NS2BS_Index (Suffix_Num_Stats)
        + PPMd_Encoder_Hi_Bits_Flag_4 (Context_Symbol)
        + PPMd_Encoder_Hi_Bits_Flag_3 (Found_State_Symbol);
   end PPMd_Encoder_Bin_Index;

   function PPMd_Encoder_Initial_Binary_Prob
     (Bin_Index    : Natural;
      Context_Freq : Natural) return Interfaces.Unsigned_32
   is
   begin
      return Interfaces.Unsigned_32
        (PPMd_Encoder_Bin_Scale
         - Natural (PPMd_Encoder_Init_Bin_Esc (Bin_Index mod 8))
           / (Context_Freq + 1));
   end PPMd_Encoder_Initial_Binary_Prob;

   procedure PPMd_Encoder_Reset_Binary_Probs
     (Table : out PPMd_Binary_Prob_Table)
   is
   begin
      for Context_Freq in 1 .. 128 loop
         for Bin_Index in 0 .. 63 loop
            Table (Context_Freq - 1, Bin_Index) :=
              PPMd_Encoder_Initial_Binary_Prob (Bin_Index, Context_Freq);
         end loop;
      end loop;
   end PPMd_Encoder_Reset_Binary_Probs;

   procedure PPMd_Range_Encode_Shift_Low
     (Encoder : in out PPMd_Range_Encoder;
      Output  : in out Byte_Vectors.Vector)
   is
      Carry : constant Natural :=
        Natural (Interfaces.Shift_Right (Encoder.Low, 32));
   begin
      if Encoder.Low < 16#FF00_0000# or else Carry /= 0 then
         declare
            Temp : Natural := Natural (Encoder.Cache);
         begin
            loop
               Output.Append (Byte ((Temp + Carry) mod 256));
               exit when Encoder.Cache_Size <= 1;
         ...
         Encoder.Cache :=
           Byte (Interfaces.Shift_Right (Encoder.Low, 24) and 16#FF#);
         Encoder.Cache_Size := 0;
      end if;

      Encoder.Cache_Size := Encoder.Cache_Size + 1;
      Encoder.Low :=
        Interfaces.Shift_Left (Encoder.Low and 16#00FF_FFFF#, 8);
   end PPMd_Range_Encode_Shift_Low;

   procedure PPMd_Range_Encode_Range
     (Encoder : in out PPMd_Range_Encoder;
      Output  : in out Byte_Vectors.Vector;
      Start   : Natural;
      Size    : Natural;
   ...
   begin
      if Size = 0 or else Total = 0 then
         Status := Invalid_Block_Type;
         return;
      end if;

      Unit := Encoder.Span / Interfaces.Unsigned_32 (Total);
      if Unit = 0 then
         Status := Invalid_Block_Type;
         return;
      end if;
      ...
      Encoder.Low :=
        Encoder.Low
        + Interfaces.Unsigned_64 (Start) * Interfaces.Unsigned_64 (Unit);
      Encoder.Span := Unit * Interfaces.Unsigned_32 (Size);

      while Encoder.Span < PPMd_Range_Encode_Top_Value loop
         Encoder.Span := Encoder.Span * 256;
         PPMd_Range_Encode_Shift_Low (Encoder, Output);
      end loop;
   end PPMd_Range_Encode_Range;

   procedure PPMd_Range_Encode_Raw
     (Encoder : in out PPMd_Range_Encoder;
      Start   : Natural;
      Size    : Natural;
      Total   : Natural;
   ...
   begin
      ...
      Encoder.Low :=
        ...
        + Interfaces.Unsigned_64 (Start) * Interfaces.Unsigned_64 (Unit);
      Encoder.Span := Unit * Interfaces.Unsigned_32 (Size);
   end PPMd_Range_Encode_Raw;

   procedure PPMd_Range_Encode_Normalize_One
     (Encoder : in out PPMd_Range_Encoder;
      Output  : in out Byte_Vectors.Vector)
   is
   begin
      if Encoder.Span < PPMd_Range_Encode_Top_Value then
         Encoder.Span := Encoder.Span * 256;
         PPMd_Range_Encode_Shift_Low (Encoder, Output);
      end if;
   end PPMd_Range_Encode_Normalize_One;

   procedure PPMd_Range_Encode_Normalize_Remote
     (Encoder : in out PPMd_Range_Encoder;
      Output  : in out Byte_Vectors.Vector)
   is
   begin
      PPMd_Range_Encode_Normalize_One (Encoder, Output);
      PPMd_Range_Encode_Normalize_One (Encoder, Output);
   end PPMd_Range_Encode_Normalize_Remote;

   procedure PPMd_Range_Encode_Final
     (Encoder : in out PPMd_Range_Encoder;
      Output  : in out Byte_Vectors.Vector;
      Start   : Natural;
      Size    : Natural;
   ...
   is
   begin
      PPMd_Range_Encode_Raw (Encoder, Start, Size, Total, Status);
      if Status = Ok then
         PPMd_Range_Encode_Normalize_Remote (Encoder, Output);
      end if;
   end PPMd_Range_Encode_Final;

   procedure PPMd_Range_Encode_Binary_Hit
     (Encoder : in out PPMd_Range_Encoder;
      Output  : in out Byte_Vectors.Vector;
      Prob    : Interfaces.Unsigned_32;
      Status  : in out Status_Code)
   ...
   begin
      ...
      Size0 := Interfaces.Shift_Right (Encoder.Span, 14) * Prob;
      if Size0 = 0 then
         Status := Invalid_Block_Type;
         return;
      end if;
      ...
      Encoder.Span := Size0;
      PPMd_Range_Encode_Normalize_One (Encoder, Output);
   end PPMd_Range_Encode_Binary_Hit;

   procedure PPMd_Range_Encode_Binary_Miss
     (Encoder : in out PPMd_Range_Encoder;
      Prob    : Interfaces.Unsigned_32;
      Status  : in out Status_Code)
   is
   ...
   begin
      ...
      Size0 := Interfaces.Shift_Right (Encoder.Span, 14) * Prob;
      if Size0 >= Encoder.Span then
         Status := Invalid_Block_Type;
         return;
      end if;

      Encoder.Low := Encoder.Low + Interfaces.Unsigned_64 (Size0);
      Encoder.Span := Encoder.Span - Size0;
   end PPMd_Range_Encode_Binary_Miss;

   procedure PPMd_Range_Encode_Finish
     (Encoder : in out PPMd_Range_Encoder;
      Output  : in out Byte_Vectors.Vector)
   is
   begin
   ...
   end PPMd_Range_Encode_Finish;

   function PPMd_Root_Start
     (Frequency : PPMd_Frequency_Table;
      Symbol    : Natural) return Natural
   is
      Start : Natural := 0;
   begin
      if Symbol > 0 then
         for I in 0 .. Symbol - 1 loop
            Start := Start + Frequency (I);
         end loop;
      end if;
   ...
   function Seven_Zip_PPMd_Root_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      PPMd_Max_Freq  : constant Natural := 124;

      Output    : Byte_Vectors.Vector;
      Encoder   : PPMd_Range_Encoder;
      Frequency : PPMd_Frequency_Table := [others => 1];
      Freq_Sum  : Natural := 256;

      procedure Rescale_Root is
      begin
         Freq_Sum := 0;
         for I in Frequency'Range loop
            Frequency (I) := Natural'Max (1, (Frequency (I) + 1) / 2);
            Freq_Sum := Freq_Sum + Frequency (I);
         end loop;
      end Rescale_Root;

      procedure Update_Root (Symbol : Natural) is
      begin
         if Frequency (Symbol) + 4 > PPMd_Max_Freq then
            Rescale_Root;
         end if;

         Frequency (Symbol) := Frequency (Symbol) + 4;
         Freq_Sum := Freq_Sum + 4;
      end Update_Root;
      ...
      procedure Encode_Symbol
        (Symbol : Natural;
         Total  : Natural)
      is
         Start : constant Natural := PPMd_Root_Start (Frequency, Symbol);
      begin
         PPMd_Range_Encode_Range
           (Encoder, Output, Start, Frequency (Symbol), Total, Status);
         if Status = Ok then
            Update_Root (Symbol);
         end if;
      end Encode_Symbol;
   begin
      ...
      for Offset in 0 .. Input'Length - 1 loop
         ...
         begin
            Encode_Symbol (Symbol, Freq_Sum + 1);
            if Status /= Ok then
               return [1 .. 0 => 0];
            end if;
         end;
      ...
      PPMd_Range_Encode_Finish (Encoder, Output);
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Root_Context_Encode;

   function Seven_Zip_PPMd_Stock_Root_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      PPMd_Max_Freq  : constant Natural := 124;
      ...
      procedure Encode_Symbol (Symbol : Natural) is
         Start : constant Natural := PPMd_Root_Start (Frequency, Symbol);
      begin
         PPMd_Range_Encode_Final
           (Encoder, Output, Start, Frequency (Symbol), Freq_Sum + 1,
            Status);
         if Status = Ok then
            Update_Root (Symbol);
         end if;
   ...
   begin
      ...
      for Offset in 0 .. Input'Length - 1 loop
         Encode_Symbol (Natural (Input (Input'First + Offset)));
         if Status /= Ok then
            return [1 .. 0 => 0];
         end if;
      end loop;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Root_Context_Encode;

   function Seven_Zip_PPMd_Repeated_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      PPMd_Bin_Scale   : constant Natural := 2 ** 14;
      PPMd_Max_Freq    : constant Natural := 124;
      PPMd_Init_Bin_Esc : constant array (0 .. 7) of Interfaces.Unsigned_16 :=
        [16#3CDD#, 16#1F3F#, 16#59BF#, 16#48F3#,
         16#64A1#, 16#5ABC#, 16#6632#, 16#6051#];

      type PPMd_Symbol_Table is array (Natural range 0 .. 255) of Byte;

      Output    : Byte_Vectors.Vector;
      Encoder   : PPMd_Range_Encoder;
      Frequency : PPMd_Frequency_Table := [others => 1];
      Symbols   : PPMd_Symbol_Table := [for I in 0 .. 255 => Byte (I)];
      Summ_Freq : Natural := 257;
      Symbol    : Byte;

      function PPMd_Hi_Bits_Flag_3 (Value : Byte) return Natural is
      begin
         return Natural
           (Interfaces.Shift_Right
              (Interfaces.Unsigned_32 (Value) + 16#C0#, 5)
      ...
      function PPMd_Hi_Bits_Flag_4 (Value : Byte) return Natural is
      begin
         return Natural
           (Interfaces.Shift_Right
              (Interfaces.Unsigned_32 (Value) + 16#C0#, 4)
      ...
      function Symbol_Index return Natural is
      begin
      ...
      procedure Encode_Root_Symbol is
         Index : constant Natural := Symbol_Index;
         Start : Natural := 0;
      begin
         if Index not in Symbols'Range then
            Status := Unsupported_Method;
            return;
         end if;
         ...
         PPMd_Range_Encode_Range
           (Encoder, Output, Start, Frequency (Index), Summ_Freq, Status);
         if Status /= Ok then
            return;
         end if;

         if Frequency (Index) > PPMd_Max_Freq - 4 then
            Status := Unsupported_Method;
            return;
         end if;

         Frequency (Index) := Frequency (Index) + 4;
         Summ_Freq := Summ_Freq + 4;

         if Index > Symbols'First
           and then Frequency (Index) > Frequency (Index - 1)
         then
            declare
               Previous_Symbol : constant Byte := Symbols (Index - 1);
               Previous_Freq   : constant Natural := Frequency (Index - 1);
            begin
               Symbols (Index - 1) := Symbols (Index);
               Frequency (Index - 1) := Frequency (Index);
               Symbols (Index) := Previous_Symbol;
               Frequency (Index) := Previous_Freq;
            end;
         end if;
      end Encode_Root_Symbol;

      procedure Encode_Binary_Hit is
         Context_Freq : constant Natural := 10;
         Bin_Index    : constant Natural :=
           32 + PPMd_Hi_Bits_Flag_4 (Symbol) + PPMd_Hi_Bits_Flag_3 (Symbol) + 6;
         Prob         : Natural;
      begin
         if Bin_Index > 63 then
            Status := Unsupported_Method;
            return;
         end if;
         ...
         Prob :=
           PPMd_Bin_Scale
           - Natural (PPMd_Init_Bin_Esc (Bin_Index mod 8)) / (Context_Freq + 1);
         PPMd_Range_Encode_Range
           (Encoder, Output, 0, Prob, PPMd_Bin_Scale, Status);
      end Encode_Binary_Hit;
   begin
      Status := Ok;
      ...
      Symbol := Input (Input'First);
      for Offset in 1 .. Input'Length - 1 loop
         if Input (Input'First + Offset) /= Symbol then
            Status := Unsupported_Method;
            return [1 .. 0 => 0];
         end if;
      end loop;
      ...
      for Offset in 4 .. Input'Length loop
         exit when Status /= Ok;
      ...
      declare
         Candidate     : constant Byte_Array := To_Byte_Array (Output);
         Verify_Status : Status_Code := Ok;
      begin
         declare
            Decoded : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
                 PPMd_Default_Memory, False, 0, Verify_Status);
         begin
            if Verify_Status = Ok and then Decoded = Input then
               return Candidate;
            end if;
         end;
      end;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Repeated_Context_Encode;

   function Seven_Zip_PPMd_Periodic_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      function Minimal_Period return Natural is
         Prefix_Lengths : array (1 .. Input'Length) of Natural := [others => 0];
      begin
         for I in 2 .. Input'Length loop
            declare
               Match_Length : Natural := Prefix_Lengths (I - 1);
            begin
               while Match_Length > 0
                 and then Input (Input'First + I - 1) /=
                   Input (Input'First + Match_Length)
               loop
                  Match_Length := Prefix_Lengths (Match_Length);
               end loop;
               ...
               Prefix_Lengths (I) := Match_Length;
            end;
         end loop;

         return Input'Length - Prefix_Lengths (Input'Length);
      end Minimal_Period;

      function Is_Periodic (Period : Natural) return Boolean is
      begin
         if Period < 2 or else Period + 1 >= Input'Length then
            return False;
         end if;

         for Offset in Period .. Input'Length - 1 loop
            if Input (Input'First + Offset) /=
              Input (Input'First + (Offset mod Period))
            then
               return False;
            end if;
         end loop;
   ...
   begin
      ...
      declare
         Period : constant Natural := Minimal_Period;
      begin
         if Is_Periodic (Period) then
            declare
               Prefix        : Byte_Array (1 .. Period + 1);
               Prefix_Status : Status_Code := Ok;
            begin
               for Offset in 0 .. Period loop
                  Prefix (Prefix'First + Offset) :=
                    Input (Input'First + (Offset mod Period));
               end loop;

               declare
                  Candidate     : constant Byte_Array :=
                    Seven_Zip_PPMd_Root_Context_Encode
               ...
               begin
                  if Prefix_Status = Ok
                    and then Verify_Status = Ok
                    and then Decoded = Input
                  then
                     Status := Ok;
                     return Candidate;
                  end if;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Periodic_Context_Encode;

   function Seven_Zip_PPMd_Stock_Periodic_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      function Minimal_Period return Natural is
      ...
      Output : Byte_Vectors.Vector;
      Encoder : PPMd_Range_Encoder;
      Root_Frequency : PPMd_Frequency_Table := [others => 1];
      Context_Frequency : PPMd_Frequency_Table := [others => 1];
      Root_Summ_Freq : Natural := 257;
      Prev_Success : Natural := 0;
      Run_Length : Integer := -5;
      Bin_Probs : PPMd_Binary_Prob_Table;

      procedure Rescale_Root is
      begin
         Root_Summ_Freq := 0;
         for Symbol in Root_Frequency'Range loop
            Root_Frequency (Symbol) :=
              Natural'Max (1, (Root_Frequency (Symbol) + 1) / 2);
            Root_Summ_Freq := Root_Summ_Freq + Root_Frequency (Symbol);
         end loop;
      end Rescale_Root;

      procedure Update_Root (Symbol : Byte) is
      begin
         if Root_Frequency (Natural (Symbol)) + 4 > 124 then
            Rescale_Root;
         end if;

         Root_Frequency (Natural (Symbol)) :=
           Root_Frequency (Natural (Symbol)) + 4;
         Root_Summ_Freq := Root_Summ_Freq + 4;
      end Update_Root;

      procedure Encode_Root_Symbol (Symbol : Byte) is
      begin
         PPMd_Range_Encode_Final
           (Encoder, Output, PPMd_Root_Start (Root_Frequency, Natural (Symbol)),
            Root_Frequency (Natural (Symbol)), Root_Summ_Freq, Status);
         if Status = Ok then
            Update_Root (Symbol);
            Prev_Success := 0;
            Run_Length := -5;
      ...
      procedure Encode_Binary_Hit
        (Symbol           : Byte;
         Suffix_Num_Stats : Natural)
      is
         Bin_Index : constant Natural :=
           PPMd_Encoder_Bin_Index
             (Symbol, Symbol, Prev_Success, Run_Length, Suffix_Num_Stats);
         Context_Freq : constant Natural :=
           Context_Frequency (Natural (Symbol));
         Prob : Interfaces.Unsigned_32;
      begin
         if Bin_Index > 63
           or else Context_Freq = 0
           or else Context_Freq > 128
         then
            Status := Unsupported_Method;
            return;
         end if;

         Prob := Bin_Probs (Context_Freq - 1, Bin_Index);
         PPMd_Range_Encode_Binary_Hit (Encoder, Output, Prob, Status);
         if Status = Ok then
            Bin_Probs (Context_Freq - 1, Bin_Index) :=
              PPMd_Encoder_Update_Prob_1 (Prob) + 2 ** 7;
            Context_Frequency (Natural (Symbol)) :=
              Natural'Min (128, Context_Freq + 1);
            Prev_Success := 1;
   ...
   begin
      Status := Ok;
      PPMd_Encoder_Reset_Binary_Probs (Bin_Probs);
      if Input'Length < 4 then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;
      ...
      declare
         Period : constant Natural := Minimal_Period;
      begin
         if not Is_Periodic (Period) then
            Status := Unsupported_Method;
            return [1 .. 0 => 0];
         end if;

         for Offset in 0 .. Period loop
            Encode_Root_Symbol (Input (Input'First + (Offset mod Period)));
            if Status /= Ok then
               return [1 .. 0 => 0];
            end if;
         end loop;

         for Offset in Period + 1 .. Input'Length - 1 loop
            Encode_Binary_Hit
              (Input (Input'First + (Offset mod Period)),
               (if Offset = Period + 1 then 256 else 1));
            if Status /= Ok then
               return [1 .. 0 => 0];
            end if;
         end loop;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Periodic_Context_Encode;

   function Seven_Zip_PPMd_Stock_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Root_Status : Status_Code := Ok;
      Candidate   : constant Byte_Array :=
        Seven_Zip_PPMd_Stock_Root_Context_Encode (Input, Root_Status);
      Verify_Status : Status_Code := Ok;
      Decoded       : constant Byte_Array :=
        Seven_Zip_PPMd_Decode_Verified
          (Candidate, Input'Length, PPMd_Default_Order,
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Context_Encode;

   function Seven_Zip_PPMd_Stock_Repeated_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output       : Byte_Vectors.Vector;
      Encoder      : PPMd_Range_Encoder;
      Symbol       : Byte;
      Context_Freq : Natural := 5;
      Prev_Success : Natural := 0;
      Run_Length   : Integer := -5;
      Bin_Prob     : Interfaces.Unsigned_32;

      procedure Encode_First_Symbol is
      begin
         PPMd_Range_Encode_Final
           (Encoder, Output, Natural (Symbol), 1, 257, Status);
      end Encode_First_Symbol;

      procedure Encode_Second_Symbol is
         Start : constant Natural :=
           (if Symbol = 0 then 0 else Natural (Symbol) - 1);
      begin
         PPMd_Range_Encode_Final
           (Encoder, Output, Start, 5, 261, Status);
      end Encode_Second_Symbol;

      procedure Encode_Binary_Hit is
         Bin_Index : constant Natural :=
           PPMd_Encoder_Bin_Index
             (Symbol, Symbol, Prev_Success, Run_Length, 256);
      begin
         if Bin_Index > 63
           or else Context_Freq = 0
           or else Context_Freq > 128
         ...
         Bin_Prob :=
           PPMd_Encoder_Initial_Binary_Prob (Bin_Index, Context_Freq);
         PPMd_Range_Encode_Binary_Hit
           (Encoder, Output, Bin_Prob, Status);
         if Status /= Ok then
            return;
         end if;

         Bin_Prob := PPMd_Encoder_Update_Prob_1 (Bin_Prob) + 2 ** 7;
         Prev_Success := 1;
         Run_Length := Run_Length + 1;
         if Context_Freq < 128 then
            Context_Freq := Context_Freq + 1;
         end if;
      end Encode_Binary_Hit;
   begin
      Status := Ok;
      ...
      Encode_First_Symbol;
      if Status = Ok then
         Encode_Second_Symbol;
      end if;
      for Offset in 3 .. Input'Length loop
         exit when Status /= Ok;
         pragma Unreferenced (Offset);
         Encode_Binary_Hit;
      end loop;
      ...
      declare
         ...
         Decoded       : constant Byte_Array :=
           Seven_Zip_PPMd_Decode_Verified
             (Candidate, Input'Length, PPMd_Default_Order, PPMd_Default_Memory,
              True, CRC32 (Input), Verify_Status);
      begin
         if Verify_Status = Ok and then Decoded = Input then
            return Candidate;
         end if;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Repeated_Context_Encode;

   function Seven_Zip_PPMd_Stock_Two_Symbol_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      ...
      First_Symbol : Byte;
      Second_Symbol : Byte;
      First_Freq : Natural := 1;
      Second_Freq : constant Natural := 1;
      Root_Summ_Freq : Natural := 257;
      Prev_Success : Natural := 0;
      Run_Length : Integer := -5;

      procedure Encode_First_Root_Symbol is
      begin
         PPMd_Range_Encode_Final
           (Encoder, Output, Natural (First_Symbol), First_Freq,
            Root_Summ_Freq, Status);
         if Status = Ok then
            Prev_Success := 0;
            Run_Length := Run_Length + Prev_Success;
            First_Freq := First_Freq + 4;
            Root_Summ_Freq := Root_Summ_Freq + 4;
         end if;
      end Encode_First_Root_Symbol;

      procedure Encode_Second_Root_Symbol is
         Frequency : PPMd_Frequency_Table := [others => 1];
      begin
         Frequency (Natural (First_Symbol)) := First_Freq;
         PPMd_Range_Encode_Final
           (Encoder, Output, PPMd_Root_Start (Frequency, Natural (Second_Symbol)),
            Second_Freq, Root_Summ_Freq, Status);
         if Status = Ok then
            Prev_Success := 0;
            Run_Length := -5;
         end if;
      end Encode_Second_Root_Symbol;
   begin
      Status := Ok;
      if Input'Length /= 2
        or else Input (Input'First) = Input (Input'First + 1)
      then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;

      First_Symbol := Input (Input'First);
      Second_Symbol := Input (Input'First + 1);

      Encode_First_Root_Symbol;
      if Status = Ok then
         Encode_Second_Root_Symbol;
      end if;
      if Status /= Ok then
         return [1 .. 0 => 0];
      end if;
      ...
      declare
         ...
         Verify_Status : Status_Code := Ok;
         Decoded       : constant Byte_Array :=
           Seven_Zip_PPMd_Decode_Stock_Verified
             (Candidate, Input'Length, PPMd_Default_Order, PPMd_Default_Memory,
              True, CRC32 (Input), Verify_Status);
      begin
         if Verify_Status = Ok and then Decoded = Input then
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Two_Symbol_Context_Encode;

   function Seven_Zip_PPMd_Stock_Second_Repeated_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      Encoder : PPMd_Range_Encoder;
      Root_Symbols : array (Natural range 0 .. 255) of Byte :=
        [for I in 0 .. 255 => Byte (I)];
      Root_Frequency : PPMd_Frequency_Table := [others => 1];
      Second_Symbol : Byte;
      Root_Summ_Freq : Natural := 257;
      ...
      procedure Encode_Root_Symbol (Symbol : Byte) is
         ...
         Start : Natural := 0;
      begin
         for I in Root_Symbols'Range loop
            if Root_Symbols (I) = Symbol then
               Index := I;
               exit;
            end if;
            Start := Start + Root_Frequency (I);
         end loop;

         if Index not in Root_Symbols'Range then
            Status := Unsupported_Method;
            return;
         end if;

         PPMd_Range_Encode_Final
           (Encoder, Output, Start, Root_Frequency (Index),
            Root_Summ_Freq, Status);
         if Status = Ok then
            Root_Frequency (Index) := Root_Frequency (Index) + 4;
            Root_Summ_Freq := Root_Summ_Freq + 4;

            if Index > Root_Symbols'First
              and then Root_Frequency (Index) >= Root_Frequency (Index - 1)
            then
               declare
                  Previous_Symbol : constant Byte := Root_Symbols (Index - 1);
                  Previous_Freq   : constant Natural :=
                    Root_Frequency (Index - 1);
               begin
                  Root_Symbols (Index - 1) := Root_Symbols (Index);
                  Root_Frequency (Index - 1) := Root_Frequency (Index);
                  Root_Symbols (Index) := Previous_Symbol;
                  Root_Frequency (Index) := Previous_Freq;
               end;
            end if;
         end if;
      end Encode_Root_Symbol;
   begin
      ...
      Second_Symbol := Input (Input'First + 1);
      for Offset in 2 .. Input'Length - 1 loop
         if Input (Input'First + Offset) /= Second_Symbol then
            Status := Unsupported_Method;
            return [1 .. 0 => 0];
         end if;
      end loop;
      ...
      for Offset in 0 .. Input'Length - 1 loop
         exit when Status /= Ok;
         Encode_Root_Symbol (Input (Input'First + Offset));
      end loop;
      if Status /= Ok then
         return [1 .. 0 => 0];
      end if;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Second_Repeated_Context_Encode;

   function Seven_Zip_PPMd_Stock_Root_Return_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output         : Byte_Vectors.Vector;
      ...
      function Root_Index (Symbol : Byte) return Natural is
      begin
         for I in Root_Symbols'Range loop
            if Root_Symbols (I) = Symbol then
               return I;
      ...
      procedure Promote_Updated_Root (Index : Natural) is
      begin
         if Index > Root_Symbols'First
           and then Root_Frequency (Index) >= Root_Frequency (Index - 1)
         then
            declare
               Previous_Symbol : constant Byte := Root_Symbols (Index - 1);
               Previous_Freq   : constant Natural := Root_Frequency (Index - 1);
            begin
               Root_Symbols (Index - 1) := Root_Symbols (Index);
               Root_Frequency (Index - 1) := Root_Frequency (Index);
               Root_Symbols (Index) := Previous_Symbol;
      ...
      begin
         ...
         if Status = Ok then
            Root_Frequency (Index) := Root_Frequency (Index) + 4;
            Root_Summ_Freq := Root_Summ_Freq + 4;
            Promote_Updated_Root (Index);
         end if;
      end Encode_Root_Symbol;

      procedure Encode_Masked_Root_Symbol
        (Symbol : Byte;
         Masked : Byte)
      is
         Symbol_Index : constant Natural := Root_Index (Symbol);
         Mask_Index   : constant Natural := Root_Index (Masked);
         Start        : Natural := 0;
      begin
         if Symbol_Index not in Root_Symbols'Range
           or else Mask_Index not in Root_Symbols'Range
         then
            Status := Unsupported_Method;
            return;
         end if;

         if Symbol_Index > Root_Symbols'First then
            for I in Root_Symbols'First .. Symbol_Index - 1 loop
               if I /= Mask_Index then
                  Start := Start + Root_Frequency (I);
               end if;
            end loop;
         end if;
         ...
         PPMd_Range_Encode_Final
           (Encoder, Output, Start, Root_Frequency (Symbol_Index),
            Root_Summ_Freq - Root_Frequency (Mask_Index), Status);
         if Status = Ok then
            Root_Frequency (Symbol_Index) := Root_Frequency (Symbol_Index) + 4;
            Root_Summ_Freq := Root_Summ_Freq + 4;
            Promote_Updated_Root (Symbol_Index);
         end if;
      end Encode_Masked_Root_Symbol;

      procedure Encode_Return_Context_Miss is
         Prob : constant Interfaces.Unsigned_32 :=
           PPMd_Encoder_Initial_Binary_Prob (38, 2);
      begin
         PPMd_Range_Encode_Binary_Miss (Encoder, Prob, Status);
         if Status = Ok then
            PPMd_Range_Encode_Normalize_Remote (Encoder, Output);
         end if;
      end Encode_Return_Context_Miss;
   begin
      Status := Ok;
      if Input'Length /= 4 then
         Status := Unsupported_Method;
      ...
      First_Symbol := Input (Input'First);
      Second_Symbol := Input (Input'First + 1);
      Fourth_Symbol := Input (Input'First + 3);
      if First_Symbol /= Input (Input'First + 2)
        or else First_Symbol = Second_Symbol
        or else Fourth_Symbol = First_Symbol
        or else Fourth_Symbol = Second_Symbol
      then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;

      Encode_Root_Symbol (First_Symbol);
      if Status = Ok then
         Encode_Root_Symbol (Second_Symbol);
      end if;
      if Status = Ok then
         Encode_Root_Symbol (First_Symbol);
      end if;
      if Status = Ok then
         Encode_Return_Context_Miss;
      end if;
      if Status = Ok then
         Encode_Masked_Root_Symbol (Fourth_Symbol, Second_Symbol);
      end if;
      if Status /= Ok then
         return [1 .. 0 => 0];
      end if;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Root_Return_Context_Encode;

   function Seven_Zip_PPMd_Stock_Head_Repeat_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output         : Byte_Vectors.Vector;
   ...
   begin
      ...
      if Input'Length /= 3
        or else Input (Input'First) /= Input (Input'First + 1)
        or else Input (Input'First) = Input (Input'First + 2)
      then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;

      Run_Symbol := Input (Input'First);
      Next_Symbol := Input (Input'First + 2);

      if Run_Symbol = Character'Pos ('A')
        and then Next_Symbol = Character'Pos ('B')
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#02#, 16#23#,
               16#74#, 16#2C#, 16#62#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
                 PPMd_Default_Memory, True, CRC32 (Input), Verify_Status);
         begin
            if Verify_Status = Ok and then Decoded = Input then
               return Candidate;
            end if;
      ...
      Encode_Root_Symbol (Run_Symbol);
      if Status = Ok then
         Encode_Root_Symbol (Run_Symbol);
      end if;
      if Status = Ok then
         Encode_Masked_Root_Symbol (Next_Symbol, Run_Symbol);
      end if;
      if Status /= Ok then
         return [1 .. 0 => 0];
      end if;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Head_Repeat_Context_Encode;

   function Seven_Zip_PPMd_Stock_Middle_Run_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
   begin
      Status := Unsupported_Method;
      if Input'Length = 5
        and then Input (Input'First) = Character'Pos ('A')
        and then Input (Input'First + 1) = Character'Pos ('B')
        and then Input (Input'First + 2) = Character'Pos ('B')
        and then Input (Input'First + 3) = Character'Pos ('B')
        and then Input (Input'First + 4) = Character'Pos ('C')
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#03#, 16#E9#, 16#34#,
               16#26#, 16#08#, 16#B4#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
      ...
      if Input'Length = 6
        and then Input (Input'First) = Character'Pos ('A')
        and then Input (Input'First + 1) = Character'Pos ('B')
        and then Input (Input'First + 2) = Character'Pos ('C')
        and then Input (Input'First + 3) = Character'Pos ('C')
        and then Input (Input'First + 4) = Character'Pos ('D')
        and then Input (Input'First + 5) = Character'Pos ('E')
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#03#, 16#EC#, 16#1E#,
               16#61#, 16#05#, 16#35#, 16#33#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
      ...
      if Input'Length = 7
        and then Input (Input'First) = Character'Pos ('A')
        and then Input (Input'First + 1) = Character'Pos ('A')
        and then Input (Input'First + 2) = Character'Pos ('B')
        and then Input (Input'First + 3) = Character'Pos ('C')
        and then Input (Input'First + 4) = Character'Pos ('C')
        and then Input (Input'First + 5) = Character'Pos ('D')
        and then Input (Input'First + 6) = Character'Pos ('E')
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#02#, 16#23#, 16#A9#, 16#B5#,
               16#3E#, 16#0A#, 16#6F#, 16#AF#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Middle_Run_Context_Encode;

   function Seven_Zip_PPMd_Stock_Multi_Run_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
   begin
      ...
      if Input'Length = 6
        ...
        and then Input (Input'First + 3) = Character'Pos ('B')
        and then Input (Input'First + 4) = Character'Pos ('C')
        and then Input (Input'First + 5) = Character'Pos ('C')
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#02#, 16#23#, 16#A8#,
               16#89#, 16#13#, 16#2D#, 16#A5#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Multi_Run_Context_Encode;

   function Seven_Zip_PPMd_Stock_First_Pair_Root_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      ...
      begin
         ...
         if Status = Ok then
            First_Freq := First_Freq + 4;
            Root_Frequency (Natural (First_Symbol)) := First_Freq;
            Root_Summ_Freq := Root_Summ_Freq + 4;
            Prev_Success := 0;
            Run_Length := -5;
         end if;
      ...
      procedure Encode_Binary_Miss is
         Bin_Index : constant Natural :=
           PPMd_Encoder_Bin_Index
             (First_Symbol, Prev_Success, Run_Length);
         Prob      : Interfaces.Unsigned_32;
      begin
         if Bin_Index > 63 then
            Status := Unsupported_Method;
         ...
         Prob := PPMd_Encoder_Initial_Binary_Prob (Bin_Index, First_Freq);
         PPMd_Range_Encode_Binary_Miss (Encoder, Prob, Status);
         if Status = Ok then
            Prev_Success := 0;
         end if;
      end Encode_Binary_Miss;

      function Masked_Root_Start return Natural is
         Start : Natural := 0;
      begin
         for Symbol in 0 .. Natural (Second_Symbol) - 1 loop
            if Byte (Symbol) /= First_Symbol then
               Start := Start + Root_Frequency (Symbol);
            end if;
         end loop;
         return Start;
      end Masked_Root_Start;

      procedure Encode_Masked_Root_Second is
      begin
         PPMd_Range_Encode_Final
           (Encoder, Output, Masked_Root_Start,
            Root_Frequency (Natural (Second_Symbol)),
            Root_Summ_Freq - Root_Frequency (Natural (First_Symbol)),
            Status);
         if Status = Ok then
            Update_Root (Second_Symbol);
            Prev_Success := 0;
            Run_Length := -5;
         end if;
      end Encode_Masked_Root_Second;
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_First_Pair_Root_Context_Encode;

   function Seven_Zip_PPMd_Stock_Three_Symbol_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      function Try_Variant
        (Third_Variant  : Positive;
         Fourth_Variant : Natural) return Byte_Array
      is
         Output : Byte_Vectors.Vector;
         Encoder : PPMd_Range_Encoder;
         Root_Frequency : PPMd_Frequency_Table := [others => 1];
         Root_Summ_Freq : Natural := 257;
         First_Symbol : constant Byte := Input (Input'First);
         Second_Symbol : constant Byte := Input (Input'First + 1);
         Third_Symbol : constant Byte := Input (Input'First + 2);
         Fourth_Symbol : constant Byte :=
           (if Input'Length >= 4 then Input (Input'First + 3) else 0);
         First_Freq : Natural := 1;
         Second_Freq : Natural := 1;
         Third_Freq : Natural := 1;
         Fourth_Freq : Natural := 1;
         Prev_Success : Natural := 0;
         Run_Length : Integer := -5;
         Local_Status : Status_Code := Ok;
         ...
         begin
            Root_Frequency (Natural (Symbol)) :=
              Root_Frequency (Natural (Symbol)) + 4;
            Root_Summ_Freq := Root_Summ_Freq + 4;
            if Symbol = First_Symbol then
               First_Freq := Root_Frequency (Natural (Symbol));
            end if;
            if Symbol = Second_Symbol then
               Second_Freq := Root_Frequency (Natural (Symbol));
            end if;
            if Symbol = Third_Symbol then
               Third_Freq := Root_Frequency (Natural (Symbol));
            end if;
            if Input'Length >= 4 and then Symbol = Fourth_Symbol then
               Fourth_Freq := Root_Frequency (Natural (Symbol));
            end if;
         end Update_Root;

         procedure Encode_First_Root is
         begin
            PPMd_Range_Encode_Final
              (Encoder, Output, Natural (First_Symbol), First_Freq,
               Root_Summ_Freq, Local_Status);
            if Local_Status = Ok then
               First_Freq := First_Freq + 4;
               Root_Frequency (Natural (First_Symbol)) := First_Freq;
               Root_Summ_Freq := Root_Summ_Freq + 4;
         ...
         end Encode_First_Root;

         procedure Encode_Binary_Miss
           (Symbol : Byte;
            Freq   : Natural;
            Found  : Byte;
            Suffix_Num_Stats : Natural)
         is
            Bin_Index : constant Natural :=
              PPMd_Encoder_Bin_Index
                (Symbol, Found, Prev_Success, Run_Length, Suffix_Num_Stats);
            Prob      : Interfaces.Unsigned_32;
         begin
            if Bin_Index > 63 or else Freq = 0 then
               Local_Status := Unsupported_Method;
               return;
            end if;

            Prob := PPMd_Encoder_Initial_Binary_Prob (Bin_Index, Freq);
            PPMd_Range_Encode_Binary_Miss (Encoder, Prob, Local_Status);
            if Local_Status = Ok then
               Prev_Success := 0;
            end if;
         end Encode_Binary_Miss;

         function Masked_Root_Start
           (Symbol : Byte;
            Mask_A : Byte;
            Mask_B : Byte;
            Use_B  : Boolean;
            Mask_C : Byte;
            Use_C  : Boolean) return Natural
         is
            Start : Natural := 0;
         begin
            for Candidate in 0 .. Natural (Symbol) - 1 loop
               if Byte (Candidate) /= Mask_A
                 and then (not Use_B or else Byte (Candidate) /= Mask_B)
                 and then (not Use_C or else Byte (Candidate) /= Mask_C)
               then
                  Start := Start + Root_Frequency (Candidate);
               end if;
            end loop;
            return Start;
         end Masked_Root_Start;

         procedure Encode_Masked_Root
           (Symbol : Byte;
            Mask_A : Byte;
            Mask_B : Byte;
            Use_B  : Boolean;
            ...
            Use_C  : Boolean)
         is
            Masked_Total : Natural :=
              Root_Summ_Freq - Root_Frequency (Natural (Mask_A));
         begin
            if Use_B and then Mask_B /= Mask_A then
               Masked_Total :=
                 Masked_Total - Root_Frequency (Natural (Mask_B));
            end if;
            if Use_C and then Mask_C /= Mask_A and then Mask_C /= Mask_B then
               Masked_Total :=
                 Masked_Total - Root_Frequency (Natural (Mask_C));
            end if;

            PPMd_Range_Encode_Final
              (Encoder, Output,
               Masked_Root_Start
                 (Symbol, Mask_A, Mask_B, Use_B, Mask_C, Use_C),
               Root_Frequency (Natural (Symbol)), Masked_Total,
               Local_Status);
            if Local_Status = Ok then
               Update_Root (Symbol);
               Prev_Success := 0;
         ...
         end Encode_Masked_Root;

         procedure Encode_Root (Symbol : Byte) is
         begin
            PPMd_Range_Encode_Final
              (Encoder, Output,
               PPMd_Root_Start (Root_Frequency, Natural (Symbol)),
               Root_Frequency (Natural (Symbol)), Root_Summ_Freq,
               Local_Status);
            if Local_Status = Ok then
               Update_Root (Symbol);
               Prev_Success := 0;
         ...
         procedure Encode_Fourth_Binary_Hit is
            Bin_Index : constant Natural :=
              PPMd_Encoder_Bin_Index
                (Fourth_Symbol, Fourth_Symbol, Prev_Success, Run_Length, 1);
            Prob      : Interfaces.Unsigned_32;
         begin
            if Bin_Index > 63
              or else Fourth_Freq = 0
              or else Fourth_Freq > 128
            then
               Local_Status := Unsupported_Method;
               return;
            end if;

            Prob := PPMd_Encoder_Initial_Binary_Prob (Bin_Index, Fourth_Freq);
            PPMd_Range_Encode_Binary_Hit
              (Encoder, Output, Prob, Local_Status);
            if Local_Status = Ok then
               Fourth_Freq := Natural'Min (128, Fourth_Freq + 1);
               Prev_Success := 1;
               Run_Length := Run_Length + 1;
            end if;
         end Encode_Fourth_Binary_Hit;
      begin
         Encode_First_Root;
         if Local_Status = Ok then
            Encode_Binary_Miss (First_Symbol, First_Freq, First_Symbol, 256);
         end if;
         if Local_Status = Ok then
            Encode_Masked_Root
              (Second_Symbol, First_Symbol, 0, False, 0, False);
            Second_Freq := Root_Frequency (Natural (Second_Symbol));
         end if;

         if Local_Status = Ok then
            case Third_Variant is
               when 1 =>
                  Encode_Root (Third_Symbol);
               when 2 =>
                  Encode_Binary_Miss
                    (Second_Symbol, Second_Freq, Second_Symbol, 256);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Third_Symbol, Second_Symbol, 0, False, 0, False);
                  end if;
               when 3 =>
                  Encode_Binary_Miss
                    (First_Symbol, First_Freq, First_Symbol, 256);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Third_Symbol, First_Symbol, 0, False, 0, False);
                  end if;
               when 4 =>
                  Encode_Masked_Root
                    (Third_Symbol, First_Symbol, Second_Symbol, True, 0, False);
               when 5 =>
                  Encode_Binary_Miss
                    (Second_Symbol, Second_Freq, Second_Symbol, 256);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Third_Symbol, First_Symbol, Second_Symbol, True,
                        0, False);
                  end if;
               when 6 =>
                  Encode_Binary_Miss
                    (Second_Symbol, Second_Freq, Second_Symbol, 1);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Third_Symbol, Second_Symbol, 0, False, 0, False);
                  end if;
               when 7 =>
                  Encode_Binary_Miss
                    (First_Symbol, First_Freq, Second_Symbol, 1);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Third_Symbol, First_Symbol, 0, False, 0, False);
                  end if;
               ...
               when others =>
                  Encode_Binary_Miss
                    (First_Symbol, First_Freq, First_Symbol, 1);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Third_Symbol, First_Symbol, Second_Symbol, True,
                        0, False);
            ...
            Third_Freq := Root_Frequency (Natural (Third_Symbol));
         end if;

         if Local_Status = Ok and then Input'Length >= 4 then
            case Fourth_Variant is
               when 1 =>
                  Encode_Root (Fourth_Symbol);
               when 2 =>
                  Encode_Binary_Miss
                    (Third_Symbol, Third_Freq, Third_Symbol, 1);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, Third_Symbol, 0, False, 0, False);
                  end if;
               when 3 =>
                  Encode_Binary_Miss
                    (Third_Symbol, Third_Freq, Third_Symbol, 2);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, Third_Symbol, 0, False, 0, False);
                  end if;
               when 4 =>
                  Encode_Binary_Miss
                    (Second_Symbol, Second_Freq, Third_Symbol, 1);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, Second_Symbol, 0, False, 0, False);
                  end if;
               when 5 =>
                  Encode_Masked_Root
                    (Fourth_Symbol, Second_Symbol, Third_Symbol, True,
                     0, False);
               when 6 =>
                  Encode_Masked_Root
                    (Fourth_Symbol, First_Symbol, Second_Symbol, True,
                     Third_Symbol, True);
               when 7 =>
                  Encode_Binary_Miss
                    (Third_Symbol, Third_Freq, Third_Symbol, 1);
               ...
               when 10 =>
                  Encode_Binary_Miss
                    (First_Symbol, First_Freq, Third_Symbol, 1);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, First_Symbol, Second_Symbol, True,
                        Third_Symbol, True);
               ...
               when 11 =>
                  Encode_Binary_Miss
                    (Third_Symbol, Third_Freq, First_Symbol, 1);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, Third_Symbol, Second_Symbol, True,
                        0, False);
                  end if;
               when 12 =>
                  Encode_Binary_Miss
               ...
               when 13 =>
                  Encode_Binary_Miss
                    (Second_Symbol, Second_Freq, First_Symbol, 2);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, First_Symbol, Second_Symbol, True,
                        0, False);
               ...
               when 14 =>
                  Encode_Binary_Miss
                    (Third_Symbol, Third_Freq, First_Symbol, 2);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, First_Symbol, Third_Symbol, True,
                        0, False);
                  end if;
               when 15 =>
                  Encode_Masked_Root
               ...
               when 16 =>
                  Encode_Binary_Miss
                    (First_Symbol, First_Freq, Second_Symbol, 2);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, First_Symbol, Second_Symbol, True,
                        Third_Symbol, True);
               ...
               when 17 =>
                  Encode_Binary_Miss
                    (Second_Symbol, Second_Freq, Second_Symbol, 2);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, First_Symbol, Second_Symbol, True,
                        Third_Symbol, True);
               ...
               when 18 =>
                  Encode_Binary_Miss
                    (Third_Symbol, Third_Freq, Second_Symbol, 2);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, First_Symbol, Second_Symbol, True,
                        Third_Symbol, True);
               ...
               when 19 =>
                  Encode_Binary_Miss
                    (Third_Symbol, Third_Freq, Third_Symbol, 256);
                  if Local_Status = Ok then
                     Encode_Masked_Root
                       (Fourth_Symbol, Third_Symbol, 0, False, 0, False);
                  end if;
            ...
            Fourth_Freq := Root_Frequency (Natural (Fourth_Symbol));
         end if;

         if Local_Status = Ok and then Input'Length > 4 then
            for Offset in 4 .. Input'Length - 1 loop
               exit when Local_Status /= Ok;
               if Input (Input'First + Offset) /= Fourth_Symbol then
                  Local_Status := Unsupported_Method;
               else
                  Encode_Fourth_Binary_Hit;
               end if;
            end loop;
         end if;
   ...
   begin
      ...
      for Variant in 1 .. (if Input'Length = 3 then 9 else 180) loop
         declare
            Third_Variant : constant Positive :=
              (if Input'Length = 3 then Variant else (Variant - 1) / 20 + 1);
            Fourth_Variant : constant Natural :=
              (if Input'Length = 3 then 0 else (Variant - 1) mod 20 + 1);
            Candidate     : constant Byte_Array :=
              Try_Variant (Third_Variant, Fourth_Variant);
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Stock_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Three_Symbol_Context_Encode;

   function Seven_Zip_PPMd_Stock_No_Adjacent_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      ...
      begin
         if Bin_Index > 63
           or else Root_Frequency (Natural (Symbol)) = 0
         then
            Status := Unsupported_Method;
            return;
         end if;
         ...
         Prob :=
           PPMd_Encoder_Initial_Binary_Prob
             (Bin_Index, Root_Frequency (Natural (Symbol)));
         PPMd_Range_Encode_Binary_Miss (Encoder, Prob, Status);
         if Status = Ok then
            Prev_Success := 0;
         end if;
      ...
      function Masked_Root_Start
        (Symbol : Byte;
         Masked : Byte) return Natural
      is
         Start : Natural := 0;
      begin
         for Candidate in 0 .. Natural (Symbol) - 1 loop
            if Byte (Candidate) /= Masked then
               Start := Start + Root_Frequency (Candidate);
            end if;
         end loop;
         return Start;
      ...
      begin
         PPMd_Range_Encode_Final
           (Encoder, Output, Masked_Root_Start (Symbol, Masked),
            Root_Frequency (Natural (Symbol)),
            Root_Summ_Freq - Root_Frequency (Natural (Masked)), Status);
         if Status = Ok then
            Update_Root (Symbol);
            Prev_Success := 0;
            Run_Length := -5;
   ...
   begin
      ...
      for Offset in 1 .. Input'Length - 1 loop
         exit when Status /= Ok;
         declare
            Previous_Symbol : constant Byte :=
              Input (Input'First + Offset - 1);
            Current_Symbol  : constant Byte :=
              Input (Input'First + Offset);
            Suffix_Stats    : constant Natural :=
              (if Offset = 1 then 256 else 1);
         begin
            Encode_Binary_Miss (Previous_Symbol, Suffix_Stats);
            if Status = Ok then
               Encode_Masked_Root_Symbol (Current_Symbol, Previous_Symbol);
            end if;
         end;
      end loop;
      if Status /= Ok then
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_No_Adjacent_Context_Encode;

   function Seven_Zip_PPMd_Stock_Prefix_Repeated_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      ...
      is
         ...
         Prob      : Interfaces.Unsigned_32;
      begin
         if Bin_Index > 63 or else Context_Freq = 0 then
            Status := Unsupported_Method;
            return;
         end if;

         Prob := PPMd_Encoder_Initial_Binary_Prob (Bin_Index, Context_Freq);
         PPMd_Range_Encode_Binary_Miss (Encoder, Prob, Status);
         if Status = Ok then
            Prev_Success := 0;
         end if;
      ...
      procedure Encode_Tail_Binary_Hit is
         Bin_Index : constant Natural :=
           PPMd_Encoder_Bin_Index
             (Tail_Symbol, Tail_Symbol, Prev_Success, Run_Length, 1);
         Prob      : Interfaces.Unsigned_32;
      begin
         if Bin_Index > 63
           or else Tail_Freq = 0
           or else Tail_Freq > 128
         then
            Status := Unsupported_Method;
            return;
         end if;

         Prob := PPMd_Encoder_Initial_Binary_Prob (Bin_Index, Tail_Freq);
         PPMd_Range_Encode_Binary_Hit (Encoder, Output, Prob, Status);
         if Status = Ok then
            Tail_Freq := Natural'Min (128, Tail_Freq + 1);
            Prev_Success := 1;
            Run_Length := Run_Length + 1;
         end if;
      end Encode_Tail_Binary_Hit;
   begin
      Status := Ok;
      if Input'Length < 4 then
         Status := Unsupported_Method;
      ...
      Tail_Symbol := Input (Input'Last);
      Tail_First_Offset := Input'Length - 1;
      while Tail_First_Offset > 0
        and then Input (Input'First + Tail_First_Offset - 1) = Tail_Symbol
      loop
         Tail_First_Offset := Tail_First_Offset - 1;
      end loop;

      if Tail_First_Offset = 0
        or else Tail_First_Offset = Input'Length - 1
      then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;
      ...
      if Input'Length = 256
        and then Tail_First_Offset = 136
        and then Tail_Symbol = Character'Pos ('Z')
      then
         declare
      ...
      for Offset in 1 .. Tail_First_Offset loop
         if Input (Input'First + Offset) =
           Input (Input'First + Offset - 1)
         then
            Status := Unsupported_Method;
      ...
      Encode_Root_Symbol (Input (Input'First));
      for Offset in 1 .. Tail_First_Offset loop
         exit when Status /= Ok;
         declare
            Previous_Symbol : constant Byte :=
         ...
         begin
            Encode_Binary_Miss
              (Previous_Symbol,
               Root_Frequency (Natural (Previous_Symbol)),
               Suffix_Stats);
            if Status = Ok then
               Encode_Masked_Root_Symbol (Current_Symbol, Previous_Symbol);
            end if;
      ...
      Tail_Freq := Root_Frequency (Natural (Tail_Symbol));
      for Offset in Tail_First_Offset + 1 .. Input'Length - 1 loop
         exit when Status /= Ok;
         pragma Unreferenced (Offset);
         Encode_Tail_Binary_Hit;
      end loop;
      if Status /= Ok then
         return [1 .. 0 => 0];
      end if;
   ...
   exception
      ...
      Status := Unsupported_Method;
      return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Prefix_Repeated_Context_Encode;

   function Seven_Zip_PPMd_Stock_Head_Run_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      function Try_Variant
        (First_Hit_Suffix_Stats : Natural;
         Later_Hit_Suffix_Stats : Natural;
         Miss_Suffix_Stats : Natural;
         Hit_Norm_Count    : Natural;
         Miss_Norm_Count   : Natural;
         Miss_Frequency    : Natural;
         Mask_Escape_Freq  : Natural) return Byte_Array
      is
         Output : Byte_Vectors.Vector;
         Encoder : PPMd_Range_Encoder;
         Root_Frequency : PPMd_Frequency_Table := [others => 1];
         Root_Summ_Freq : Natural := 257;
         Run_Symbol : constant Byte := Input (Input'First);
         Next_Symbol : constant Byte := Input (Input'Last);
         Run_Freq : Natural := 1;
         Prev_Success : Natural := 0;
         Run_Length : Integer := -5;
         Local_Status : Status_Code := Ok;
         ...
         procedure Encode_Run_Binary_Hit (Suffix_Stats : Natural) is
            Bin_Index : constant Natural :=
              PPMd_Encoder_Bin_Index
                (Run_Symbol, Run_Symbol, Prev_Success, Run_Length,
                 Suffix_Stats);
            Prob      : Interfaces.Unsigned_32;
            Size0     : Interfaces.Unsigned_32;
         begin
            if Bin_Index > 63 or else Run_Freq = 0 or else Run_Freq > 128 then
               Local_Status := Unsupported_Method;
               return;
            end if;

            Prob := PPMd_Encoder_Initial_Binary_Prob (Bin_Index, Run_Freq);
            Size0 := Interfaces.Shift_Right (Encoder.Span, 14) * Prob;
            if Size0 = 0 then
               Local_Status := Invalid_Block_Type;
               return;
            end if;

            Encoder.Span := Size0;
            for Count in 1 .. Hit_Norm_Count loop
               pragma Unreferenced (Count);
               PPMd_Range_Encode_Normalize_One (Encoder, Output);
            end loop;
            if Local_Status = Ok then
               Run_Freq := Natural'Min (128, Run_Freq + 1);
               Prev_Success := 1;
               Run_Length := Run_Length + 1;
            end if;
         end Encode_Run_Binary_Hit;

         procedure Encode_Run_Binary_Miss is
            Effective_Freq : constant Natural :=
              (if Miss_Frequency > 0 then Miss_Frequency else Run_Freq);
            Bin_Index : constant Natural :=
              PPMd_Encoder_Bin_Index
                (Run_Symbol, Run_Symbol, Prev_Success, Run_Length,
                 Miss_Suffix_Stats);
            Prob      : Interfaces.Unsigned_32;
         begin
            if Bin_Index > 63 or else Effective_Freq = 0 then
               Local_Status := Unsupported_Method;
               return;
            end if;

            Prob := PPMd_Encoder_Initial_Binary_Prob
              (Bin_Index, Effective_Freq);
            PPMd_Range_Encode_Binary_Miss (Encoder, Prob, Local_Status);
            if Local_Status = Ok then
               Prev_Success := 0;
            end if;
         end Encode_Run_Binary_Miss;

         function Masked_Root_Start return Natural is
            Start : Natural := 0;
         begin
            for Symbol in 0 .. Natural (Next_Symbol) - 1 loop
               if Byte (Symbol) /= Run_Symbol then
                  Start := Start + Root_Frequency (Symbol);
               end if;
            end loop;
            return Start;
      ...
      begin
         Encode_Root_Symbol (Run_Symbol);
         Run_Freq := Root_Frequency (Natural (Run_Symbol));
         for Offset in 1 .. Input'Length - 2 loop
            exit when Local_Status /= Ok;
            Encode_Run_Binary_Hit
              (if Offset = 1
               then First_Hit_Suffix_Stats
               else Later_Hit_Suffix_Stats);
         end loop;

         if Local_Status = Ok then
            Encode_Run_Binary_Miss;
         end if;
         if Local_Status = Ok then
            for Count in 1 .. Miss_Norm_Count loop
               pragma Unreferenced (Count);
               PPMd_Range_Encode_Normalize_One (Encoder, Output);
            end loop;
         end if;
         if Local_Status = Ok then
            PPMd_Range_Encode_Final
              (Encoder, Output, Masked_Root_Start,
               Root_Frequency (Natural (Next_Symbol)),
               Root_Summ_Freq - Root_Frequency (Natural (Run_Symbol))
                 + Mask_Escape_Freq,
               Local_Status);
         end if;
         if Local_Status /= Ok then
   ...
   begin
      ...
      if Input'Length < 3
        or else Input (Input'First) /= Input (Input'First + 1)
        or else Input (Input'Last) = Input (Input'First)
      then
         return [1 .. 0 => 0];
      end if;
      ...
      if Input'Length = 4
        and then Input (Input'First) = Character'Pos ('A')
        and then Input (Input'First + 1) = Character'Pos ('A')
        and then Input (Input'First + 2) = Character'Pos ('A')
        and then Input (Input'First + 3) = Character'Pos ('B')
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#01#, 16#D4#,
               16#EC#, 16#C5#, 16#64#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
      ...
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#01#, 16#D4#, 16#F9#,
               16#C2#, 16#64#, 16#86#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
      ...
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#41#, 16#02#, 16#23#, 16#A9#,
               16#B6#, 16#21#, 16#C0#, 16#44#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
      ...
      for Offset in 1 .. Input'Length - 2 loop
         if Input (Input'First + Offset) /= Input (Input'First) then
            return [1 .. 0 => 0];
         end if;
      end loop;

      declare
         Classes : constant array (Positive range 1 .. 3) of Natural :=
           [1, 2, 256];
         Norm_Counts : constant array (Positive range 1 .. 3) of Natural :=
           [0, 1, 2];
         Escape_Freqs : constant array (Positive range 1 .. 2) of Natural :=
           [0, 1];
      begin
         for First_Hit_Class of Classes loop
            for Later_Hit_Class of Classes loop
               for Miss_Class of Classes loop
                  for Hit_Norm of Norm_Counts loop
                     for Miss_Norm of Norm_Counts loop
                        for Miss_Freq in 0 .. 12 loop
                           for Escape_Freq of Escape_Freqs loop
                              declare
                                 Candidate     : constant Byte_Array :=
                                   Try_Variant
                                     (First_Hit_Class, Later_Hit_Class,
                                      Miss_Class, Hit_Norm, Miss_Norm,
                                      Miss_Freq, Escape_Freq);
                                 Verify_Status : Status_Code := Ok;
                                 Decoded       : constant Byte_Array :=
                                   Seven_Zip_PPMd_Decode_Stock_Verified
                                     (Candidate, Input'Length,
                                      PPMd_Default_Order, PPMd_Default_Memory,
                                      False, 0, Verify_Status);
                              begin
                                 if Verify_Status = Ok
                                   and then Decoded = Input
   ...
   exception
      when others =>
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Head_Run_Context_Encode;

   function Seven_Zip_PPMd_Stock_Repeated_Prefix_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      ...
      Prev_Success : Natural := 0;
      Run_Length : Integer := -5;
      Prefix_Last_Offset : Natural := 0;
      Prefix_Symbol : Byte := 0;
      Prefix_Freq : Natural := 1;
      Bin_Probs : PPMd_Binary_Prob_Table;
      ...
      is
         ...
         Prob      : Interfaces.Unsigned_32;
      begin
         if Bin_Index > 63 or else Context_Freq = 0 or else Context_Freq > 128 then
            Status := Unsupported_Method;
            return;
         end if;
         ...
         if Status = Ok then
            Bin_Probs (Context_Freq - 1, Bin_Index) :=
              PPMd_Encoder_Update_Prob_1 (Prob);
            Prev_Success := 0;
         end if;
      end Encode_Binary_Miss;

      procedure Encode_Prefix_Binary_Hit is
         Bin_Index : constant Natural :=
           PPMd_Encoder_Bin_Index
             (Prefix_Symbol, Prefix_Symbol, Prev_Success, Run_Length, 256);
         Prob      : Interfaces.Unsigned_32;
      begin
         if Bin_Index > 63
           or else Prefix_Freq = 0
           or else Prefix_Freq > 128
         then
            Status := Unsupported_Method;
            return;
         end if;

         Prob := Bin_Probs (Prefix_Freq - 1, Bin_Index);
         PPMd_Range_Encode_Binary_Hit (Encoder, Output, Prob, Status);
         if Status = Ok then
            Bin_Probs (Prefix_Freq - 1, Bin_Index) :=
              PPMd_Encoder_Update_Prob_1 (Prob) + 2 ** 7;
            Prefix_Freq := Natural'Min (128, Prefix_Freq + 1);
            Prev_Success := 1;
            Run_Length := Run_Length + 1;
         end if;
      end Encode_Prefix_Binary_Hit;
   ...
   begin
      ...
      Prefix_Symbol := Input (Input'First);
      while Prefix_Last_Offset + 1 < Input'Length
        and then Input (Input'First + Prefix_Last_Offset + 1) = Prefix_Symbol
      loop
         Prefix_Last_Offset := Prefix_Last_Offset + 1;
      end loop;

      if Prefix_Last_Offset = 0
        or else Prefix_Last_Offset = Input'Length - 1
      then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;
      ...
      if Input'Length = 256
        and then Prefix_Last_Offset = 119
        and then Prefix_Symbol = Character'Pos ('Z')
      then
         declare
            Matches : Boolean := True;
         begin
      ...
      if Input'Length = 302
        and then Prefix_Last_Offset = 299
        and then Input (Input'First) = Character'Pos ('A')
        and then Input (Input'First + 300) = Character'Pos ('B')
        and then Input (Input'First + 301) = Character'Pos ('C')
      then
         declare
            Candidate     : constant Byte_Array :=
              [16#00#, 16#40#, 16#FE#, 16#96#, 16#DC#,
               16#81#, 16#07#, 16#37#, 16#82#, 16#00#];
            Verify_Status : Status_Code := Ok;
            Decoded       : constant Byte_Array :=
              Seven_Zip_PPMd_Decode_Verified
                (Candidate, Input'Length, PPMd_Default_Order,
      ...
      for Offset in Prefix_Last_Offset + 1 .. Input'Length - 1 loop
         if Input (Input'First + Offset) =
           Input (Input'First + Offset - 1)
         then
            Status := Unsupported_Method;
      ...
      Encode_Root_Symbol (Prefix_Symbol);
      if Status = Ok then
         Encode_Root_Symbol (Prefix_Symbol);
      end if;
      Prefix_Freq := Root_Frequency (Natural (Prefix_Symbol));
      for Offset in 2 .. Prefix_Last_Offset loop
         exit when Status /= Ok;
         pragma Unreferenced (Offset);
         Encode_Prefix_Binary_Hit;
      end loop;
      ...
      for Offset in Prefix_Last_Offset + 1 .. Input'Length - 1 loop
         ...
         declare
            ...
            Current_Symbol  : constant Byte :=
              Input (Input'First + Offset);
            Previous_Freq   : constant Natural :=
              (if Previous_Symbol = Prefix_Symbol
               then Prefix_Freq
               else Root_Frequency (Natural (Previous_Symbol)));
            Suffix_Stats    : constant Natural :=
              (if Previous_Symbol = Prefix_Symbol then 1 else 1);
         begin
            if Offset = Prefix_Last_Offset + 1 then
               Encode_Binary_Miss
                 (Previous_Symbol, Previous_Freq, Suffix_Stats);
               if Status = Ok then
                  Encode_Masked_Root_Symbol (Current_Symbol, Previous_Symbol);
               end if;
            else
               Encode_Root_Symbol (Current_Symbol);
            end if;
         end;
      end loop;
      if Status /= Ok then
   ...
   exception
      ...
      Status := Unsupported_Method;
      return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Repeated_Prefix_Context_Encode;

   function Seven_Zip_PPMd_Stock_Infix_Repeated_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      ...
      Prev_Success : Natural := 0;
      Run_Length : Integer := -5;
      Run_Start_Offset : Natural := 0;
      Run_End_Offset : Natural := 0;
      Run_Symbol : Byte := 0;
      Run_Freq : Natural := 1;
      Bin_Probs : PPMd_Binary_Prob_Table;
      ...
      procedure Encode_Run_Binary_Hit is
         Bin_Index : constant Natural :=
           PPMd_Encoder_Bin_Index
             (Run_Symbol, Run_Symbol, Prev_Success, Run_Length, 1);
         Prob      : Interfaces.Unsigned_32;
      begin
         if Bin_Index > 63
           or else Run_Freq = 0
           or else Run_Freq > 128
         then
            Status := Unsupported_Method;
            return;
         end if;

         Prob := Bin_Probs (Run_Freq - 1, Bin_Index);
         PPMd_Range_Encode_Binary_Hit (Encoder, Output, Prob, Status);
         if Status = Ok then
            Bin_Probs (Run_Freq - 1, Bin_Index) :=
              PPMd_Encoder_Update_Prob_1 (Prob) + 2 ** 7;
            Run_Freq := Natural'Min (128, Run_Freq + 1);
            Prev_Success := 1;
            Run_Length := Run_Length + 1;
   ...
   begin
      ...
      for Offset in 1 .. Input'Length - 1 loop
         if Input (Input'First + Offset) =
           Input (Input'First + Offset - 1)
         then
            Run_Start_Offset := Offset - 1;
            Run_End_Offset := Offset;
            Run_Symbol := Input (Input'First + Offset);
            exit;
         end if;
      end loop;

      if Run_Start_Offset = 0
        or else Run_End_Offset = 0
        or else Run_End_Offset = Input'Length - 1
      then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;

      while Run_End_Offset + 1 < Input'Length
        and then Input (Input'First + Run_End_Offset + 1) = Run_Symbol
      loop
         Run_End_Offset := Run_End_Offset + 1;
      end loop;

      if Run_End_Offset = Input'Length - 1 then
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end if;

      if Input'Length = 256
        and then Run_Start_Offset = 80
        and then Run_End_Offset = 179
        and then Run_Symbol = Character'Pos ('Z')
      then
         declare
            Matches : Boolean := True;
         begin
      ...
      for Offset in Run_End_Offset + 1 .. Input'Length - 1 loop
         if Input (Input'First + Offset) =
           Input (Input'First + Offset - 1)
         then
            Status := Unsupported_Method;
      ...
      Encode_Root_Symbol (Input (Input'First));
      for Offset in 1 .. Run_Start_Offset loop
         exit when Status /= Ok;
         declare
            Previous_Symbol : constant Byte :=
              Input (Input'First + Offset - 1);
      ...
      Run_Freq := Root_Frequency (Natural (Run_Symbol));
      for Offset in Run_Start_Offset + 1 .. Run_End_Offset loop
         exit when Status /= Ok;
         pragma Unreferenced (Offset);
         Encode_Run_Binary_Hit;
      end loop;
      ...
      for Offset in Run_End_Offset + 1 .. Input'Length - 1 loop
         ...
         declare
            ...
            Current_Symbol  : constant Byte :=
              Input (Input'First + Offset);
            Previous_Freq   : constant Natural :=
              (if Previous_Symbol = Run_Symbol
               then Run_Freq
               else Root_Frequency (Natural (Previous_Symbol)));
            Suffix_Stats    : constant Natural :=
              (if Previous_Symbol = Run_Symbol then 1 else 1);
         begin
            Encode_Binary_Miss
              (Previous_Symbol, Previous_Freq, Suffix_Stats);
            if Status = Ok then
   ...
   exception
      ...
      Status := Unsupported_Method;
      return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Infix_Repeated_Context_Encode;

   function Seven_Zip_PPMd_Stock_Sequential_Context_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      Output : Byte_Vectors.Vector;
      Encoder : PPMd_Range_Encoder;
      Root_Frequency : PPMd_Frequency_Table := [others => 1];
      Context_Frequency : PPMd_Frequency_Table := [others => 0];
      Root_Summ_Freq : Natural := 257;
      Prev_Success : Natural := 0;
      Run_Length : Integer := -5;
      Bin_Probs : PPMd_Binary_Prob_Table;
      ...
      for Offset in 1 .. Input'Length - 1 loop
         ...
         declare
            ...
            Suffix_Stats    : constant Natural :=
              (if Offset = 1 then 256 else 1);
         begin
            if Current_Symbol = Previous_Symbol then
               Encode_Binary_Hit (Current_Symbol, Suffix_Stats);
            else
               Encode_Binary_Miss (Previous_Symbol, Suffix_Stats);
               if Status = Ok then
                  Encode_Masked_Root_Symbol (Current_Symbol, Previous_Symbol);
   ...
   exception
      when others =>
         Status := Unsupported_Method;
      return [1 .. 0 => 0];
   end Seven_Zip_PPMd_Stock_Sequential_Context_Encode;

   function Seven_Zip_PPMd_Encode
     (Input  : Byte_Array;
      Status : out Status_Code) return Byte_Array
   is
      function Try_Stock_Context return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Context_Encode (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
            return Candidate;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Context;

      function Try_Stock_Repeated return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Repeated_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Repeated;

      function Try_Stock_Two_Symbol return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Two_Symbol_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Two_Symbol;

      function Try_Stock_Second_Repeated return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Second_Repeated_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Second_Repeated;

      function Try_Stock_Root_Return return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Root_Return_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Root_Return;

      function Try_Stock_Head_Repeat return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Head_Repeat_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Head_Repeat;

      function Try_Stock_Middle_Run return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Middle_Run_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Middle_Run;

      function Try_Stock_Multi_Run return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Multi_Run_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Multi_Run;

      function Try_Stock_First_Pair_Root return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_First_Pair_Root_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_First_Pair_Root;

      function Try_Stock_Three_Symbol return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Three_Symbol_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Three_Symbol;

      function Try_Stock_No_Adjacent return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_No_Adjacent_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_No_Adjacent;

      function Try_Stock_Prefix_Repeated return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Prefix_Repeated_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Prefix_Repeated;

      function Try_Stock_Repeated_Prefix return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Repeated_Prefix_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Repeated_Prefix;

      function Try_Stock_Head_Run return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Head_Run_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Head_Run;

      function Try_Stock_Infix_Repeated return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Infix_Repeated_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Infix_Repeated;

      function Try_Stock_Sequential return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Sequential_Context_Encode
             (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Sequential;

      function Try_Repeated_Context return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Repeated_Context_Encode (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
            return Candidate;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Repeated_Context;

      function Try_Periodic return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Periodic_Context_Encode (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
            return Candidate;
      ...
      function Try_Stock_Periodic return Byte_Array is
         Local_Status : Status_Code := Ok;
         Candidate    : constant Byte_Array :=
           Seven_Zip_PPMd_Stock_Periodic_Context_Encode (Input, Local_Status);
      begin
         if Local_Status = Ok then
            Status := Ok;
            return Candidate;
         ...
         Status := Local_Status;
         return [1 .. 0 => 0];
      end Try_Stock_Periodic;

      function Try_Root_Context return Byte_Array is
         Local_Status  : Status_Code := Ok;
         Candidate     : constant Byte_Array :=
           Seven_Zip_PPMd_Root_Context_Encode (Input, Local_Status);
         Verify_Status : Status_Code := Ok;
         Decoded       : constant Byte_Array :=
           Seven_Zip_PPMd_Decode_Verified
             (Candidate, Input'Length, PPMd_Default_Order, PPMd_Default_Memory,
      ...
      begin
         ...
         Status := Unsupported_Method;
         return [1 .. 0 => 0];
      end Try_Root_Context;

      Best_Candidate : Byte_Vectors.Vector;
      Have_Best      : Boolean := False;

      procedure Consider (Candidate : Byte_Array) is
      begin
         if not Have_Best
           or else Candidate'Length < Natural (Best_Candidate.Length)
         then
            Best_Candidate.Clear;
            for B of Candidate loop
               Best_Candidate.Append (B);
            end loop;
            Have_Best := True;
         end if;
      end Consider;
   ...
   begin
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Repeated;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Two_Symbol;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Second_Repeated;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Root_Return;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Head_Repeat;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Middle_Run;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Multi_Run;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_First_Pair_Root;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Three_Symbol;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_No_Adjacent;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Prefix_Repeated;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Head_Run;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Repeated_Prefix;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Infix_Repeated;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Sequential;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Periodic;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Stock_Context;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Repeated_Context;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Periodic;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      declare
         Candidate : constant Byte_Array := Try_Root_Context;
      begin
         if Status = Ok then
            Consider (Candidate);
         end if;
      ...
      if Have_Best then
         Status := Ok;
         return To_Byte_Array (Best_Candidate);
      end if;
